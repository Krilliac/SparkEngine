...
- [`readiness/work-items/`](readiness/work-items/) — dependency-ordered, code-session-ready implementation briefs.
  Each work item's `acceptanceStatus` records every acceptance criterion, in order, as `unmet`, `implemented` (committed code plus a committed test or check, cited as repository paths), or `evidenced` (additionally an exact-commit CI run cited as `ci:<workflow>/<run id>@<commit>`). A `criterionDigest` binds each entry to its criterion's exact wording, so rewording a criterion forces a fresh assessment. Only evidenced criteria count toward release: an item with any progress is at least `in-progress`, and a `done` item has every criterion evidenced. The generated handoff reports per-item and overall progress.
