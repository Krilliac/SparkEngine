...
def scan_editor_panels(project_root: pathlib.Path) -> list[str]:
    """Discover editor panel names from header files."""
    panel_dir = project_root / "SparkEditor" / "Source" / "Panels"
    if not panel_dir.is_dir():
        return []
    panels = []
    for f in sorted(panel_dir.glob("*Panel.h")):
        name = f.stem.replace("Panel", "")
...
def scan_ecs_systems(project_root: pathlib.Path) -> list[str]:
    """Discover ECS system class names."""
    path = project_root / "SparkEngine" / "Source" / "Engine" / "ECS" / "Systems" / "ECSystems.h"
    if not path.is_file():
        return []
    systems = []
    for line in path.read_text(errors="replace").splitlines():
        m = re.search(r'class\s+(\w+System)\s*:', line)
        if m:
            systems.append(m.group(1))
    return systems


def scan_components(project_root: pathlib.Path) -> list[str]:
    """Discover ECS component struct names."""
    comp_dir = project_root / "SparkEngine" / "Source" / "Engine" / "ECS" / "Components"
    if not comp_dir.is_dir():
        return []
    components = []
    for f in sorted(comp_dir.glob("*.h")):
        for line in f.read_text(errors="replace").splitlines():
            m = re.search(r'^\s*struct\s+(\w+)\s*(?:\{|$)', line)
            if m and not m.group(1).startswith("_"):
                components.append(m.group(1))
    return components


def scan_rhi_backends(project_root: pathlib.Path) -> list[str]:
    """Discover RHI backend directories."""
    rhi_dir = project_root / "SparkEngine" / "Source" / "Graphics" / "RHI"
    if not rhi_dir.is_dir():
        return []
    backends = []
    for d in sorted(rhi_dir.iterdir()):
        if d.is_dir() and d.name not in (".", ".."):
            backends.append(d.name)
    null_rhi = rhi_dir / "NullRHIDevice.h"
    if null_rhi.is_file() and "NullRHI" not in backends:
        backends.append("NullRHI")
    return backends



def generate_page(args) -> str:
    """Generate the full markdown page content."""
    project_root = pathlib.Path(args.project_root)

    # Live scans
    panels = scan_editor_panels(project_root)
    systems = scan_ecs_systems(project_root)
    components = scan_components(project_root)
    backends = scan_rhi_backends(project_root)

    n_panels = len(panels) if panels else int(args.panels)
    n_systems = len(systems) if systems else int(args.systems)
    ...
    # HEADER
    # ================================================================
    sections.append(f"""# Engine Architecture Flowchart
...
""")
    ...
    # ================================================================
    module_str = ", ".join(args.modules.split()) if args.modules else "SparkGame, SparkGameMMO"
    sections.append(f"""
...
```
 ...
 │ - Windowed or      │  │ - {n_panels} editor panels │  │ - Command input │
 ...
 └────────────────────┬─────────────────────┘
        ...
        ▼             ▼              ▼
 ┌────────────┐ ┌───────────┐ ┌────────────┐
 │{module_str.split(",")[0].strip():^12s}│ │ Module 2  │ │  Module N  │
...
""")
    ...
    # ================================================================
    # SECTION 2: STARTUP & INITIALIZATION
    # ================================================================
    sections.append("""
...
""")
    ...
    # ================================================================
    # SECTION 6: RHI ABSTRACTION
    # ================================================================
    backend_boxes = "\n".join(f"     │ ├─ {b}" for b in backend_list)
    sections.append(f"""
...
""")
    ...
    # ================================================================
    # SECTION 13: WORLD STREAMING & ORIGIN REBASING
    # ================================================================
    sections.append("""
...
""")
    ...
    # ================================================================
    # SECTION 14: EDITOR ARCHITECTURE
    # ================================================================
    panel_list = "\n".join(f" │  {p:<40s}│" for p in panels[:20]) if panels else " │  (scan your SparkEditor/Source/Panels/)  │"
    remaining = len(panels) - 20 if len(panels) > 20 else 0
    if remaining > 0:
        panel_list += f"\n │  ... and {remaining} more panels{' ' * (27 - len(str(remaining)))}│"

    sections.append(f"""
## 14. Editor Architecture (SparkEditor)

The editor is a standalone executable that embeds the engine and adds a Dear ImGui-based interface with {n_panels} specialized panels.
...
- `SparkEditor/Source/Panels/` — All {n_panels} panel implementations
...
""")
    ...
    # ================================================================
    # SECTION 15: GAME MODULE LIFECYCLE
    # ================================================================
    sections.append("""
...
""")
    ...
    # ================================================================
    # SECTION 16: SUPPORTING SUBSYSTEMS
    # ================================================================
    sections.append("""
...
def main():
    parser = argparse.ArgumentParser(description="Generate Engine Architecture Flowchart wiki page")
    parser.add_argument("--output", required=True, help="Output markdown file path")
    parser.add_argument("--headers", default="0", help="Number of header files")
    parser.add_argument("--sources", default="0", help="Number of source files")
    parser.add_argument("--components", default="0", help="Number of ECS components")
    parser.add_argument("--systems", default="0", help="Number of ECS systems")
    parser.add_argument("--panels", default="0", help="Number of editor panels")
    parser.add_argument("--backends", default="", help="Space-separated RHI backends")
    parser.add_argument("--modules", default="", help="Space-separated game module names")
    parser.add_argument("--project-root", required=True, help="Project root directory")

    args = parser.parse_args()
    content = generate_page(args)
