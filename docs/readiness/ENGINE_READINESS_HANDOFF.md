...
| `G07` Security and hostile-input safety | security | **blocked** | yes | Threat model and security ownership are current; Authenticated encryption, replay protection, rotation, bounds, and version negotiation pass; Credentials never appear in captures/logs/dumps; Network, asset, save, and script fuzzers meet budgets | `SEC-100`, `NET-100`, `SEC-120` |
| `G08` Platform support certification | platform | **blocked** | yes | Every supported platform has a declared compiler/OS/device matrix; Build, install, launch, content, input, audio, crash, save, upgrade, and uninstall pass; Experimental and unsupported targets remain labeled | `PLT-200`, `PLT-210`, `PLT-220` |
| `G09` Renderer parity and visual correctness | rendering | **blocked** | yes | D3D11 primary scene suite passes; Each supported backend meets its declared pass subset and visual thresholds; Skinned glTF import works; GPU captures and budgets attach to the tested SHA | `RHI-210`, `RHI-220`, `RHI-225`, `RHI-230`, `RHI-240`, `ENG-220` |
...
|---|---|---|---|---|---|---|---|
| `platform.windows` Windows host | unassigned | complete | integration-tested | primary | **candidate** | `G02`, `G03`, `G05`, `G08`, `G17` | `CI-100`, `REL-100`, `PLT-200` |
...
| `runtime.nullrhi` NullRHI headless runtime | unassigned | complete | integration-tested | supported | **candidate** | `G02`, `G03`, `G10`, `G14` | `HEAD-220`, `OPS-110` |
