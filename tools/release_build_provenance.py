...
SCHEMA_VERSION = "spark-build-provenance-v1"
...
RECORD_KEYS = {"schemaVersion", "sourceSHA", "version", "profile", "platform", "configuration",
...
def _load_record(path: Path) -> dict[str, Any]:
    ...
    record_doc = _closed(document, RECORD_KEYS, f"{path.name} schema")
    _require(record_doc["schemaVersion"] == SCHEMA_VERSION, f"{path.name} has an unknown schemaVersion")
...
def verify(args: argparse.Namespace) -> tuple[int, int]:
    ...
    if args.stable:
        ...
        only = records[0]
        _require((only["profile"], only["platform"], only["configuration"]) == ("stable-v1", "Windows", "MinSizeRel"),
