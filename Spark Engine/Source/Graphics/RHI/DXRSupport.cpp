...
DXRManager::DXRStats DXRManager::GetStats() const {
    ...
    for (const auto& blas : m_blasList) m_stats.accelerationStructureMemory += blas.size;
...
void DXRManager::Console_EnableFeature(const std::string& feature, bool enabled) {
    ...
    if (feature == "reflections") flag = RTFeature::Reflections;
    else if (feature == "shadows") flag = RTFeature::Shadows;
    else if (feature == "ao") flag = RTFeature::AmbientOcclusion;
    else if (feature == "gi") flag = RTFeature::GlobalIllumination;
    else return;

    if (enabled)
        m_settings.enabledFeatures = m_settings.enabledFeatures | flag;
    else
        m_settings.enabledFeatures = static_cast<RTFeature>(
            static_cast<uint32_t>(m_settings.enabledFeatures) & ~static_cast<uint32_t>(flag));
}
