...
public:
    ...
    void EndFrame() {
        ...
        if (m_fpsAccumulator >= 1000.0f) {
            ...
            if (m_currentFPS < m_minFPS || m_minFPS == 0.0f) m_minFPS = m_currentFPS;
        }
    ...
    float GetAverageFPS() const {
        if (m_totalFrameTimeMs <= 0.0f) return 0.0f;
    ...
    std::string GetSummary() const {
        ...
        ss << "FPS: " << static_cast<int>(m_currentFPS)
           << " (min:" << static_cast<int>(m_minFPS)
           << " max:" << static_cast<int>(m_maxFPS)
           << ") Frame: " << GetAverageFrameTime() << "ms"
           << " Draw calls: " << m_drawCallCount
           << " Entities: " << m_entityCount
           << " Tris: " << m_triangleCount;
