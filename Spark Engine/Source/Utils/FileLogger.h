...
public:
     ...
     */
    bool Initialize(const std::string& directory = "Logs/",
                    FileLogLevel minLevel = FileLogLevel::Debug)
    {
               ...
               const char* func = nullptr)
    {
        ...
        if (level < m_config.minimumLevel) return;
...
private:
    ...
    void RotateLogFile()
    {
        ...
        std::string ext = (m_config.format == LogFileFormat::CSV) ? ".csv" : ".log";
        m_currentFilePath = m_config.outputDirectory + m_config.filenamePrefix + "_" + timeBuf +
                            "_" + std::to_string(m_fileIndex) + ext;
    ...
    void WritePlainTextLine(FileLogLevel level, const char* category, const char* message,
                            const char* file, int line, const char* func)
    {
        ...
        offset += snprintf(buf + offset, sizeof(buf) - offset, "[%s][%-8s] %s",
    ...
    void WriteCSVLine(FileLogLevel level, const char* category, const char* message,
                      const char* file, int line, const char* func)
    {
        auto now = std::chrono::system_clock::now();
        std::time_t t = std::chrono::system_clock::to_time_t(now);
    ...
    static std::string LevelToString(FileLogLevel level)
    {
        switch (level)
        {
            case FileLogLevel::Trace: return "TRACE";
            case FileLogLevel::Debug: return "DEBUG";
            case FileLogLevel::Info:  return "INFO";
            case FileLogLevel::Warn:  return "WARN";
            case FileLogLevel::Error: return "ERROR";
            case FileLogLevel::Fatal: return "FATAL";
            default: return "?????";
        }
    }
