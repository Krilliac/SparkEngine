...
public:
    ...
    const char* GetStateString() const
    {
        switch (m_state)
        {
            case FrameState::Running:    return "Running";
            case FrameState::Paused:     return "Paused";
            case FrameState::Stepping:   return "Stepping";
            case FrameState::SlowMotion: return "SlowMotion";
     ...
     */
    void RegisterStateProvider(const std::string& key,
    ...
    void RemoveStateProvider(const std::string& key)
    {
        m_stateProviders.erase(key);
    }
    ...
    {
        if (index1 < 0 || index1 >= static_cast<int>(m_snapshots.size()) ||
            index2 < 0 || index2 >= static_cast<int>(m_snapshots.size()))
        {
    ...
    void AddBreakpointFrame(uint64_t frame)
    {
        m_breakpointFrames.push_back(frame);
    }
     ...
     */
    void AddBreakCondition(const std::string& name,
