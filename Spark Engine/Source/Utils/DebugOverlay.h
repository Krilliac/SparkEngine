...
public:
    ...
    void Update(float deltaTime)
    {
        ...
        for (const auto& s : m_frameHistory)
        {
            ...
            {
                sum += s.frameTimeMs;
                if (s.frameTimeMs < minT) minT = s.frameTimeMs;
                if (s.frameTimeMs > maxT) maxT = s.frameTimeMs;
                count++;
            }
        }
    ...
    void SetSection(OverlaySection section, bool visible)
    {
        m_sectionVisible[static_cast<int>(section)] = visible;
    }

    bool IsSectionVisible(OverlaySection section) const
    {
        return m_sectionVisible[static_cast<int>(section)];
    }
     ...
     */
    void AddCustomLine(const std::string& key,
    ...
    const std::array<FrameTimeSample, FRAME_HISTORY_SIZE>& GetFrameHistory() const
    {
    ...
    std::vector<OverlayStat> BuildStatsLines() const
    {
        ...
        {
            char buf[128];
            snprintf(buf, sizeof(buf), "FPS: %.0f (%.2f ms avg, %.2f-%.2f ms)",
                     m_currentFPS, m_avgFrameTime, m_minFrameTime, m_maxFrameTime);

            DirectX::XMFLOAT4 fpsColor = {0, 1, 0, 1}; // Green
            if (m_currentFPS < 30.0f) fpsColor = {1, 0, 0, 1}; // Red
            else if (m_currentFPS < 60.0f) fpsColor = {1, 1, 0, 1}; // Yellow
        ...
        if (!m_compactMode)
        {
            ...
            {
                char buf[128];
                snprintf(buf, sizeof(buf), "Draw Calls: %d | Triangles: %d",
                         m_drawCalls, m_triangles);
            ...
            {
                char buf[128];
                snprintf(buf, sizeof(buf), "RAM: %.1f MB | VRAM: %.1f MB",
                         m_memoryUsageMB, m_vramUsageMB);
            ...
            {
                char buf[128];
                snprintf(buf, sizeof(buf), "Entities: %d (%d visible)",
                         m_entities, m_visibleEntities);
            ...
            if (m_sectionVisible[static_cast<int>(OverlaySection::Timing)])
            {
                ...
                for (const auto& [name, timeMs] : m_systemTimings)
                {
                    ...
                    if (timeMs > 8.0f) color = {1, 0, 0, 1};
                    else if (timeMs > 4.0f) color = {1, 1, 0, 1};
            ...
            {
                char buf[128];
                snprintf(buf, sizeof(buf), "Pos: (%.1f, %.1f, %.1f) Rot: (%.1f, %.1f, %.1f)",
                         m_cameraPos.x, m_cameraPos.y, m_cameraPos.z,
                         m_cameraRot.x, m_cameraRot.y, m_cameraRot.z);
