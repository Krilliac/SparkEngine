...
public:
    ...
    Octree(
        ...
        const DirectX::XMFLOAT3& worldMax,
        int maxDepth = 6,
        int maxPerNode = 8,
        float looseness = 2.0f)
        : m_maxDepth(maxDepth)
    ...
    bool Remove(const T& item) {
        auto it = m_itemNodes.find(item);
        if (it == m_itemNodes.end()) return false;
        RemoveFromNode(it->second, item);
...
private:
    ...
    Graphics::AABB GetChildBounds(const Node* node, int childIndex) const {
        ...
        if (m_looseness > 1.0f) {
            ...
            tight.min = {tc.x - ext.x * (1.0f + expand), tc.y - ext.y * (1.0f + expand), tc.z - ext.z * (1.0f + expand)};
            tight.max = {tc.x + ext.x * (1.0f + expand), tc.y + ext.y * (1.0f + expand), tc.z + ext.z * (1.0f + expand)};
        }
    ...
    void InsertIntoNode(Node* node, const T& item, const Graphics::AABB& bounds) {
        ...
        // Subdivide if needed
        if (!node->hasChildren
            && static_cast<int>(node->entries.size()) > m_maxPerNode
    ...
    void QueryAABBRecursive(const Node* node, const Graphics::AABB& queryBox, std::vector<T>& results) const {
        if (!AABBOverlap(node->bounds, queryBox)) return;
    ...
    void QuerySphereRecursive(const Node* node, const DirectX::XMFLOAT3& center, float radius, std::vector<T>& results) const {
        if (!SphereAABBOverlap(center, radius, node->bounds)) return;
    ...
    void QueryFrustumRecursive(const Node* node, const Graphics::Frustum& frustum, std::vector<T>& results) const {
        ...
        if (result == Graphics::CullResult::Outside) return;
    ...
    void ForEachInAABBRecursive(const Node* node, const Graphics::AABB& queryBox, const std::function<void(const T&)>& callback) const {
        if (!AABBOverlap(node->bounds, queryBox)) return;
