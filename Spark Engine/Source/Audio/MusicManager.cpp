...
std::string Playlist::GetCurrentTrack() const {
    if (trackNames.empty() || currentIndex < 0 ||
        currentIndex >= static_cast<int>(trackNames.size()))
        return "";
...
void Playlist::Advance() {
    if (trackNames.empty()) return;
    if (mode == PlaylistMode::LoopOne) return;
...
float AudioMixer::GetEffectiveVolume(AudioBus bus) const {
    if (m_buses[static_cast<int>(bus)].muted) return 0.0f;
...
void AudioMixer::FadeBus(AudioBus bus, float targetVolume, float duration) {
    ...
    m_activeFades.erase(
        std::remove_if(m_activeFades.begin(), m_activeFades.end(),
...
std::string AudioMixer::Console_GetMixerInfo() const {
    ...
    for (int i = 0; i < static_cast<int>(AudioBus::Count); ++i) {
        ss << "  " << busNames[i] << ": "
           << (m_buses[i].muted ? "[MUTED] " : "")
           << "Vol=" << m_buses[i].volume
...
void MusicManager::Play(const std::string& trackName, float fadeInDuration) {
    if (m_tracks.find(trackName) == m_tracks.end()) return;
...
void MusicManager::Pause() { m_isPaused = true; }
void MusicManager::Resume() { m_isPaused = false; }

void MusicManager::CrossfadeTo(const std::string& trackName, float duration) {
    if (m_tracks.find(trackName) == m_tracks.end()) return;
    if (trackName == m_currentTrack) return;
...
void MusicManager::PreviousTrack() {
    ...
    if (!prev.empty()) CrossfadeTo(prev);
}

void MusicManager::SetPlaylistMode(PlaylistMode mode) {
    if (m_activePlaylist.empty()) return;
    auto it = m_playlists.find(m_activePlaylist);
    if (it != m_playlists.end()) it->second.mode = mode;
}
...
void MusicManager::SetCombatIntensity(CombatIntensity intensity) {
    if (intensity == m_dynamicState.currentIntensity) return;

    m_targetIntensity = intensity;
    std::string targetTrack;
    switch (intensity) {
        case CombatIntensity::Exploration: targetTrack = m_dynamicState.explorationTrack; break;
        case CombatIntensity::LowThreat: targetTrack = m_dynamicState.lowThreatTrack; break;
        case CombatIntensity::Combat: targetTrack = m_dynamicState.combatTrack; break;
        case CombatIntensity::BossFight: targetTrack = m_dynamicState.bossFightTrack; break;
    }
...
OcclusionResult MusicManager::ComputeOcclusion(const XMFLOAT3& sourcePos,
                                                 const XMFLOAT3& listenerPos) const {
    OcclusionResult result{0.0f, 22000.0f};
    if (!m_occlusionSettings.enabled) return result;
...
void MusicManager::UpdateCrossfade(float deltaTime) {
    ...
    float t = (m_crossfadeDuration > 0.0f)
        ? (std::min)(m_crossfadeProgress / m_crossfadeDuration, 1.0f)
        : 1.0f;
...
std::string MusicManager::Console_GetStatus() const {
    ...
    switch (m_dynamicState.currentIntensity) {
        case CombatIntensity::Exploration: ss << "Exploration"; break;
        case CombatIntensity::LowThreat: ss << "Low Threat"; break;
        case CombatIntensity::Combat: ss << "Combat"; break;
        case CombatIntensity::BossFight: ss << "Boss Fight"; break;
    }
    ss << "\nReverb Zones: " << m_reverbZones.size() << "\n";
    if (m_activeReverbZone) ss << "Active Reverb: " << m_activeReverbZone->name << "\n";
    return ss.str();
}
