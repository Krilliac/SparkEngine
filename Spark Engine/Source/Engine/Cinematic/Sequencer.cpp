...
CameraKeyframe CameraPathTrack::Evaluate(float time) const {
    ...
    if (kf0.interpolation == InterpolationMode::CatmullRom && m_keyframes.size() >= 4) {
        ...
        result.position = CatmullRomInterpolate(
            m_keyframes[i0].position, kf0.position,
            kf1.position, m_keyframes[i3].position, t);
        result.lookAt = CatmullRomInterpolate(
            m_keyframes[i0].lookAt, kf0.lookAt,
            kf1.lookAt, m_keyframes[i3].lookAt, t);
    } else {
...
XMFLOAT3 CameraPathTrack::CatmullRomInterpolate(
...
float EntityTransformTrack::GetDuration() const {
    ...
    auto getMax = [&](const auto& keys) {
        if (!keys.empty()) maxTime = (std::max)(maxTime, keys.back().time);
    };
...
XMFLOAT3 EntityTransformTrack::EvaluateScale(float time) const {
    ...
    if (m_scaleKeys.empty()) return {1.0f, 1.0f, 1.0f};
    return result;
}
...
float EntityPropertyTrack::Evaluate(float time) const {
    if (m_keyframes.empty()) return 0.0f;
    if (m_keyframes.size() == 1 || time <= m_keyframes.front().time) return m_keyframes.front().value;
...
void EventTrack::AddCue(const EventCue& cue) {
    m_cues.push_back(cue);
    std::sort(m_cues.begin(), m_cues.end(),
              [](const EventCue& a, const EventCue& b) { return a.time < b.time; });
}
...
float SubtitleTrack::GetDuration() const {
    if (m_subtitles.empty()) return 0.0f;
...
FadeKeyframe FadeTrack::Evaluate(float time) const {
    if (m_keyframes.empty()) return {0.0f, 0.0f, {0,0,0}};
    if (m_keyframes.size() == 1 || time <= m_keyframes.front().time) return m_keyframes.front();
...
EntityPropertyTrack* Sequence::AddEntityPropertyTrack(const std::string& trackName, uint32_t entityID, const std::string& property) {
...
void Sequence::Update(float deltaTime) {
    if (m_playState != SequencePlayState::Playing) return;
...
bool SequencerManager::IsAnyCutscenePlaying() const {
    for (const auto& [name, seq] : m_sequences) {
        if (seq->GetPlayState() == SequencePlayState::Playing) return true;
    }
    return false;
}
...
Sequence* SequencerManager::GetActiveSequence() {
    for (auto& [name, seq] : m_sequences) {
        if (seq->GetPlayState() == SequencePlayState::Playing) return seq.get();
    }
    return nullptr;
}
...
std::string SequencerManager::Console_ListSequences() const {
    ...
    for (const auto& [name, seq] : m_sequences) {
        ...
        if (seq->GetPlayState() == SequencePlayState::Playing) stateStr = "Playing";
        else if (seq->GetPlayState() == SequencePlayState::Paused) stateStr = "Paused";

        ss << "  " << name << " [" << stateStr << "] "
           << seq->GetTracks().size() << " tracks, "
           << seq->GetDuration() << "s\n";
    }
    return ss.str();
}
...
std::string SequencerManager::Console_GetSequenceInfo(const std::string& name) const {
    auto it = m_sequences.find(name);
    if (it == m_sequences.end()) return "Sequence '" + name + "' not found\n";
    ...
    for (const auto& track : seq->GetTracks()) {
        ...
        switch (track->GetType()) {
            case TrackType::CameraPath: typeStr = "Camera"; break;
            case TrackType::EntityProperty: typeStr = "Property"; break;
            case TrackType::EntityTransform: typeStr = "Transform"; break;
            case TrackType::AudioCue: typeStr = "Audio"; break;
            case TrackType::Event: typeStr = "Event"; break;
            case TrackType::Subtitle: typeStr = "Subtitle"; break;
            case TrackType::Fade: typeStr = "Fade"; break;
        }
        ss << "  [" << typeStr << "] " << track->GetName()
           << " (" << track->GetDuration() << "s)"
