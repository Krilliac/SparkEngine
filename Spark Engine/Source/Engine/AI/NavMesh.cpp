...
NavMeshQuery::NavMeshQuery(const NavMeshData* navMesh)
    : m_navMesh(navMesh) {}

NavMeshHit NavMeshQuery::FindNearestPoint(const XMFLOAT3& position, float searchRadius) const {
    NavMeshHit hit{};
    if (!m_navMesh || m_navMesh->triangles.empty()) return hit;
...
PathResult NavMeshQuery::FindPath(const PathRequest& request) const {
    PathResult result;
    if (!m_navMesh || m_navMesh->triangles.empty()) return result;

    uint32_t startTri = FindContainingTriangle(request.start);
    uint32_t endTri = FindContainingTriangle(request.end);

    if (startTri == UINT32_MAX || endTri == UINT32_MAX) return result;
    if (startTri == endTri) {
        result.found = true;
    ...
    float h = HeuristicCost(m_navMesh->triangles[startTri].centroid,
                             m_navMesh->triangles[endTri].centroid);
    ...
    while (!openSet.empty()) {
        ...
        for (int e = 0; e < 3; ++e) {
            ...
            if (neighbor == UINT32_MAX || closed[neighbor]) continue;

            // Check flags
            if ((m_navMesh->triangles[neighbor].flags & request.includeFlags) == 0) continue;
            if ((m_navMesh->triangles[neighbor].flags & request.excludeFlags) != 0) continue;
            ...
            if (newG < gCosts[neighbor]) {
                ...
                float newH = HeuristicCost(m_navMesh->triangles[neighbor].centroid,
                                           m_navMesh->triangles[endTri].centroid);
...
uint32_t NavMeshQuery::FindContainingTriangle(const XMFLOAT3& point) const {
    if (!m_navMesh) return UINT32_MAX;
...
std::unique_ptr<NavMeshData> NavMeshBuilder::Build(
    ...
    if (indices.size() < 3 || vertices.empty()) return navMesh;
    ...
    for (size_t i = 0; i + 2 < indices.size(); i += 3) {
        ...
        if (slopeAngle > settings.agentMaxSlope) continue;
...
std::unique_ptr<NavMeshData> NavMeshBuilder::BuildFromHeightfield(
...
bool NavMeshManager::BuildNavMesh(const std::string& name,
                                   const std::vector<XMFLOAT3>& vertices,
                                   const std::vector<uint32_t>& indices,
                                   const NavMeshBuildSettings& settings) {
...
std::string NavMeshManager::Console_ListNavMeshes() const {
    ...
    for (const auto& [name, nm] : m_navMeshes) {
        ss << "  " << name << " [" << nm->triangles.size() << " triangles, "
           << nm->vertices.size() << " vertices]\n";
    }
    return ss.str();
}
...
std::string NavMeshManager::Console_GetNavMeshInfo(const std::string& name) const {
    auto it = m_navMeshes.find(name);
    if (it == m_navMeshes.end()) return "NavMesh '" + name + "' not found\n";
    ...
    ss << "Bounds: (" << nm->boundsMin.x << "," << nm->boundsMin.y << "," << nm->boundsMin.z
       << ") to (" << nm->boundsMax.x << "," << nm->boundsMax.y << "," << nm->boundsMax.z << ")\n";
    return ss.str();
}
