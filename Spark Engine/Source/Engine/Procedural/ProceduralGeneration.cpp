...
float NoiseGenerator::Fade(float t) const { return t * t * t * (t * (t * 6 - 15) + 10); }
float NoiseGenerator::Lerp(float a, float b, float t) const { return a + t * (b - a); }
...
float NoiseGenerator::RidgedMultifractal(float x, float y, int octaves,
...
std::vector<float> HeightmapGenerator::Generate(const HeightmapSettings& settings) {
    ...
    if (settings.applyErosion) {
        ApplyHydraulicErosion(heightmap, settings.width, settings.height,
                              settings.erosionIterations, settings.seed);
    }
...
void HeightmapGenerator::ApplyHydraulicErosion(std::vector<float>& heightmap, int width, int height,
...
void HeightmapGenerator::Normalize(std::vector<float>& heightmap) {
    ...
    for (auto& h : heightmap) h = (h - minH) / range;
}
...
ProceduralMeshData ProceduralMesh::CreateTerrainFromHeightmap(
    const float* heightmap, int width, int height, float cellSize, float heightScale) {
    ProceduralMeshData mesh;
...
ProceduralMeshData ProceduralMesh::CreateTree(float height, int branches, uint32_t seed) {
    ...
    // Offset trunk up
    for (auto& v : trunk.vertices) v.position.y += height * 0.3f;
    return trunk;
}
...
std::vector<PlacementResult> ObjectPlacer::PlaceObjects(
    const float* heightmap, int width, int height,
    ...
    for (const auto& rule : rules) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (normalizedH < rule.minHeight || normalizedH > rule.maxHeight) continue;

            // Noise mask
            if (rule.noiseMaskThreshold > 0.0f) {
                float nm = noise.Perlin2D(px / rule.noiseMaskScale, py / rule.noiseMaskScale);
                if (nm < rule.noiseMaskThreshold) continue;
            }
...
bool WaveFunctionCollapse::Collapse(uint32_t seed) {
    ...
    while (!m_grid.IsCollapsed()) {
        if (m_grid.HasContradiction()) return false;
        ...
        for (int id : cell) weights.push_back(m_tiles[id].weight);
...
const WFCTile* WaveFunctionCollapse::GetTile(int id) const {
    if (id >= 0 && id < static_cast<int>(m_tiles.size())) return &m_tiles[id];
    return nullptr;
}
...
void WaveFunctionCollapse::Propagate(int x, int y) {
    ...
    while (!toProcess.empty()) {
        ...
        for (int dir = 0; dir < 4; ++dir) {
            int nx = cx + dx[dir];
            int ny = cy + dy[dir];
            if (nx < 0 || nx >= m_grid.width || ny < 0 || ny >= m_grid.height) continue;

            auto& neighborCell = m_grid.cells[ny][nx];
            if (neighborCell.size() <= 1) continue;
            ...
            neighborCell.erase(
                std::remove_if(neighborCell.begin(), neighborCell.end(),
                    [&](int tileId) {
                        const auto& neighborSocket = m_tiles[tileId].sockets[oppositeDir[dir]];
                        for (const auto& vs : validSockets) {
                            if (AreSocketsCompatible(vs, neighborSocket)) return false;
                        }
                        return true;
                    }),
                neighborCell.end());
