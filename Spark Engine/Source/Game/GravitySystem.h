...
public:
     ...
     */
    int CreateLowGravityZone(const std::string& name, const XMFLOAT3& center,
                              const XMFLOAT3& halfExtents, float strength = -5.0f);

    /**
     * @brief Create a zero-gravity zone
     */
    int CreateZeroGravityZone(const std::string& name, const XMFLOAT3& center,
                               const XMFLOAT3& halfExtents);

    /**
     * @brief Create a reverse gravity zone
     */
    int CreateReverseGravityZone(const std::string& name, const XMFLOAT3& center,
                                  const XMFLOAT3& halfExtents, float strength = 15.0f);

    /**
     * @brief Create a high-gravity zone
     */
    int CreateHighGravityZone(const std::string& name, const XMFLOAT3& center,
                               const XMFLOAT3& halfExtents, float strength = -40.0f);

    /**
     * @brief Create a radial gravity zone (pulls toward center)
     */
    int CreateRadialGravityZone(const std::string& name, const XMFLOAT3& center,
                                 float radius, float strength = -15.0f);

    // === Console Integration ===

    std::string Console_ListZones() const;
    bool Console_AddZone(const std::string& name, const std::string& type,
