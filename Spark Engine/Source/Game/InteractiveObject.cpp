...
InteractiveObject::InteractiveObject(InteractiveObjectType type)
    : m_objectType(type)
{
    SetName("InteractiveObject");
}
...
{
    if (!m_isInteractable || !player) return false;
...
{
    if (m_isLocked) return "[E] Locked";
    if (IsOpen() || m_doorState == DoorState::Opening) return "[E] Close Door";
    return "[E] Open Door";
}
...
{
    if (!player || !m_classSystem) return false;
...
void ElevatorObject::Update(float deltaTime)
{
    ...
    XMFLOAT3 pos = {
        m_bottomPosition.x + (m_topPosition.x - m_bottomPosition.x) * m_moveProgress,
...
{
    if (m_state == ElevatorState::AtBottom) return "[E] Go Up";
    if (m_state == ElevatorState::AtTop) return "[E] Go Down";
...
{
    if (!player || m_cooldownTimer > 0.0f) return false;
    ...
    player->Console_SetPosition(
        player->GetPosition().x + m_launchDirection.x * 0.1f,
...
{
    if (m_cooldownTimer > 0.0f) return "[E] Recharging...";
    return "[E] Teleport";
}
...
{
    if (!player || !m_isAvailable) return false;
...
std::string PickupObject::GetInteractionPrompt() const
{
    ...
    switch (m_objectType) {
    case InteractiveObjectType::HEALTH_PICKUP: return "[E] Health +" + std::to_string((int)m_amount);
    case InteractiveObjectType::ARMOR_PICKUP: return "[E] Armor +" + std::to_string((int)m_amount);
    case InteractiveObjectType::AMMO_PICKUP: return "[E] Ammo +" + std::to_string((int)m_amount);
    case InteractiveObjectType::WEAPON_PICKUP: return "[E] Pick Up Weapon";
    case InteractiveObjectType::SHIELD_PICKUP: return "[E] Shield +" + std::to_string((int)m_amount);
    default: return "[E] Pick Up";
    }
}
...
void InteractionSystem::Update(float deltaTime, Player* player)
{
    ...
    if (m_highlightCheckTimer >= HIGHLIGHT_CHECK_INTERVAL && player) {
        ...
        for (auto& obj : m_objects) {
            ...
            if (!obj->IsPlayerInRange(player)) continue;
...
{
    if (!m_highlightedObject || !player) return false;
...
void InteractionSystem::RemoveObject(InteractiveObject* obj)
{
    m_objects.erase(
        std::remove_if(m_objects.begin(), m_objects.end(),
            [obj](const std::unique_ptr<InteractiveObject>& o) {
                return o.get() == obj;
            }),
        m_objects.end());
...
DoorObject* InteractionSystem::SpawnDoor(const XMFLOAT3& position, const XMFLOAT3& scale,
...
ClassTerminal* InteractionSystem::SpawnClassTerminal(const XMFLOAT3& position,
...
PickupObject* InteractionSystem::SpawnPickup(InteractiveObjectType type, const XMFLOAT3& position,
                                              float amount, ID3D11Device* device, ID3D11DeviceContext* context)
{
...
JumpPadObject* InteractionSystem::SpawnJumpPad(const XMFLOAT3& position, float force,
                                                ID3D11Device* device, ID3D11DeviceContext* context)
{
...
void InteractionSystem::SpawnTeleporterPair(const XMFLOAT3& posA, const XMFLOAT3& posB,
                                             ID3D11Device* device, ID3D11DeviceContext* context)
{
...
ElevatorObject* InteractionSystem::SpawnElevator(const XMFLOAT3& bottom, const XMFLOAT3& top,
                                                  ID3D11Device* device, ID3D11DeviceContext* context)
{
