...
XMFLOAT3 GravityZone::GetGravityAt(const XMFLOAT3& point) const
{
    ...
    case GravityZoneType::DIRECTIONAL:
        return {
            gravityDirection.x * std::abs(gravityStrength),
            gravityDirection.y * std::abs(gravityStrength),
...
{
    if ((int)m_zones.size() >= MAX_ZONES) return -1;
...
void GravitySystem::RemoveZoneByName(const std::string& name)
{
    m_zones.erase(
        std::remove_if(m_zones.begin(), m_zones.end(),
            [&name](const GravityZone& z) { return z.name == name; }),
        m_zones.end());
}
...
GravityState GravitySystem::UpdateObjectGravity(const XMFLOAT3& position,
                                                  GravityState& state, float dt) const
{
...
int GravitySystem::CreateRadialGravityZone(const std::string& name, const XMFLOAT3& center,
...
std::string GravitySystem::Console_ListZones() const
{
    ...
    ss << "World Gravity: (" << m_worldGravity.x << ", " << m_worldGravity.y
       << ", " << m_worldGravity.z << ")\n";

    const char* typeNames[] = {
        "Normal", "Low", "Zero", "High", "Reverse", "Directional", "Radial"
    };
...
bool GravitySystem::Console_AddZone(const std::string& name, const std::string& type,
                                      float cx, float cy, float cz,
                                      float hx, float hy, float hz, float strength)
{
    GravityZoneType zoneType = GravityZoneType::NORMAL;
    if (type == "low") zoneType = GravityZoneType::LOW_GRAVITY;
    else if (type == "zero") zoneType = GravityZoneType::ZERO_GRAVITY;
    else if (type == "high") zoneType = GravityZoneType::HIGH_GRAVITY;
    else if (type == "reverse") zoneType = GravityZoneType::REVERSE;
    else if (type == "radial") zoneType = GravityZoneType::RADIAL;
