...
public:
    ...
    enum class DoorStyle { Sliding, Rotating };
...
public:
    enum class ElevatorState { AtBottom, MovingUp, AtTop, MovingDown };
...
public:
    ...
    void SetHealth(float health) { m_objectHealth = health; m_maxObjectHealth = health; }
...
public:
     ...
     * @brief Spawn a door
     */
    DoorObject* SpawnDoor(const XMFLOAT3& position, const XMFLOAT3& scale,
                          ID3D11Device* device, ID3D11DeviceContext* context);

    /**
     * @brief Spawn a class terminal
     */
    ClassTerminal* SpawnClassTerminal(const XMFLOAT3& position,
     ...
     */
    PickupObject* SpawnPickup(InteractiveObjectType type, const XMFLOAT3& position,
                              float amount, ID3D11Device* device, ID3D11DeviceContext* context);

    /**
     * @brief Spawn a jump pad
     */
    JumpPadObject* SpawnJumpPad(const XMFLOAT3& position, float force,
                                ID3D11Device* device, ID3D11DeviceContext* context);

    /**
     * @brief Spawn a teleporter pair
     */
    void SpawnTeleporterPair(const XMFLOAT3& posA, const XMFLOAT3& posB,
                             ID3D11Device* device, ID3D11DeviceContext* context);

    /**
     * @brief Spawn an elevator
     */
    ElevatorObject* SpawnElevator(const XMFLOAT3& bottom, const XMFLOAT3& top,
                                  ID3D11Device* device, ID3D11DeviceContext* context);

    /**
     * @brief Spawn a destructible object
     */
    DestructibleObject* SpawnDestructible(const XMFLOAT3& position, float health,
                                          ID3D11Device* device, ID3D11DeviceContext* context);
