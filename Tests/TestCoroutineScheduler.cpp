...
public:
    ...
    bool IsReady(float dt) override { elapsed += dt; return elapsed >= target; }
...
public:
    ...
    void Update(float dt) {
        if (m_cancelled || IsFinished()) return;
        while (m_step < (int)m_steps.size()) {
            auto& s = m_steps[m_step];
            if (s.type == Step::Action) { if (s.action) s.action(); m_step++; continue; }
            if (s.yield && s.yield->IsReady(dt)) { m_step++; continue; }
            break;
        }
    }
...
public:
    ...
    void Stop(const std::string& name) {
        for (auto& c : m_cos) if (c && c->GetName() == name) c->Cancel();
    }
    void StopAll() {
        for (auto& c : m_cos) if (c) c->Cancel();
    }
    bool IsRunning(const std::string& name) const {
        for (auto& c : m_cos)
            if (c && c->GetName() == name && !c->IsFinished() && !c->IsCancelled()) return true;
        return false;
    }
    void Update(float dt) {
        for (auto& c : m_cos) if (c && !c->IsCancelled() && !c->IsFinished()) c->Update(dt);
        m_cos.erase(std::remove_if(m_cos.begin(), m_cos.end(),
            [](const std::unique_ptr<Coroutine>& c) { return !c || c->IsCancelled() || c->IsFinished(); }),
            m_cos.end());
    }
