...
struct Vec3 {
    ...
    bool Near(const Vec3& o, float tol = 0.5f) const {
        return DistanceTo(o) <= tol;
    }
};
...
public:
        ...
        const NavMeshBuildSettings& settings)
    {
        ...
        for (size_t i = 0; i < triCount; ++i) {
            ...
            // Centroid
            tri.centroid = {(v0.x + v1.x + v2.x) / 3.0f,
                            (v0.y + v1.y + v2.y) / 3.0f,
                            (v0.z + v1.z + v2.z) / 3.0f};
...
public:
    ...
    PathResult FindPath(const Vec3& start, const Vec3& end) const {
        ...
        float hStart = m_navMesh->triangles[startTri].centroid.DistanceTo(
        ...
        while (!openSet.empty()) {
            ...
            if (current == endTri) {
                ...
                for (size_t i = 1; i + 1 < triPath.size(); ++i) {
                    result.path.push_back({
                        m_navMesh->triangles[triPath[i]].centroid,
                        triPath[i]
                    });
                }
                result.path.push_back({end, endTri});
            ...
            for (uint32_t neighbor : m_navMesh->triangles[current].adjacency) {
                ...
                float tentG = gScore[current] +
                    m_navMesh->triangles[current].centroid.DistanceTo(
                        m_navMesh->triangles[neighbor].centroid);

                if (tentG < gScore[neighbor]) {
                    gScore[neighbor] = tentG;
                    cameFrom[neighbor] = current;
                    float h = m_navMesh->triangles[neighbor].centroid.DistanceTo(
                               m_navMesh->triangles[endTri].centroid);
