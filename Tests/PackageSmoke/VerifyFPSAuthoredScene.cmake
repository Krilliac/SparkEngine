...
# Exercise the public installed FPS runtime with an isolated edit to its
# packaged scene. An unchanged hard-coded camera/spawn must fail this probe.
...
if(NOT EXISTS "${SPARK_PACKAGE_BIN}/SparkEngine.exe" OR
   ...
   NOT EXISTS "${SPARK_PACKAGE_BIN}/Assets/Scenes/level1.scene")
    message(FATAL_ERROR "Installed FPS engine, module, or scene is missing")
endif()
...
if(NOT _lifecycle_ok)
    message(FATAL_ERROR "Installed FPS authored-scene run failed: ${_reason}; logs: ${_run_root}")
endif()

set(_audit "${_run_root}/exec_audit.log")
if(NOT EXISTS "${_audit}")
    message(FATAL_ERROR "Installed FPS did not execute game_status; logs: ${_run_root}")
endif()
file(READ "${_audit}" _status)
if(NOT _status MATCHES "Camera initialized from authored scene at \\(7[.]000000, 3[.]000000, -17[.]000000\\)")
    message(FATAL_ERROR "Installed FPS ignored edited [Camera] position in live startup; logs: ${_run_root}")
endif()
if(NOT _status MATCHES "Camera authored state: rotation \\(5[.]0, 30[.]0, 0[.]0\\) near/far \\(0[.]25, 250[.]0\\)")
    message(FATAL_ERROR "Installed FPS ignored edited [Camera] rotation/clipping in live startup; logs: ${_run_root}")
endif()
if(NOT _status MATCHES "Scene-authored respawn points: 4; preferred at \\(4[.]000000, 2[.]000000, -19[.]000000\\)")
    message(FATAL_ERROR "Installed FPS ignored edited [SpawnPoint] position in live startup; logs: ${_run_root}")
endif()
string(FIND "${_status}" "frame 4 " _post_frame_offset)
if(_post_frame_offset LESS 0)
    message(FATAL_ERROR "Installed FPS did not execute the post-update frame-4 diagnostics; logs: ${_run_root}")
endif()
...
if(NOT _post_frame_status MATCHES "Camera/Player XZ: \\(7[.]0, -17[.]0\\) / \\(7[.]0, -17[.]0\\)")
    message(FATAL_ERROR "Installed FPS did not retain authored camera/player location after a gameplay frame; logs: ${_run_root}")
endif()
if(NOT _post_frame_status MATCHES "Wave Spawn Points: 10; first \\(11[.]0, 1[.]5, 22[.]0\\)")
    message(FATAL_ERROR "Installed FPS did not configure authored wave spawns; logs: ${_run_root}")
endif()
if(NOT _status MATCHES "frame 5 [^\n]*[|] ok  [|] wave_start")
    message(FATAL_ERROR "Installed FPS did not start the survival match; logs: ${_run_root}")
endif()
string(FIND "${_status}" "frame 6 " _restart_frame_offset)
if(_restart_frame_offset LESS 0)
    message(FATAL_ERROR "Installed FPS did not execute post-restart game_status; logs: ${_run_root}")
endif()
...
if(NOT _restart_frame_status MATCHES "Camera/Player XZ: \\(4[.]0, -19[.]0\\) / \\(4[.]0, -19[.]0\\)")
    message(FATAL_ERROR "Installed FPS match restart did not use the edited North_Spawn; logs: ${_run_root}")
endif()
set(_image "${_run_root}/fps-authored.png")
if(NOT EXISTS "${_image}" OR IS_DIRECTORY "${_image}")
    message(FATAL_ERROR "Installed FPS did not capture an authored rendered frame; logs: ${_run_root}")
endif()
execute_process(
...
if(NOT _visual_result EQUAL 0)
    message(FATAL_ERROR "Installed FPS authored frame failed visual geometry check: ${_visual_stderr}; logs: ${_run_root}")
endif()
message(STATUS "Installed FPS honored edited scene camera and respawn data; logs: ${_run_root}")
