...
namespace
{
    ...
    float EaseEval(Ease e, float t)
    {
        ...
        case Ease::InBounce:
            return 1 - EaseEval(Ease::OutBounce, 1 - t);
        }
        return t;
    }
    ...
    class Tween
    {
      public:
        ...
        Tween& SetDelay(float d)
        {
            m_delay = d;
            m_delayRem = d;
            return *this;
        }
        Tween& SetLoops(int n, LoopType lt = LoopType::Restart)
        {
            m_loops = n;
            m_lt = lt;
            return *this;
        }
        Tween& OnComplete(std::function<void()> cb)
        {
            m_onDone = std::move(cb);
            return *this;
        }
        ...
        bool Update(float dt)
        {
            ...
            if (m_target)
                *m_target = m_start + (m_end - m_start) * eased;
            if (m_elapsed >= m_dur)
            {
                m_loopsDone++;
                ...
                {
                    if (m_target)
                        *m_target = m_reverse ? m_start : m_end;
                    m_finished = true;
                    if (m_onDone)
                        m_onDone();
                    return false;
