...
namespace
{
    ...
    class Frustum
    {
      public:
        ...
        {
            planes[0] = {m.m[0][3] + m.m[0][0], m.m[1][3] + m.m[1][0], m.m[2][3] + m.m[2][0],
                         m.m[3][3] + m.m[3][0]}; // Left
            planes[1] = {m.m[0][3] - m.m[0][0], m.m[1][3] - m.m[1][0], m.m[2][3] - m.m[2][0],
                         m.m[3][3] - m.m[3][0]}; // Right
            planes[2] = {m.m[0][3] + m.m[0][1], m.m[1][3] + m.m[1][1], m.m[2][3] + m.m[2][1],
                         m.m[3][3] + m.m[3][1]}; // Bottom
            planes[3] = {m.m[0][3] - m.m[0][1], m.m[1][3] - m.m[1][1], m.m[2][3] - m.m[2][1],
                         m.m[3][3] - m.m[3][1]};                      // Top
            planes[4] = {m.m[0][2], m.m[1][2], m.m[2][2], m.m[3][2]}; // Near
            planes[5] = {m.m[0][3] - m.m[0][2], m.m[1][3] - m.m[1][2], m.m[2][3] - m.m[2][2],
                         m.m[3][3] - m.m[3][2]}; // Far
            for (auto& p : planes)
                p.Normalize();
        }
        ...
        CullResult TestAABB(const AABB& box) const
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                Vec3 pv = {p.a >= 0 ? box.max.x : box.min.x, p.b >= 0 ? box.max.y : box.min.y,
                           p.c >= 0 ? box.max.z : box.min.z};
                Vec3 nv = {p.a >= 0 ? box.min.x : box.max.x, p.b >= 0 ? box.min.y : box.max.y,
