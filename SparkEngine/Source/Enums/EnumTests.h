 ...
 */
     ...
     */
        template <typename EnumType> static EnumTestResult TestEnumValidation()
        {
            ...
            try
            {
                ...
                for (auto value : allValues)
                {
                    if (!EnumUtils<EnumType>::IsValid(value))
                    {
                        ...
                        return EnumTestResult(std::string("EnumValidation<") + typeid(EnumType).name() + ">", false,
                                              "Valid enum value failed validation: " +
                ...
                auto end = std::chrono::high_resolution_clock::now();
                auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                return EnumTestResult(std::string("EnumValidation<") + typeid(EnumType).name() + ">", true, "",
                                      duration.count() / 1000.0);
            }
            catch (const std::exception& e)
            {
                ...
                auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                return EnumTestResult(std::string("EnumValidation<") + typeid(EnumType).name() + ">", false,
                                      std::string("Exception: ") + e.what(), duration.count() / 1000.0);
            }
        }
     ...
     */
        template <typename EnumType> static EnumTestResult TestEnumStringConversion()
        {
            auto start = std::chrono::high_resolution_clock::now();
            ...
            try
            {
                ...
                for (auto value : allValues)
                {
                    ...
                    if (str.empty() || str == "Unknown")
                    {
                        auto end = std::chrono::high_resolution_clock::now();
                        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                        return EnumTestResult(std::string("EnumStringConversion<") + typeid(EnumType).name() + ">",
                                              false,
                                              "ToString returned empty/unknown for valid enum value: " +
                                                  std::to_string(static_cast<int>(value)),
                                              duration.count() / 1000.0);
                    }
                    ...
                    if (converted != value)
                    {
                        ...
                        if (!EnumUtils<EnumType>::IsValid(converted))
                        {
                            ...
                            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                            return EnumTestResult(std::string("EnumStringConversion<") + typeid(EnumType).name() + ">",
                                                  false, "Round-trip conversion failed for: " + str,
                                                  duration.count() / 1000.0);
                        }
                    }
                }
                ...
                auto end = std::chrono::high_resolution_clock::now();
                auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                return EnumTestResult(std::string("EnumStringConversion<") + typeid(EnumType).name() + ">", true, "",
                                      duration.count() / 1000.0);
            }
            catch (const std::exception& e)
            {
                auto end = std::chrono::high_resolution_clock::now();
                auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                return EnumTestResult(std::string("EnumStringConversion<") + typeid(EnumType).name() + ">", false,
                                      std::string("Exception: ") + e.what(), duration.count() / 1000.0);
            }
        }
     ...
     */
        template <typename EnumType> static EnumTestResult TestEnumIteration()
        {
            auto start = std::chrono::high_resolution_clock::now();
            ...
            try
            {
                ...
                if (iteratedValues.size() != allValues.size())
                {
                    auto end = std::chrono::high_resolution_clock::now();
                    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                    return EnumTestResult(std::string("EnumIteration<") + typeid(EnumType).name() + ">", false,
                ...
                for (auto expected : allValues)
                {
                    ...
                    if (!found)
                    {
                        ...
                        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                        return EnumTestResult(std::string("EnumIteration<") + typeid(EnumType).name() + ">", false,
                                              "Missing enum value in iteration: " +
                ...
                auto end = std::chrono::high_resolution_clock::now();
                auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                return EnumTestResult(std::string("EnumIteration<") + typeid(EnumType).name() + ">", true, "",
                                      duration.count() / 1000.0);
            }
            catch (const std::exception& e)
            {
     ...
     */
        template <typename EnumType> static EnumTestResult TestEnumCount()
        {
            auto start = std::chrono::high_resolution_clock::now();
            ...
            try
            {
                ...
                if (utilsCount != allValues.size())
                {
                    auto end = std::chrono::high_resolution_clock::now();
                    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                    return EnumTestResult(std::string("EnumCount<") + typeid(EnumType).name() + ">", false,
                ...
                auto end = std::chrono::high_resolution_clock::now();
                auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
                return EnumTestResult(std::string("EnumCount<") + typeid(EnumType).name() + ">", true, "",
                                      duration.count() / 1000.0);
            }
            catch (const std::exception& e)
            {
     ...
     */
        template <typename EnumType> static EnumTestResult TestEnumBoundaryConditions()
        {
     ...
     */
        template <typename EnumType> static EnumTestResult TestEnumCaseInsensitivity()
        {
     ...
     */
        static EnumTestResult TestEnumFlags() { return EnumTestResult("EnumFlags", true); }

        /**
     * @brief Test enum validator functionality
     */
        template <typename EnumType> static EnumTestResult TestEnumValidator()
        {
     ...
     */
        template <typename EnumType> static EnumTestResult TestEnumPerformance()
        {
            auto start = std::chrono::high_resolution_clock::now();
            ...
            return EnumTestResult(std::string("EnumPerformance<") + typeid(EnumType).name() + ">", true,
