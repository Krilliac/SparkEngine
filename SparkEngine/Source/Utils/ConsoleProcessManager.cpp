    ...
    m_commandRegistry->RegisterCommand("help",
        [this](const std::vector<std::string>& args) -> std::string {
            ...
            for (const auto& cmd : commands) {
                ss << "  " << cmd.name;
                if (!cmd.description.empty()) ss << " - " << cmd.description;
                ss << "\n";
                if (!cmd.usage.empty()) ss << "    Usage: " << cmd.usage << "\n";
            }
            return ss.str();
        },
...
bool ConsoleProcessManager::Initialize(const std::wstring& consolePath) {
    ...
    for (const auto& path : searchPaths) {
        ...
        if (std::filesystem::path(path).is_absolute()) {
            ...
            if (!std::filesystem::exists(fullPath)) fullPath = path;
        }
...
void ConsoleProcessManager::Shutdown() {
    ...
    if (m_stdInWrite) { CloseHandle(m_stdInWrite); m_stdInWrite = NULL; }
    if (m_stdOutRead) { CloseHandle(m_stdOutRead); m_stdOutRead = NULL; }
    if (m_stdInRead) { CloseHandle(m_stdInRead); m_stdInRead = NULL; }
    if (m_stdOutWrite) { CloseHandle(m_stdOutWrite); m_stdOutWrite = NULL; }
    ...
    if (m_threadHandle) { CloseHandle(m_threadHandle); m_threadHandle = NULL; }
    m_initialized = false;
}
...
bool ConsoleProcessManager::LaunchConsoleProcess(const std::wstring& path) {
    ...
    if (!CreatePipe(&hChildStdInRead, &hChildStdInWrite, &sa, 0)) return false;
    ...
    BOOL success = CreateProcessW(NULL, const_cast<LPWSTR>(commandLine.c_str()),
...
bool ConsoleProcessManager::WriteToConsole(const std::wstring& message) {
    ...
    BOOL result = WriteFile(m_stdInWrite, utf8Message.c_str(),
        static_cast<DWORD>(utf8Message.length()), &bytesWritten, NULL);
    if (result) FlushFileBuffers(m_stdInWrite);
    return result != FALSE;
}

void ConsoleProcessManager::ConsoleThreadMain() {
    while (!m_shouldStopThread && m_consoleRunning) {
        if (ReadFromConsole()) continue;
...
bool ConsoleProcessManager::Initialize(const std::wstring& consolePath) {
    ...
    if (consoleBaseName.empty()) consoleBaseName = "SparkConsole";
...
void ConsoleProcessManager::Shutdown() {
    ...
    if (m_pipeToChild[1] >= 0) { close(m_pipeToChild[1]); m_pipeToChild[1] = -1; }
    if (m_pipeFromChild[0] >= 0) { close(m_pipeFromChild[0]); m_pipeFromChild[0] = -1; }
...
bool ConsoleProcessManager::LaunchConsoleProcess(const std::wstring& path) {
    ...
    if (pipe(m_pipeToChild) == -1) return false;
    if (pipe(m_pipeFromChild) == -1) {
        close(m_pipeToChild[0]); close(m_pipeToChild[1]);
        return false;
    }
    ...
    if (m_childPid == -1) {
        close(m_pipeToChild[0]); close(m_pipeToChild[1]);
        close(m_pipeFromChild[0]); close(m_pipeFromChild[1]);
        return false;
    }
...
bool ConsoleProcessManager::ReadFromConsole() {
    if (m_pipeFromChild[0] < 0) return false;
...
bool ConsoleProcessManager::WriteToConsole(const std::wstring& message) {
    if (m_pipeToChild[1] < 0) return false;
...
bool ConsoleProcessManager::Initialize(const std::wstring&) { m_initialized = true; return true; }
void ConsoleProcessManager::Shutdown() { m_initialized = false; }
void ConsoleProcessManager::Log(const std::wstring& msg, const std::wstring& type) {
    std::cerr << "[" << WStrToStr(type) << "] " << WStrToStr(msg) << "\n";
}
void ConsoleProcessManager::LogCrash(const std::string& info) { std::cerr << "[CRASH] " << info << "\n"; }
bool ConsoleProcessManager::LaunchConsoleProcess(const std::wstring&) { return false; }
bool ConsoleProcessManager::ReadFromConsole() { return false; }
bool ConsoleProcessManager::WriteToConsole(const std::wstring&) { return false; }
void ConsoleProcessManager::ConsoleThreadMain() {}
#endif
...
std::string CommandRegistry::ExecuteCommand(const std::string& commandLine) {
    ...
    if (args.empty()) return "Empty command";
    std::string commandName = args[0];
    args.erase(args.begin());
    auto it = m_commands.find(commandName);
    if (it == m_commands.end()) return "Unknown command: " + commandName;
    try {
...
std::vector<CommandRegistry::CommandInfo> CommandRegistry::GetAllCommands() const {
    ...
    for (const auto& pair : m_commands) result.push_back(pair.second);
    return result;
}
