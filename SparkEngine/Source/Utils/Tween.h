...
inline float EaseEvaluate(Ease ease, float t) {
    ...
    case Ease::OutQuart:   { float u = t - 1.0f; return 1.0f - u * u * u * u; }
    case Ease::InOutQuart: return t < 0.5f ? 8.0f * t * t * t * t : 1.0f - 8.0f * (t - 1.0f) * (t - 1.0f) * (t - 1.0f) * (t - 1.0f);

    // Quint
    case Ease::InQuint:    return t * t * t * t * t;
    case Ease::OutQuint:   { float u = t - 1.0f; return 1.0f + u * u * u * u * u; }
    case Ease::InOutQuint: return t < 0.5f ? 16.0f * t * t * t * t * t : 1.0f + 16.0f * (t - 1.0f) * (t - 1.0f) * (t - 1.0f) * (t - 1.0f) * (t - 1.0f);

    // Sine
    case Ease::InSine:     return 1.0f - std::cos(t * PI * 0.5f);
    case Ease::OutSine:    return std::sin(t * PI * 0.5f);
    case Ease::InOutSine:  return 0.5f * (1.0f - std::cos(PI * t));
    ...
    // Back (overshoot)
    case Ease::InBack:     { constexpr float s = 1.70158f; return t * t * ((s + 1.0f) * t - s); }
    case Ease::OutBack:    { constexpr float s = 1.70158f; float u = t - 1.0f; return u * u * ((s + 1.0f) * u + s) + 1.0f; }
    case Ease::InOutBack: {
    ...
    case Ease::InBounce:   return 1.0f - EaseEvaluate(Ease::OutBounce, 1.0f - t);
    case Ease::InOutBounce:
        return t < 0.5f
            ? 0.5f * EaseEvaluate(Ease::InBounce, t * 2.0f)
...
public:
    Tween(float* target, float endValue, float duration, Ease ease = Ease::Linear)
        : m_target(target)
        , m_startValue(target ? *target : 0.0f)
        , m_endValue(endValue)
        , m_duration(duration)
        , m_ease(ease)
    {}
    ...
    Tween& OnUpdate(std::function<void(float)> callback) { m_onUpdate = std::move(callback); return *this; }

    /** @brief Callback invoked when the tween completes (all loops). */
    Tween& OnComplete(std::function<void()> callback) { m_onComplete = std::move(callback); return *this; }
    ...
    bool Update(float deltaTime) {
        ...
        if (m_delayRemaining > 0.0f) {
            ...
            if (m_delayRemaining > 0.0f) return true;
        ...
        if (m_onUpdate) m_onUpdate(value);
        ...
        if (m_elapsed >= m_duration) {
            ...
            if (m_loopCount == -1 || m_completedLoops < m_loopCount) {
                ...
                if (m_loopType == LoopType::PingPong || m_loopType == LoopType::Yoyo) {
                    ...
                    if (m_target) *m_target = m_startValue;
                }
            } else {
                // Finished
                if (m_target) *m_target = m_pingPongReverse ? m_startValue : m_endValue;
                m_finished = true;
    ...
    void Update(float deltaTime) {
        m_tweens.erase(
            std::remove_if(m_tweens.begin(), m_tweens.end(),
                [deltaTime](std::unique_ptr<Tween>& tween) {
