...
namespace Spark
{
    ...
    class UUID
    {
      public:
        ...
        {
            char buf[17];
            std::snprintf(buf, sizeof(buf), "%016llx", static_cast<unsigned long long>(m_value));
...
namespace std
{
    template <> struct hash<Spark::UUID>
    {
        size_t operator()(const Spark::UUID& uuid) const noexcept { return hash<uint64_t>()(uuid.GetValue()); }
    };
} // namespace std
