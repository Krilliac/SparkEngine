...
namespace Spark
{
                ...
                if (file.is_open())
                {
                    ...
                    while (std::getline(file, line))
                    {
                        ...
                        if (pos != std::string::npos)
                        {
                            ...
                            {
                                if (key == "Gravity")
                                    g_gameState.gravity = std::stof(value);
                                else if (key == "PlayerSpeed")
                                    g_gameState.playerSpeed = std::stof(value);
                                else if (key == "JumpHeight")
                                    g_gameState.jumpHeight = std::stof(value);
                                else if (key == "Friction")
                                    g_gameState.friction = std::stof(value);
                            }
                            else if (section == "Camera")
                            {
                                if (key == "FOV")
                                    g_gameState.fov = std::stof(value);
                                else if (key == "MouseSensitivity")
                                    g_gameState.mouseSensitivity = std::stof(value);
                                else if (key == "InvertY")
                                    g_gameState.invertY = (value == "true");
                            }
                            else if (section == "System")
                            {
                                if (key == "TimeScale")
                                    g_gameState.timeScale = std::stof(value);
                                else if (key == "VSync")
                                    g_gameState.vsync = (value == "true");
                                else if (key == "Wireframe")
                                    g_gameState.wireframe = (value == "true");
                                else if (key == "ShowFPS")
                                    g_gameState.showFPS = (value == "true");
                                else if (key == "Noclip")
                                    g_gameState.noclip = (value == "true");
                                else if (key == "GodMode")
                                    g_gameState.godMode = (value == "true");
                                else if (key == "InfiniteAmmo")
                                    g_gameState.infiniteAmmo = (value == "true");
                            }
                        }
                    }
                    file.close();
    ...
    {
        m_watchEntries.erase(std::remove_if(m_watchEntries.begin(), m_watchEntries.end(),
    ...
    {
        if (!m_watchActive || m_watchEntries.empty())
            return;
    ...
    void SimpleConsole::RegisterDefaultCommands()
    {
        ...
        RegisterCommand(
            ...
            {
                if (args.empty())
                    return "Usage: exec <command1; command2; ...>";
            ...
            [this](const std::vector<std::string>& args) -> std::string
            {
                ...
                try
                {
                    ...
                    if (count < 1 || count > 100)
                        return "Repeat count must be between 1 and 100";
                    std::string cmd;
                    for (size_t i = 1; i < args.size(); ++i)
                    {
                        if (i > 1)
    ...
    void SimpleConsole::RegisterPerformanceCommands()
    {
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                ss << "==========================================\n";
                ss << "Render Time:      " << std::fixed << std::setprecision(2) << g_perfCounters.renderTime
                   << " ms\n";
                ss << "Update Time:      " << std::fixed << std::setprecision(2) << g_perfCounters.updateTime
                   << " ms\n";
    ...
    void SimpleConsole::RegisterPlayerCommands()
    {
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                if (g_game && g_game->GetPlayer())
                {
                    ...
                    ss << "Health:           " << std::fixed << std::setprecision(1) << state.health << "/"
                       << state.maxHealth << " (" << std::setprecision(0) << (state.health / state.maxHealth * 100)
                       << "%)\n";
                    ss << "Armor:            " << std::fixed << std::setprecision(1) << state.armor << "/"
                       << state.maxArmor << "\n";
                    ss << "Stamina:          " << std::fixed << std::setprecision(1) << state.stamina << "/"
                       << state.maxStamina << "\n";
                    ss << "Position:         (" << std::setprecision(2) << state.position.x << ", " << state.position.y
                       << ", " << state.position.z << ")\n";
                    ss << "Velocity:         (" << std::setprecision(2) << state.velocity.x << ", " << state.velocity.y
                       << ", " << state.velocity.z << ")\n";
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                return "Player healed by " + std::to_string(actualHealed) +
                       " HP. Current health: " + std::to_string(newState.health) + "/" +
                       std::to_string(newState.maxHealth) + " (live game integration)";
            },
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                std::string result = "Player took " + std::to_string(actualDamage) +
                                     " damage. Current health: " + std::to_string(newState.health) + "/" +
                                     std::to_string(newState.maxHealth) + " (live game integration)";
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                {
                    auto state = g_game->GetPlayer()->Console_GetState();
                    return "Current player speed: " + std::to_string(state.speed) +
                           " units/sec\n"
    ...
    void SimpleConsole::RegisterPhysicsCommands()
    {
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                if (args.empty())
                {
                    return "Current gravity: " + std::to_string(g_gameState.gravity) +
                           " units/sec�\n"
                ...
                try
                {
                    ...
                    if (g_game)
                    {
                        g_game->ApplyPhysicsSettings(newGravity, g_gameState.playerSpeed, g_gameState.jumpHeight,
                                                     g_gameState.friction);
                    }
                    ...
                    return "Gravity set to " + std::to_string(g_gameState.gravity) +
                           " units/sec� (applied to game systems)";
                }
                catch (...)
                {
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                if (args.empty())
                {
                    return "Current player speed: " + std::to_string(g_gameState.playerSpeed) +
                           " units/sec\n"
                ...
                try
                {
                    ...
                    if (g_game)
                    {
                        g_game->ApplyPhysicsSettings(g_gameState.gravity, newSpeed, g_gameState.jumpHeight,
                                                     g_gameState.friction);
                    }
                    ...
                    return "Player speed set to " + std::to_string(g_gameState.playerSpeed) +
                           " units/sec (applied to game systems)";
                }
                catch (...)
                {
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                if (args.empty())
                {
                    return "Current jump height: " + std::to_string(g_gameState.jumpHeight) +
                           " units\n"
                ...
                try
                {
                    ...
                    if (g_game)
                    {
                        g_game->ApplyPhysicsSettings(g_gameState.gravity, g_gameState.playerSpeed, newJump,
                                                     g_gameState.friction);
                    }
                    ...
                    return "Jump height set to " + std::to_string(g_gameState.jumpHeight) +
                           " units (applied to game systems)";
                }
                catch (...)
                {
                ...
                try
                {
                    ...
                    if (g_game)
                    {
                        g_game->ApplyPhysicsSettings(g_gameState.gravity, g_gameState.playerSpeed,
                                                     g_gameState.jumpHeight, newFriction);
                    }
                    ...
                    return "Friction coefficient set to " + std::to_string(g_gameState.friction) +
                           " (applied to game systems)";
                }
                catch (...)
                {
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                if (g_game)
                {
                    g_game->ApplyPhysicsSettings(g_gameState.gravity, g_gameState.playerSpeed, g_gameState.jumpHeight,
                                                 g_gameState.friction);
                }
                ...
                return "Physics parameters reset to defaults and applied to game systems:\n"
                       "  Gravity: " +
                       std::to_string(g_gameState.gravity) +
                       " units/sec�\n"
                       "  Speed: " +
                       std::to_string(g_gameState.playerSpeed) +
                       " units/sec\n"
                       "  Jump Height: " +
                       std::to_string(g_gameState.jumpHeight) +
                       " units\n"
                       "  Friction: " +
                       std::to_string(g_gameState.friction);
            },
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                if (args.empty())
                {
                    if (g_game)
                    {
                        return "Current time scale: " + std::to_string(g_game->GetTimeScale()) +
                               "x\n"
                ...
                try
                {
                    ...
                    {
                        g_gameState.timeScale = newScale;
                        return "Time scale set to " + std::to_string(newScale) +
                               "x (console simulation - no game system)";
                    }
                }
                catch (...)
                {
    ...
    void SimpleConsole::RegisterSceneCommands()
    {
                    ...
                    if (type == "cube" || type == "sphere" || type == "wall" || type == "plane")
                    {
                        ...
                        if (g_game)
                        {
                            ...
                            if (success)
                            {
                                ...
                                std::stringstream ss;
                                ss << "Spawned " << type << " at (" << x << ", " << y << ", " << z
                                   << ") via game integration";
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                if (args[0] == "all")
                {
                    ...
                    if (g_game)
                    {
                        ...
                        return "Cleared " + std::to_string(clearedObjects - 1) +
                               " objects from scene via game integration (player preserved)";
                    }
                    else
                    {
                        // Fallback to console simulation
                        ...
                        return "Cleared " + std::to_string(clearedObjects - 1) +
                               " objects from scene (console simulation)";
                    }
                }
                    ...
                    if (g_game)
                    {
                        ...
                        if (success)
                        {
                            ...
                            if (g_gameState.activeObjects > 1)
                            {
                                g_gameState.activeObjects--;
                                g_gameState.visibleObjects =
                                    std::min(g_gameState.visibleObjects, g_gameState.activeObjects);
                            }

                            return "Object " + args[0] +
                                   " deleted from scene via game integration. Remaining objects: " +
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                ss << "Player Position:  (" << std::fixed << std::setprecision(2) << g_gameState.playerPosition.x
                   << ", " << g_gameState.playerPosition.y << ", " << g_gameState.playerPosition.z << ")\n";
                ...
                ss << "  Draw Calls:     " << g_perfCounters.drawCalls << "\n";
                ss << "  Triangles:      " << g_perfCounters.triangles << "\n";
                ss << "  Memory Usage:   " << (g_perfCounters.textureMemory + g_perfCounters.bufferMemory) / 1024 / 1024
                   << " MB";
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                if (g_game && g_game->GetCamera())
                {
                    ...
                    ss << "==========================================\n";
                    ss << "Position:         (" << std::fixed << std::setprecision(2) << state.position.x << ", "
                       << state.position.y << ", " << state.position.z << ")\n";
                    ss << "Rotation:         (" << std::setprecision(1) << state.rotation.x << "�, " << state.rotation.y
                       << "�, " << state.rotation.z << "�)\n";
                    ss << "Forward:          (" << std::setprecision(3) << state.forward.x << ", " << state.forward.y
                       << ", " << state.forward.z << ")\n";
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                if (args.empty())
                {
                    ...
                    return "Current FOV: " + std::to_string(state.defaultFov) +
                           " degrees\n"
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                if (args.empty())
                {
                    auto state = g_game->GetCamera()->Console_GetState();
                    std::stringstream ss;
                    ss << "Current camera position: (" << std::fixed << std::setprecision(2) << state.position.x << ", "
                       << state.position.y << ", " << state.position.z << ")\n";
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                if (args.empty())
                {
                    auto state = g_game->GetCamera()->Console_GetState();
                    std::stringstream ss;
                    ss << "Current camera rotation: (" << std::fixed << std::setprecision(1) << state.rotation.x
                ...
                try
                {
                    ...
                    std::stringstream ss;
                    ss << "Camera rotation set to (" << pitch << "�, " << yaw << "�, " << roll
                       << "�) via live game integration";
                    return ss.str();
                }
                catch (...)
    ...
    void SimpleConsole::RegisterSystemCommands()
    {
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                return "Debug setting " + setting + " " + (enable ? "enabled" : "disabled") +
                       " (applied to game systems)";
            },
    ...
    void SimpleConsole::RegisterGraphicsCommands()
    {
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                return "Graphics debug mode " + std::string(enable ? "enabled" : "disabled") +
                       " via live graphics integration";
            },
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                if (success)
                {
                    return "Screenshot captured successfully" +
                           (filename.empty() ? " (auto-named)" : " as " + filename);
                }
                else
                {
        ...
        RegisterCommand(
            ...
            [this](const std::vector<std::string>& args) -> std::string
            {
                if (args.empty())
                {
                    ...
                    for (const auto& entry : m_watchEntries)
                    {
                        ss << "  " << std::left << std::setw(16) << entry.name << " = " << entry.lastValue
                           << (entry.active ? "" : " (paused)") << "\n";
                    }
                    return ss.str();
                }
                ...
                else if (varName == "position")
                {
                    AddWatch("position",
                             ...
                             {
                                 std::stringstream ss;
                                 ss << std::fixed << std::setprecision(1) << g_gameState.playerPosition.x << ","
                                    << g_gameState.playerPosition.y << "," << g_gameState.playerPosition.z;
                                 return ss.str();
                             });
                }
                ...
                else
                {
                    return "Unknown watch variable: " + varName +
                           "\n"
                           "Available: fps, frametime, position, health, objects, memory, drawcalls, triangles, "
                           "timescale";
                }
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                else if (setting == "vsync")
                {
                    ...
                    if (GetGfx())
                        GetGfx()->Console_SetVSync(g_gameState.vsync);
    ...
    void SimpleConsole::RegisterFileCommands()
    {
        ...
        RegisterCommand(
            ...
            {
                if (args.empty())
                    return "Usage: file_read <path> [max_lines]";
                ...
                try
                {
                    ...
                    if (!file.is_open())
                        return "Cannot open file: " + args[0];
        ...
        RegisterCommand(
            "file_cwd", [](const std::vector<std::string>& args) -> std::string
        ...
        RegisterCommand(
            ...
            {
                if (args.empty())
                    return "Usage: file_find <pattern> [directory]";
    ...
    void SimpleConsole::RegisterRenderingCommands()
    {
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                if (args.empty())
                {
                    return "Usage: render_mode <solid|wireframe|points>\n"
                           "Current: " +
                           std::string(g_gameState.wireframe ? "wireframe" : "solid");
                }
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                if (args.size() < 2)
                    return "Usage: render_resolution <width> <height>";
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                if (args.empty())
                {
                    return "Current FOV: " + std::to_string(g_gameState.fov) +
                           " degrees\n"
                ...
                try
                {
                    ...
                    if (fov < 30.0f || fov > 170.0f)
                        return "FOV must be between 30 and 170";
                    g_gameState.fov = fov;
                    if (g_game && g_game->GetCamera())
                    {
    ...
    void SimpleConsole::RegisterProfilingCommands()
    {
        RegisterCommand(
            ...
            [this](const std::vector<std::string>& args) -> std::string
            {
                ...
                ss << "  Triangles:     " << g_perfCounters.triangles << "\n";
                ss << "  Vertices:      " << g_perfCounters.vertices << "\n";
                ss << "  Objects:       " << g_gameState.activeObjects << " active, " << g_gameState.visibleObjects
                   << " visible\n\n";
            ...
            [this](const std::vector<std::string>& args) -> std::string
            {
                ...
                try
                {
                    ...
                    file << std::put_time(std::localtime(&time_t), "%Y-%m-%d %H:%M:%S") << ","
                         << g_perfCounters.currentFPS << "," << g_perfCounters.averageFPS << ","
                         << g_perfCounters.frameTime << "," << g_perfCounters.renderTime << ","
                         << g_perfCounters.updateTime << "," << g_perfCounters.drawCalls << ","
                         << g_perfCounters.triangles << "," << g_perfCounters.vertices << ","
                         << g_gameState.activeObjects << "," << (pmc.WorkingSetSize / 1024 / 1024) << "\n";
    ...
    void SimpleConsole::RegisterGameCommands()
    {
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                try
                {
                    ...
                    if (!file.is_open())
                        return "Cannot create save file: " + filename;
        ...
        RegisterCommand(
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                std::string filename = args.empty() ? "spark_save.dat" : args[0];
                if (!std::filesystem::exists(filename))
                    return "Save file not found: " + filename;

                try
                {
                    std::ifstream file(filename, std::ios::binary);
                    if (!file.is_open())
                        return "Cannot open save file: " + filename;
            ...
            [](const std::vector<std::string>& args) -> std::string
            {
                ...
                try
                {
                    ...
                    if (scale < 0.01f || scale > 10.0f)
                        return "Time scale must be between 0.01 and 10.0";
    ...
    void SimpleConsole::RegisterCrashCommands()
    {
        RegisterCommand(
            ...
            [this](const std::vector<std::string>& args) -> std::string
            {
                ...
                {
                    if (args.size() < 2)
                        return "Usage: crash_config assert <on|off>";
                    std::string value = args[1];
                    std::transform(value.begin(), value.end(), value.begin(), ::tolower);
                    bool enable = (value == "on" || value == "true" || value == "1");
                    SetAssertCrashBehavior(enable);
                    return "Assert crash behavior: " +
                           std::string(enable ? "ENABLED (asserts will crash)" : "DISABLED (asserts log only)");
                }
        ...
        RegisterCommand(
            ...
            [this](const std::vector<std::string>& args) -> std::string
            {
                ...
                HANDLE hFile =
                    CreateFileA(filename.c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        ...
        RegisterCommand(
            ...
            [this](const std::vector<std::string>& args) -> std::string
            {
                ...
                try
                {
                    std::ofstream file(filename);
                    if (!file.is_open())
                        return "Cannot create report file: " + filename;
    ...
    void SimpleConsole::RegisterHealthCommands()
    {
        RegisterCommand(
            ...
            [this](const std::vector<std::string>& args) -> std::string
            {
                ...
                {
                    ss << "  Frame Time:      " << std::setprecision(2) << g_perfCounters.frameTime
                       << " ms  WARNING (>33ms)\n";
                    warnings++;
                }
                else
        ...
        RegisterCommand(
            ...
            [this](const std::vector<std::string>& args) -> std::string
            {
                ...
                if (GetProcessMemoryInfo(GetCurrentProcess(), reinterpret_cast<PROCESS_MEMORY_COUNTERS*>(&pmcEx),
                                         sizeof(pmcEx)))
                {
        ...
        RegisterCommand(
            ...
            [this](const std::vector<std::string>& args) -> std::string
            {
                ...
                ss << "==========================================\n";
                ss << "  FPS Range:      " << std::fixed << std::setprecision(1) << minFPS << " - " << maxFPS << "\n";
                ss << "  Average FPS:    " << avgFPS << "\n";
                ss << "  Frame Time:     " << std::setprecision(2) << minFrame << " - " << maxFrame << " ms\n";
                ss << "  Avg Frame Time: " << avgFrame << " ms\n";
                ss << "  Stability:      " << std::setprecision(1) << stability << "%\n\n";

                // Performance classification
                if (avgFPS >= 60)
                    ss << "  Rating: EXCELLENT (60+ FPS)\n";
                else if (avgFPS >= 30)
                    ss << "  Rating: GOOD (30-60 FPS)\n";
                else if (avgFPS >= 15)
                    ss << "  Rating: POOR (15-30 FPS)\n";
                else
                    ss << "  Rating: CRITICAL (<15 FPS)\n";
        ...
        RegisterCommand(
            ...
            [this](const std::vector<std::string>& args) -> std::string
            {
                ...
                if (metrics.gpuUsage > 95.0f)
                    ss << "  CRITICAL\n";
                else if (metrics.gpuUsage > 80.0f)
                    ss << "  WARNING\n";
                else
                    ss << "  OK\n";
...
namespace Spark
{
    ...
    void SimpleConsole::Log(const std::string& message, const std::string& type)
    {
        ...
        if (!m_logFilter.empty() && type != m_logFilter)
            return;
        if (!m_logSearchTerm.empty() && message.find(m_logSearchTerm) == std::string::npos)
            return;
    ...
    void SimpleConsole::LogInfo(const std::string& msg)
    {
        Log(msg, "INFO");
    }
    void SimpleConsole::LogWarning(const std::string& msg)
    {
        Log(msg, "WARNING");
    }
    void SimpleConsole::LogError(const std::string& msg)
    {
        Log(msg, "ERROR");
    }
    void SimpleConsole::LogSuccess(const std::string& msg)
    {
        Log(msg, "SUCCESS");
    }
    void SimpleConsole::LogCritical(const std::string& msg)
    {
        Log(msg, "CRITICAL");
    }
    void SimpleConsole::LogTrace(const std::string& msg)
    {
        Log(msg, "TRACE");
    }
    void SimpleConsole::LogDebug(const std::string& msg)
    {
        Log(msg, "DEBUG");
    }
