...
public:
    ...
    size_t PrintLeakReport() const
    {
        ...
        for (const auto& leak : leaks)
        {
            fprintf(stderr, "  [#%llu] %p: %zu bytes [%s]\n"
                            "         at %s\n",
                    static_cast<unsigned long long>(leak.allocationIndex),
                    leak.address,
                    leak.size,
                    leak.category.c_str(),
                    leak.location.c_str());
    ...
    std::vector<MemoryCategoryStats> GetCategoryStats() const
    {
        ...
        std::sort(stats.begin(), stats.end(),
                  [](const MemoryCategoryStats& a, const MemoryCategoryStats& b) {
...
#define SPARK_TRACK_FREE(ptr) \
    Spark::MemoryDebugger::GetInstance().RecordFree(ptr)

/** @brief Print the leak report */
#define SPARK_PRINT_LEAK_REPORT() \
    Spark::MemoryDebugger::GetInstance().PrintLeakReport()

/** @brief Print memory usage summary */
#define SPARK_PRINT_MEMORY_SUMMARY() \
    Spark::MemoryDebugger::GetInstance().PrintSummary()
