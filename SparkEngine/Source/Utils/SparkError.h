...
namespace SparkError
{
    ...
    inline void LogMessage(Severity severity, const char* category, const char* file, int line, const char* func,
                           const char* fmt, ...)
    {
        ...
        char fullMsg[2048];
        snprintf(fullMsg, sizeof(fullMsg), "[%s][%-8s] %s  (%s:%d in %s)\n", SeverityToString(severity),
                 category ? category : "Engine", userMsg, shortFile ? shortFile : "?", line, func ? func : "?");
    ...
    inline bool CheckFailed(const char* expr, const char* file, int line, const char* func, const char* msg = nullptr)
    {
        if (msg && msg[0])
        {
            LogMessage(Severity::Error, "Check", file, line, func, "CHECK FAILED: %s -- %s", expr, msg);
        }
        else
        {
    ...
    inline bool BoundsCheckFailed(const char* indexExpr, long long index, long long size, const char* file, int line,
                                  const char* func)
    {
    ...
    inline bool HResultFailed(const char* expr, long hr, const char* file, int line, const char* func,
                              const char* msg = nullptr)
    {
#ifdef _WIN32
        // Attempt to get the system error message for this HRESULT
        char sysMsg[256] = {};
        FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, static_cast<DWORD>(hr),
        ...
        if (msg && msg[0])
        {
            LogMessage(Severity::Error, "HRESULT", file, line, func, "HR FAILED: %s returned 0x%08lX (%s) -- %s", expr,
                       hr, sysMsg, msg);
        }
        else
        {
    ...
    {
      public:
        ScopedContext(const char* context, const char* file, int line) : m_context(context), m_file(file), m_line(line)
        {
            LogMessage(Severity::Trace, "Context", file, line, nullptr, ">> ENTER: %s", context);
        }
        ~ScopedContext() { LogMessage(Severity::Trace, "Context", m_file, m_line, nullptr, "<< EXIT:  %s", m_context); }
...
// ---------------------------------------------------------------------------
#define SPARK_CHECK(expr) ((expr) ? true : SparkError::CheckFailed(#expr, __FILE__, __LINE__, __FUNCTION__))
...
#define SPARK_BOUNDS_CHECK(index, size)                                                                                \
    (((long long)(index) >= 0 && (long long)(index) < (long long)(size))                                               \
         ? true                                                                                                        \
         : SparkError::BoundsCheckFailed(#index, (long long)(index), (long long)(size), __FILE__, __LINE__,            \
                                         __FUNCTION__))
...
#define SPARK_BOUNDS_CHECK_MSG(index, size, msg)                                                                       \
    do                                                                                                                 \
    {                                                                                                                  \
        if (!SPARK_BOUNDS_CHECK(index, size))                                                                          \
        {                                                                                                              \
...
// ---------------------------------------------------------------------------
#define SPARK_CHECK_NOT_NULL(ptr) SPARK_CHECK_MSG((ptr) != nullptr, "Pointer '" #ptr "' must not be null")
...
// ---------------------------------------------------------------------------
#define SPARK_SCOPED_CONTEXT(name) SparkError::ScopedContext _sparkCtx_##__LINE__(name, __FILE__, __LINE__)
