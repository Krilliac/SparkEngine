...
void NetBuffer::WriteUint8(uint8_t val) { m_data.push_back(val); }
...
void NetBuffer::WriteString(const std::string& val) {
    ...
    for (char c : val) m_data.push_back(static_cast<uint8_t>(c));
}

void NetBuffer::WriteVector3(const XMFLOAT3& val) {
    WriteFloat(val.x); WriteFloat(val.y); WriteFloat(val.z);
}
...
void NetworkManager::MarkPropertyDirty(uint32_t networkID, const std::string& propertyName) {
    ...
    if (it == m_replicatedEntities.end()) return;
    ...
    for (auto& [netID, entity] : m_replicatedEntities) {
        ...
        if (!hasDirty) {
            for (const auto& prop : entity.properties) {
                if (prop.dirty) { hasDirty = true; break; }
            }
        }
...
std::string NetworkManager::Console_GetStatus() const {
    ...
    ss << "Role: ";
    switch (m_role) {
        case NetworkRole::None: ss << "None"; break;
        case NetworkRole::Server: ss << "Server"; break;
        case NetworkRole::Client: ss << "Client"; break;
    }
    ss << "\nState: ";
    switch (m_connectionState) {
        case ConnectionState::Disconnected: ss << "Disconnected"; break;
        case ConnectionState::Connecting: ss << "Connecting"; break;
        case ConnectionState::Connected: ss << "Connected"; break;
        case ConnectionState::Disconnecting: ss << "Disconnecting"; break;
    }
...
std::string NetworkManager::Console_GetStats() const {
    ...
    ss << "Packets: Sent " << m_stats.packetsSent << ", Received " << m_stats.packetsReceived
       << ", Dropped " << m_stats.packetsDropped << "\n";
    return ss.str();
}
