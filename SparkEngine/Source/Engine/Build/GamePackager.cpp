...
LegacyPackageResult GamePackager::PackageLegacy(const LegacyPackageConfig& config)
{
    ...
    const auto publish = [this](const LegacyPackageResult& legacy, bool countPackage)
    {
        ...
        m_lastResult.filesCopied = legacy.assetCount + legacy.dllCount;
    ...
    const fs::path outputRoot = fs::absolute(fs::path(config.outputDirectory) / std::format(
                                                           "{}_{}_{}", config.projectName,
                                                           PlatformName(config.platform), configName));
