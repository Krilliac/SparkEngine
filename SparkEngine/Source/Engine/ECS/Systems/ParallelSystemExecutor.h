...
namespace Spark::ECS
{
    ...
    // ========================================================================
    // Stage-Based Executor
    // ========================================================================

    /**
     * @brief ECS execution stage (determines sequential ordering between stages)
     *
     * Systems within the same stage may run in parallel.
     * Stages execute sequentially: Physics -> Animation -> AI -> Audio -> Lifecycle -> Render.
     */
    ...
    /**
     * @brief A registered system entry with its stage and update function
     */
    struct SystemEntry
    {
        std::string name;
        SystemStage stage = SystemStage::Lifecycle;
        std::function<void(float)> updateFn;
    };

    /**
     * @brief Stage-based parallel system executor
     *
     * Groups ECS systems by execution stage and runs systems within each
     * stage concurrently on the JobSystem thread pool. Stages execute
     * sequentially to maintain the required ordering:
     *   Physics -> Animation -> AI -> Audio -> Lifecycle -> Render
     *
     * This complements the component-access-based ParallelSystemExecutor
     * by providing a simpler, explicit grouping model for the main
     * engine ECS pipeline.
     */
    class StageBasedExecutor
    {
      public:
        ...
        /**
         * @brief Register a system for parallel execution within a stage
         * @param name    Human-readable name for diagnostics
         * @param stage   The execution stage this system belongs to
         * @param fn      Update function called with delta time
         */
        void RegisterSystem(const std::string& name, SystemStage stage, std::function<void(float)> fn);

        /**
         * @brief Execute all registered systems stage by stage
         *
         * Stages run sequentially. Systems within each stage are dispatched
         * to the JobSystem for parallel execution, then joined before
        ...
        /**
         * @brief Get diagnostic status string for console output
         */
        std::string Console_GetStatus() const;

      private:
        StageBasedExecutor() = default;
