...
void AnimationEvaluator::SolveFABRIK(std::vector<XMFLOAT4X4>& localTransforms,
                                       const Skeleton& skeleton,
                                       const IKChain& chain) {
    if (!chain.enabled || chain.boneIndices.size() < 2) return;
    ...
    for (size_t i = 0; i < numJoints - 1; ++i) {
        ...
        XMVECTOR origDir = XMVector3Normalize(
            XMVectorSubtract(globalTransforms[childIdx].r[3], globalTransforms[boneIdx].r[3]));

        // Compute new direction from solved positions
        XMVECTOR newDir = XMVector3Normalize(
            XMVectorSubtract(positions[i + 1], positions[i]));
