...
namespace Spark::Animation
{
    ...
    class IAnimationSystem
    {
      public:
        ...
        [[nodiscard]] virtual std::shared_ptr<Skeleton> LoadSkeleton(const std::string& filepath) = 0;
        ...
        [[nodiscard]] virtual std::vector<std::shared_ptr<AnimationClip>> LoadAnimations(
        ...
        [[nodiscard]] virtual std::shared_ptr<AnimationClip> GetClip(const std::string& name) const = 0;

        /// @brief Retrieve a cached skeleton by name.
        /// @return Shared pointer to the skeleton, or nullptr if not found.
        [[nodiscard]] virtual std::shared_ptr<Skeleton> GetSkeleton(const std::string& name) const = 0;
