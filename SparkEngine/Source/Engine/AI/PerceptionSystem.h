...
struct PerceptionMemory {
    ...
    inline bool IsValid(float minConfidence = 0.05f) const {
...
struct PerceptionComponent {
    ...
    inline void Remember(PerceptionEntityID entityId,
                         const XMFLOAT3& position, float currentTime) {
    ...
    inline const PerceptionMemory* GetMemory(PerceptionEntityID entityId) const {
        ...
        if (it != memories.end()) return &it->second;
        return nullptr;
    }
...
inline bool CanSee(const XMFLOAT3& observerPos, const XMFLOAT3& observerForward,
...
inline bool CanHear(const XMFLOAT3& listenerPos, const XMFLOAT3& soundPos,
                    float soundRadius) {
...
inline bool CanHearWithLoudness(const XMFLOAT3& listenerPos, const XMFLOAT3& soundPos,
...
inline void UpdatePerception(
    PerceptionComponent& component,
    const XMFLOAT3& observerPos,
    const XMFLOAT3& observerForward,
    float currentTime,
    ...
    const std::vector<PerceptionEntityID>& soundSourceIds = {})
{
    ...
    for (size_t i = 0; i < targetIds.size() && i < targetPositions.size(); ++i) {
        if (CanSee(observerPos, observerForward, targetPositions[i],
                   component.sightFOV, component.sightRange)) {
    ...
    if (soundRadii.size() < soundCount) soundCount = soundRadii.size();
    if (soundSourceIds.size() < soundCount) soundCount = soundSourceIds.size();
