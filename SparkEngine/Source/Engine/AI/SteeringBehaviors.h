...
namespace Spark::AI
{
    ...
    namespace SteeringBehaviors
    {
        ...
        inline XMFLOAT3 Arrive(const XMFLOAT3& position, const XMFLOAT3& target, float maxSpeed, float slowRadius)
        {
        ...
        inline XMFLOAT3 Pursue(const XMFLOAT3& position, const XMFLOAT3& targetPos, const XMFLOAT3& targetVelocity,
                               float maxSpeed)
        {
            ...
            XMFLOAT3 predictedPos =
                SteeringDetail::Add(targetPos, SteeringDetail::Scale(targetVelocity, lookAheadTime));
        ...
        inline XMFLOAT3 Evade(const XMFLOAT3& position, const XMFLOAT3& threatPos, const XMFLOAT3& threatVelocity,
                              float maxSpeed)
        {
            ...
            XMFLOAT3 predictedPos =
                SteeringDetail::Add(threatPos, SteeringDetail::Scale(threatVelocity, lookAheadTime));
        ...
        inline XMFLOAT3 ObstacleAvoidance(const XMFLOAT3& position, const XMFLOAT3& velocity,
                                          const std::vector<Obstacle>& obstacles, float avoidRadius)
        {
            ...
            for (int i = 0; i < static_cast<int>(obstacles.size()); ++i)
            {
                ...
                if (forwardProj < 0.0f || forwardProj > feelerLength)
                    continue;

                // Compute lateral distance from the feeler line
                XMFLOAT3 projPoint = SteeringDetail::Add(position, SteeringDetail::Scale(forward, forwardProj));
        ...
        inline XMFLOAT3 Wander(const XMFLOAT3& position, const XMFLOAT3& velocity, float wanderRadius,
                               float wanderDistance, float wanderJitter, std::mt19937& rng)
        {
            ...
            if (speed < 1e-6f)
            {
                ...
                return XMFLOAT3(std::cos(angle) * wanderRadius, 0.0f, std::sin(angle) * wanderRadius);
            }
            ...
            XMFLOAT3 displacement = SteeringDetail::Add(SteeringDetail::Scale(forward, jitterForward),
        ...
        inline XMFLOAT3 Separation(const XMFLOAT3& position, const std::vector<XMFLOAT3>& neighbors,
        ...
        inline XMFLOAT3 Alignment(const XMFLOAT3& velocity, const std::vector<XMFLOAT3>& neighborVelocities)
        {
        ...
        inline XMFLOAT3 Cohesion(const XMFLOAT3& position, const std::vector<XMFLOAT3>& neighborPositions)
        {
