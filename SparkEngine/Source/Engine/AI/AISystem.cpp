...
BehaviorTree* AISystem::CreateBehaviorInstance(const std::string& templateName) {
    ...
    if (it == m_behaviorTemplates.end()) return nullptr;
...
void AISystem::UpdateMovement(World& world, AIComponent& ai, Transform& transform, float deltaTime) {
    if (ai.currentPath.empty() || ai.currentPathIndex >= ai.currentPath.size()) return;
...
std::string AISystem::Console_GetAgentInfo(World& world, EntityID entity) const {
    ...
    if (!ai) return "Entity has no AIComponent\n";
...
std::unique_ptr<BehaviorTree> CreatePatrolBehavior(const std::vector<XMFLOAT3>& patrolPoints) {
    ...
    combatSeq->AddChild(std::make_unique<ConditionNode>("HasTarget",
        [](const Blackboard& bb) { return bb.Get<bool>("hasTarget", false); }));
    combatSeq->AddChild(std::make_unique<ActionNode>("EngageTarget",
        [](float dt, Blackboard& bb) { return NodeStatus::Running; }));

    // Patrol sub-tree: walk between waypoints
    auto patrolSeq = std::make_unique<SequenceNode>("PatrolSequence");
    patrolSeq->AddChild(std::make_unique<ActionNode>("MoveToWaypoint",
        [](float dt, Blackboard& bb) { return NodeStatus::Running; }));
...
std::unique_ptr<BehaviorTree> CreateCombatBehavior(const AIAgentConfig& config) {
    ...
    fleeSeq->AddChild(std::make_unique<ConditionNode>("LowHealth",
        [](const Blackboard& bb) { return bb.Get<float>("healthPercent", 1.0f) < 0.2f; }));
    ...
    coverSeq->AddChild(std::make_unique<ConditionNode>("UnderFire",
        [](const Blackboard& bb) { return bb.Get<bool>("underFire", false); }));
    coverSeq->AddChild(std::make_unique<ActionNode>("FindCover",
        [](float dt, Blackboard& bb) { return NodeStatus::Running; }));

    // Attack if in range
    auto attackSeq = std::make_unique<SequenceNode>("AttackSequence");
    attackSeq->AddChild(std::make_unique<ConditionNode>("InAttackRange",
        [&config](const Blackboard& bb) {
    ...
    // Chase target
    auto chaseAction = std::make_unique<ActionNode>("ChaseTarget",
        [](float dt, Blackboard& bb) { return NodeStatus::Running; });
...
std::unique_ptr<BehaviorTree> CreateGuardBehavior(const XMFLOAT3& guardPosition, float guardRadius) {
    ...
    auto combatSeq = std::make_unique<SequenceNode>("CombatSequence");
    combatSeq->AddChild(std::make_unique<ConditionNode>("EnemyDetected",
        [](const Blackboard& bb) { return bb.Get<bool>("hasTarget", false); }));
    ...
    returnSeq->AddChild(std::make_unique<ConditionNode>("FarFromPost",
        [guardRadius](const Blackboard& bb) {
            return bb.Get<float>("distFromPost", 0.0f) > guardRadius;
        }));
    returnSeq->AddChild(std::make_unique<ActionNode>("MoveToPost",
        [](float dt, Blackboard& bb) { return NodeStatus::Running; }));

    // Idle
    auto idle = std::make_unique<ActionNode>("Idle",
        [](float dt, Blackboard& bb) { return NodeStatus::Running; });
