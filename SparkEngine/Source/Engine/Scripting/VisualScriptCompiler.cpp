...
namespace Spark::Scripting
{
    ...
    {
        SPARK_LOG_INFO(Spark::LogCategory::Scripting, "VisualScriptCompiler::Compile — class='%s', nodes=%zu, debug=%s",
                       graph.className.c_str(), graph.nodes.size(), debugMode ? "true" : "false");
        ScriptCompileResult result;

        if (graph.nodes.empty())
        {
            SPARK_LOG_WARN(Spark::LogCategory::Scripting, "Compile failed: empty graph for class '%s'",
                           graph.className.c_str());
        ...
        result.success = true;
        SPARK_LOG_INFO(Spark::LogCategory::Scripting, "Compile succeeded for '%s' — %zu bytes of AngelScript generated",
                       graph.className.c_str(), result.angelScriptSource.size());
        return result;
    }
