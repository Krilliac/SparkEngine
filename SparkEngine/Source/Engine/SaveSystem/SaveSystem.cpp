...
void ComponentSerializerRegistry::Register(const std::string& typeName,
...
SerializedComponent ComponentSerializerRegistry::Serialize(const std::string& typeName,
                                                            const void* component) const {
    auto it = m_serializers.find(typeName);
    if (it != m_serializers.end()) return it->second.serialize(component);
    return {};
}

void ComponentSerializerRegistry::Deserialize(const std::string& typeName,
                                               World& world, EntityID entity,
                                               const SerializedComponent& data) const {
    auto it = m_serializers.find(typeName);
    if (it != m_serializers.end()) it->second.deserialize(world, entity, data);
}
...
void ComponentSerializerRegistry::RegisterBuiltins() {
    ...
    Register("LightComponent",
        ...
        [](World& world, EntityID entity, const SerializedComponent& data) {
            ...
            l.castShadows = (data.properties.count("castShadows") &&
                             data.properties.at("castShadows") == "1");
        });
    ...
    Register("Camera",
        ...
        [](World& world, EntityID entity, const SerializedComponent& data) {
            ...
            c.isMainCamera = (data.properties.count("isMainCamera") &&
                              data.properties.at("isMainCamera") == "1");
        });
    ...
    Register("RigidBodyComponent",
        ...
        [](World& world, EntityID entity, const SerializedComponent& data) {
            ...
            rb.isTrigger = (data.properties.count("isTrigger") &&
                            data.properties.at("isTrigger") == "1");
...
bool SaveSystem::Load(const std::string& slotName, World& world) {
    SaveData data;
    if (!ReadFromFile(GetSavePath(slotName), data)) return false;
...
bool SaveSystem::WriteToFile(const std::string& filepath, const SaveData& data) const {
    try {
        ...
        metaStream << data.metadata.playerPosition.x << " "
                   << data.metadata.playerPosition.y << " "
...
bool SaveSystem::ReadFromFile(const std::string& filepath, SaveData& outData) const {
    try {
        ...
        char magic[4];
        file.read(magic, 4);
        if (std::string(magic, 4) != "SPRK") return false;
        ...
        metaStream >> outData.metadata.playerPosition.x
                   >> outData.metadata.playerPosition.y
...
std::string SaveSystem::Console_ListSaves() const {
    ...
    for (const auto& slot : slots) {
        ss << "  " << slot.saveName << " [" << slot.sceneName
           << ", HP:" << slot.playerHealth
...
std::string SaveSystem::Console_GetSaveInfo(const std::string& slotName) const {
    ...
    ss << "Position: (" << meta.playerPosition.x << ", "
       << meta.playerPosition.y << ", " << meta.playerPosition.z << ")\n";
    return ss.str();
}
