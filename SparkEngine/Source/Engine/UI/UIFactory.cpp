...
namespace Spark::UI
{
    ...
    UIFloatBinding::UIFloatBinding(float* target, std::function<void(float)> onChange)
    ...
    void UIFloatBinding::PushToWidget()
    {
        if (m_target)
            m_widgetValue = *m_target;
    }

    void UIFloatBinding::PullFromWidget()
    {
    ...
    std::string UIFloatBinding::GetDisplayValue() const
    {
        return std::to_string(m_widgetValue);
    }

    void UIFloatBinding::SetValue(float value)
    {
        m_widgetValue = value;
        PullFromWidget();
    }
    ...
    UIStringBinding::UIStringBinding(std::string* target, std::function<void(const std::string&)> onChange)
        : m_target(target), m_onChange(std::move(onChange))
    {
        if (m_target)
            m_widgetValue = *m_target;
    ...
    void UIStringBinding::PushToWidget()
    {
        if (m_target)
            m_widgetValue = *m_target;
    }

    void UIStringBinding::PullFromWidget()
    {
        if (m_target && *m_target != m_widgetValue)
        {
            *m_target = m_widgetValue;
    ...
    std::string UIStringBinding::GetDisplayValue() const
    {
    ...
    UIBoolBinding::UIBoolBinding(bool* target, std::function<void(bool)> onChange)
        : m_target(target), m_onChange(std::move(onChange))
    {
        if (m_target)
            m_widgetValue = *m_target;
    ...
    void UIBoolBinding::PushToWidget()
    {
        if (m_target)
            m_widgetValue = *m_target;
    }

    void UIBoolBinding::PullFromWidget()
    {
        if (m_target && *m_target != m_widgetValue)
        {
            *m_target = m_widgetValue;
    ...
    std::string UIBoolBinding::GetDisplayValue() const
    {
    ...
    UIIntBinding::UIIntBinding(int32_t* target, std::function<void(int32_t)> onChange)
        : m_target(target), m_onChange(std::move(onChange))
    {
        if (m_target)
            m_widgetValue = *m_target;
    ...
    void UIIntBinding::PushToWidget()
    {
        if (m_target)
            m_widgetValue = *m_target;
    }

    void UIIntBinding::PullFromWidget()
    {
        if (m_target && *m_target != m_widgetValue)
        {
            *m_target = m_widgetValue;
    ...
    std::string UIIntBinding::GetDisplayValue() const
    {
        return std::to_string(m_widgetValue);
    }
