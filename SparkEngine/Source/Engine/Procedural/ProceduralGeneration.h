...
public:
    ...
    float FBM(float x, float y, int octaves = 6, float lacunarity = 2.0f,
              float persistence = 0.5f) const;

    /// Ridged multifractal noise
    float RidgedMultifractal(float x, float y, int octaves = 6,
                              float lacunarity = 2.0f, float gain = 2.0f) const;
...
public:
    ...
    static void ApplyThermalErosion(std::vector<float>& heightmap, int width, int height,
                                     int iterations = 100, float talus = 0.01f);

    /// Apply hydraulic erosion
    static void ApplyHydraulicErosion(std::vector<float>& heightmap, int width, int height,
                                       int iterations = 10000, uint32_t seed = 42);
...
public:
    ...
    static ProceduralMeshData CreateTerrainFromHeightmap(
...
public:
    ...
    static std::vector<PlacementResult> PlaceObjects(
        const float* heightmap, int width, int height,
