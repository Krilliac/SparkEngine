/**
 * @file MeshLODLinux.cpp
 * @brief Linux implementation — split from MeshLOD.cpp
 */
...
namespace Spark::Graphics
{
    ...
    // ============================================================================
    // MeshSimplifier (Linux stub)
    // ============================================================================

    SimplificationResult MeshSimplifier::Simplify(const SimplificationVertex* vertices, uint32_t vertexCount,
                                                  ...
                                                  uint32_t targetIndexCount, float maxError)
    {
        ...
        // Simplified edge collapse (same algorithm, no DirectXMath)
        struct Edge
        {
            uint32_t v0, v1;
            float cost;
        ...
        while (result.indices.size() > targetIndexCount && result.indices.size() >= 3)
        {
            ...
            for (size_t i = 0; i + 2 < result.indices.size(); i += 3)
            {
                ...
                for (int e = 0; e < 3; ++e)
                {
                    ...
                    uint64_t edgeKey =
                        (static_cast<uint64_t>((std::min)(a, b)) << 32) | static_cast<uint64_t>((std::max)(a, b));
    ...
    LODChain LODManager::GenerateLODChain(const std::string& meshName, const SimplificationVertex* vertices,
                                          ...
                                          const LODGenerationSettings& settings)
    {
        ...
        for (int i = 0; i < numLevels; ++i)
        {
            ...
            {
                auto simplified = MeshSimplifier::SimplifyByRatio(vertices, vertexCount, indices, indexCount,
                ...
                level.qualityPercent =
                    (indexCount > 0) ? (static_cast<float>(level.indexCount) / indexCount * 100.0f) : 0.0f;
            }
    ...
    std::string LODManager::Console_ListLODChains() const
    {
        ...
        {
            ss << "  " << name << " [" << chain.levels.size() << " levels, " << chain.totalVertices
               << " total verts]\n";
        }
    ...
    {
        auto it = m_lodChains.find(meshName);
        if (it == m_lodChains.end())
            return "LOD chain '" + meshName + "' not found\n";
        ...
        for (size_t i = 0; i < chain.levels.size(); ++i)
        {
            ...
            ss << "  LOD " << i << ": " << level.indexCount / 3 << " tris (" << level.qualityPercent
               << "%), dist: " << level.maxDistance << "\n";
        }
        return ss.str();
    }
