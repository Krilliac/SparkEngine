/**
 * @file GPUParticleSystemWindows.cpp
 * @brief Windows/D3D11 GPU particle system — compute shader pipeline, buffer management, rendering
 *
 * Contains all D3D11-dependent methods: Initialize, CompileComputeShaders,
 * Shutdown, CreateEmitter, DestroyEmitter, CreateEmitterBuffers, emitter control,
 * Update/DispatchEmitter, BitonicSort, Render, and status queries.
 * Linux counterpart (stubs) lives in GPUParticleSystemLinux.cpp.
 */
...
HRESULT GPUParticleSystem::CreateEmitterBuffers(GPUEmitterResources& emitter)
{
...

    {
        ...
        bufDesc.ByteWidth = sizeof(initArgs);
        bufDesc.Usage = D3D11_USAGE_DEFAULT;
        bufDesc.BindFlags = D3D11_BIND_UNORDERED_ACCESS;
        bufDesc.MiscFlags = D3D11_RESOURCE_MISC_DRAWINDIRECT_ARGS;
        D3D11_SUBRESOURCE_DATA initData = {};
        initData.pSysMem = initArgs;
        hr = m_device->CreateBuffer(&bufDesc, &initData, emitter.indirectArgsBuffer.GetAddressOf());
        if (FAILED(hr))
            return hr;
    ...
    {
        D3D11_BUFFER_DESC bufDesc = {};
        bufDesc.ByteWidth = sizeof(GPUParticleEmitterCB);
        bufDesc.Usage = D3D11_USAGE_DYNAMIC;
        bufDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
        bufDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
        hr = m_device->CreateBuffer(&bufDesc, nullptr, emitter.emitterCBBuffer.GetAddressOf());
        if (FAILED(hr))
            return hr;
    }
...
void GPUParticleSystem::UpdateEmitterCB(GPUEmitterResources& emitter, float deltaTime)
{
    ...
    uint32_t emitCountCB = std::min(static_cast<uint32_t>(pendingEmission), emitter.capacity);
