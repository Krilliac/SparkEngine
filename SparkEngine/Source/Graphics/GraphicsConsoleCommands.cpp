...
void RegisterGraphicsConsoleCommands(GraphicsEngine& engine)
{
    ...
    console.RegisterCommand("gfx_vsync", [&engine](const std::vector<std::string>& args) -> std::string {
    ...
    console.RegisterCommand("gfx_wireframe", [&engine](const std::vector<std::string>& args) -> std::string {
    ...
    console.RegisterCommand("gfx_metrics", [&engine](const std::vector<std::string>&) -> std::string {
        try {
    ...
    console.RegisterCommand("gfx_screenshot", [&engine](const std::vector<std::string>& args) -> std::string {
    ...
    console.RegisterCommand("gfx_quality", [&engine](const std::vector<std::string>& args) -> std::string {
        if (args.empty()) return "Usage: gfx_quality <low|medium|high|ultra>";
        engine.Console_SetQuality(args[0]);
        return "Quality set to: " + args[0];
    }, "Set graphics quality preset");

    console.RegisterCommand("gfx_renderpath", [&engine](const std::vector<std::string>& args) -> std::string {
        if (args.empty()) return "Usage: gfx_renderpath <forward|deferred>";
        engine.Console_SetRenderPath(args[0]);
        return "Render path set to: " + args[0];
    }, "Set render path (forward/deferred)");

    console.RegisterCommand("gfx_feature", [&engine](const std::vector<std::string>& args) -> std::string {
        if (args.size() < 2) return "Usage: gfx_feature <name> <on|off>";
    ...
    console.RegisterCommand("gfx_reload_shaders", [&engine](const std::vector<std::string>&) -> std::string {
        engine.Console_ReloadShaders();
        return "Shaders reloaded";
    }, "Reload all shaders");

    console.RegisterCommand("gfx_sysinfo", [&engine](const std::vector<std::string>&) -> std::string {
        return engine.Console_GetSystemInfo();
    }, "Display graphics system information");

    console.RegisterCommand("gfx_benchmark", [&engine](const std::vector<std::string>& args) -> std::string {
        int seconds = args.empty() ? 10 : std::stoi(args[0]);
        return engine.Console_Benchmark(seconds);
    }, "Run graphics benchmark");

    console.RegisterCommand("gfx_hdr", [&engine](const std::vector<std::string>& args) -> std::string {
    ...
    console.RegisterCommand("gfx_debug", [&engine](const std::vector<std::string>& args) -> std::string {
        if (args.empty()) return "Usage: gfx_debug <on|off>";
        bool enable = (args[0] == "on" || args[0] == "true" || args[0] == "1");
    ...
    console.RegisterCommand("gfx_clearcolor", [&engine](const std::vector<std::string>& args) -> std::string {
        if (args.size() < 3) return "Usage: gfx_clearcolor <r> <g> <b> [a]";
    ...
    console.RegisterCommand("gfx_renderscale", [&engine](const std::vector<std::string>& args) -> std::string {
        if (args.empty()) return "Usage: gfx_renderscale <0.25-2.0>";
    ...
    console.RegisterCommand("gfx_reset", [&engine](const std::vector<std::string>&) -> std::string {
        engine.Console_ResetToDefaults();
        return "Graphics settings reset to defaults";
    }, "Reset all graphics settings to defaults");

    console.RegisterCommand("gfx_gc", [&engine](const std::vector<std::string>&) -> std::string {
    ...
    console.RegisterCommand("gfx_gpu_timing", [&engine](const std::vector<std::string>& args) -> std::string {
        if (args.empty()) return "Usage: gfx_gpu_timing <on|off>";
        bool enable = (args[0] == "on" || args[0] == "true" || args[0] == "1");
        engine.Console_SetGPUTiming(enable);
        return enable ? "GPU timing enabled" : "GPU timing disabled";
    }, "Enable/disable GPU timing queries");

    console.RegisterCommand("gfx_vram", [&engine](const std::vector<std::string>&) -> std::string {
    ...
    console.RegisterCommand("gfx_reset_device", [&engine](const std::vector<std::string>&) -> std::string {
