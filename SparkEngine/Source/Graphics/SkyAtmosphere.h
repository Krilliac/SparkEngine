...
namespace Spark
{
    ...
    namespace Graphics
    {
        ...
        struct AtmosphereSettings
        {
            ...
            XMFLOAT3 rayleighScattering = {SkyConstants::kRayleighCoeffR, SkyConstants::kRayleighCoeffG,
            ...
            XMFLOAT3 ozoneAbsorption = {SkyConstants::kOzoneAbsorptionR, SkyConstants::kOzoneAbsorptionG,
            ...
            void ResetToEarthDefaults() { *this = AtmosphereSettings{}; }
        };
        ...
        class SkyAtmosphereSystem
        {
          public:
             ...
             */
            void Render([[maybe_unused]] ID3D11DeviceContext* context, [[maybe_unused]] const XMMATRIX& inverseViewProj,
             ...
             */
            void RenderToCubemap([[maybe_unused]] ID3D11DeviceContext* context,
            ...
            void SetSunIntensity(float intensity) { m_atmosphereSettings.sunIntensity = std::max(0.0f, intensity); }
            ...
            void SetSunDiskSize(float radians) { m_atmosphereSettings.sunDiskSize = std::max(0.0f, radians); }
            ...
            void SetCloudCoverage(float coverage) { m_cloudSettings.coverage = std::clamp(coverage, 0.0f, 1.0f); }

            void SetCloudDensity(float density) { m_cloudSettings.density = std::max(0.0f, density); }

            void SetCloudWindSpeed(float speed) { m_cloudSettings.windSpeed = std::max(0.0f, speed); }
...
#ifdef SPARK_PLATFORM_WINDOWS
            ID3D11ShaderResourceView* GetTransmittanceLutSRV() const { return m_transmittanceLutSRV.Get(); }

            ID3D11ShaderResourceView* GetMultiScatterLutSRV() const { return m_multiScatterLutSRV.Get(); }

            ID3D11ShaderResourceView* GetSkyViewLutSRV() const { return m_skyViewLutSRV.Get(); }
#endif
            ...
            {
                return XMConvertToDegrees(std::asin(std::clamp(m_atmosphereSettings.sunDirection.y, -1.0f, 1.0f)));
            }
            ...
            XMFLOAT3 ComputeTransmittanceCPU(float heightKm, float cosZenith) const
            {
                ...
                for (int i = 0; i < kSteps; ++i)
                {
                    ...
                    float ozoneDensity =
                        std::max(0.0f, 1.0f - std::abs(sampleHeight - s.ozoneCenterAltitude) / s.ozoneWidth);

                    opticalDepthR +=
                        (s.rayleighScattering.x * rayleighDensity + (s.mieScattering + s.mieAbsorption) * mieDensity +
                         s.ozoneAbsorption.x * ozoneDensity) *
                        stepSize;

                    opticalDepthG +=
                        (s.rayleighScattering.y * rayleighDensity + (s.mieScattering + s.mieAbsorption) * mieDensity +
                         s.ozoneAbsorption.y * ozoneDensity) *
                        stepSize;

                    opticalDepthB +=
                        (s.rayleighScattering.z * rayleighDensity + (s.mieScattering + s.mieAbsorption) * mieDensity +
                         s.ozoneAbsorption.z * ozoneDensity) *
                        stepSize;
                }

                return {std::exp(-opticalDepthR), std::exp(-opticalDepthG), std::exp(-opticalDepthB)};
            }
            ...
            XMFLOAT3 EstimateSkyLuminanceCPU(const XMFLOAT3& viewDir) const
            {
                ...
                float cosTheta =
                    viewDir.x * s.sunDirection.x + viewDir.y * s.sunDirection.y + viewDir.z * s.sunDirection.z;
                ...
                float miePhase = (denom > 0.0001f) ? (1.0f - g2) / (4.0f * XM_PI * std::pow(denom, 1.5f)) : 1.0f;
                ...
                float lr =
                    s.rayleighScattering.x * rayleighDensity * rayleighPhase + s.mieScattering * mieDensity * miePhase;
                float lg =
                    s.rayleighScattering.y * rayleighDensity * rayleighPhase + s.mieScattering * mieDensity * miePhase;
                float lb =
                    s.rayleighScattering.z * rayleighDensity * rayleighPhase + s.mieScattering * mieDensity * miePhase;

                float sunFactor = s.sunIntensity * std::max(0.0f, s.sunDirection.y);

                return {lr * transmittance.x * sunFactor * s.sunColor.x * s.exposureMultiplier,
            ...
            std::string Console_GetStatus() const
            {
                ...
                s += "  Render mode: " +
                     std::string(m_skyboxSettings.mode == SkyRenderMode::Procedural ? "Procedural" : "Cubemap") + "\n";
                s += "  Sun elevation: " + std::to_string(GetSunElevationDegrees()) + " deg\n";
                s += "  Sun intensity: " + std::to_string(m_atmosphereSettings.sunIntensity) + "\n";
                s += "  Sun direction: (" + std::to_string(m_atmosphereSettings.sunDirection.x) + ", " +
                ...
                s += "  Stars: " + std::string(m_starsEnabled ? "enabled" : "disabled") + " (" +
                     std::to_string(m_starCount) + " stars, visibility: " + std::to_string(m_starVisibility) + ")\n";
            ...
            std::string Console_SetSunDirection(float x, float y, float z)
            {
                ...
                return "Sun direction set to (" + std::to_string(normalized.x) + ", " + std::to_string(normalized.y) +
            ...
            std::string Console_SetCloudType(const std::string& typeName)
            {
                ...
                {
                    return "Unknown cloud type: " + typeName + ". Options: none, 2d, layer, volumetric, vol";
                }
          ...
          private:
            ...
            {
                m_cloudSettings.windOffset.x += m_cloudSettings.windDirection.x * m_cloudSettings.windSpeed * deltaTime;
                m_cloudSettings.windOffset.y += m_cloudSettings.windDirection.y * m_cloudSettings.windSpeed * deltaTime;
            }
            ...
            void UpdateAtmosphereCBuffer([[maybe_unused]] const XMMATRIX& inverseViewProj,
                                         [[maybe_unused]] const XMFLOAT3& cameraPosition)
            {
                ...
                m_cbufferData.nightTransition = std::clamp(1.0f - (atm.sunDirection.y + 0.1f) / 0.2f, 0.0f, 1.0f);
            }
            ...
            bool CreateConstantBuffer()
            {
                ...
                HRESULT hr = m_device->CreateBuffer(&desc, nullptr, m_atmosphereCBuffer.GetAddressOf());
                return SUCCEEDED(hr);
            }
            ...
            bool CreateTransmittanceLut()
            {
                ...
                HRESULT hr = m_device->CreateTexture2D(&texDesc, nullptr, m_transmittanceLutTexture.GetAddressOf());
                if (FAILED(hr))
                    return false;

                hr = m_device->CreateShaderResourceView(m_transmittanceLutTexture.Get(), nullptr,
                                                        m_transmittanceLutSRV.GetAddressOf());
                if (FAILED(hr))
                    return false;

                hr = m_device->CreateUnorderedAccessView(m_transmittanceLutTexture.Get(), nullptr,
            ...
            bool CreateMultiScatterLut()
            {
                ...
                HRESULT hr = m_device->CreateTexture2D(&texDesc, nullptr, m_multiScatterLutTexture.GetAddressOf());
                if (FAILED(hr))
                    return false;

                hr = m_device->CreateShaderResourceView(m_multiScatterLutTexture.Get(), nullptr,
                                                        m_multiScatterLutSRV.GetAddressOf());
                if (FAILED(hr))
                    return false;

                hr = m_device->CreateUnorderedAccessView(m_multiScatterLutTexture.Get(), nullptr,
            ...
            bool CreateSkyViewLut()
            {
                ...
                HRESULT hr = m_device->CreateTexture2D(&texDesc, nullptr, m_skyViewLutTexture.GetAddressOf());
                if (FAILED(hr))
                    return false;

                hr = m_device->CreateShaderResourceView(m_skyViewLutTexture.Get(), nullptr,
                                                        m_skyViewLutSRV.GetAddressOf());
                if (FAILED(hr))
                    return false;

                hr = m_device->CreateUnorderedAccessView(m_skyViewLutTexture.Get(), nullptr,
            ...
            bool CreateStarVertexBuffer()
            {
                ...
                m_starVertexBuffer.Reset();
                HRESULT hr = m_device->CreateBuffer(&desc, &initData, m_starVertexBuffer.GetAddressOf());
                return SUCCEEDED(hr);
            }
            ...
            void UploadConstantBuffer(ID3D11DeviceContext* context)
            {
                ...
                HRESULT hr = context->Map(m_atmosphereCBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped);
                if (SUCCEEDED(hr))
                {
            ...
            void DispatchTransmittanceLut(ID3D11DeviceContext* context)
            {
                ...
                context->Dispatch((SkyConstants::kTransmittanceLutWidth + 7) / 8,
                                  (SkyConstants::kTransmittanceLutHeight + 7) / 8, 1);
            ...
            void DispatchMultiScatterLut(ID3D11DeviceContext* context)
            {
                ...
                context->Dispatch((SkyConstants::kMultiScatterLutSize + 7) / 8,
                                  (SkyConstants::kMultiScatterLutSize + 7) / 8, 1);
            ...
            void DispatchSkyViewLut(ID3D11DeviceContext* context)
            {
                ...
                ID3D11ShaderResourceView* srvs[] = {m_transmittanceLutSRV.Get(), m_multiScatterLutSRV.Get()};
                ...
                context->Dispatch((SkyConstants::kSkyViewLutWidth + 7) / 8, (SkyConstants::kSkyViewLutHeight + 7) / 8,
                                  1);
