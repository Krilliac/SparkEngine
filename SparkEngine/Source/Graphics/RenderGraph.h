...
struct RenderGraphResourceHash
{
    size_t operator()(const RenderGraphResource& r) const
    {
        return std::hash<uint64_t>{}(
            (static_cast<uint64_t>(r.index) << 32) | r.version);
    }
};
...
struct RenderGraphTextureDesc
{
    ...
    RenderTargetUsage usage = RenderTargetUsage::RenderTarget
                            | RenderTargetUsage::ShaderResource;
...
{
public:
    explicit RenderGraphResourceRegistry(
        const std::vector<RenderGraphResourceNode>& resources)
...
public:
     ...
     */
    RenderGraphResource Create(const std::string& name,
...
public:
    ...
    bool Has() const
    {
        return m_storage.count(std::type_index(typeid(T))) > 0;
    }
    ...
    void Remove()
    {
        m_storage.erase(std::type_index(typeid(T)));
    }
...
public:
     ...
     */
    const std::vector<std::unique_ptr<RenderGraphPass>>& GetPasses() const
    {
        return m_passes;
    }
     ...
     */
    const std::vector<RenderGraphResourceNode>& GetResources() const
    {
        return m_resources;
    }
     ...
     */
    const std::vector<uint32_t>& GetExecutionOrder() const
    {
    ...
    std::string DebugVisualize() const
    {
        ...
        {
            ss << "  [" << pass->GetIndex() << "] " << pass->GetName()
               << " (" << PassTypeToString(pass->GetType()) << ")";
            if (pass->IsCulled())
            {
                ss << " [CULLED]";
            }
        ...
        for (size_t i = 0; i < m_resources.size(); ++i)
        {
            ...
            ss << "  [" << i << "] " << res.name
               << " (" << (res.lifetime == RenderGraphResourceLifetime::Transient
                           ? "Transient" : "Imported")
               << ", refs=" << res.refCount << ")\n";
        }
    ...
    std::string ExportDot() const
    {
        ...
        {
            const auto& res = m_resources[i];
            std::string shape = (res.lifetime == RenderGraphResourceLifetime::Imported)
                ? "ellipse" : "diamond";
            ss << "  res_" << i
               << " [label=\"" << res.name << "\""
               << ", shape=" << shape
    ...
    {
        std::ostringstream ss;
        ss << "RenderGraph '" << m_name << "': "
           << m_stats.executedPasses << "/" << m_stats.totalPasses << " passes executed, "
           << m_stats.culledPasses << " culled, "
           << m_stats.totalResources << " resources ("
           << m_stats.transientResources << " transient, "
           << m_stats.importedResources << " imported), "
           << (m_stats.peakTransientMemory / 1024) << " KB peak, "
           << m_stats.compileTimeMs << " ms compile, "
           << m_stats.executeTimeMs << " ms execute";
        return ss.str();
    }
     ...
     */
    RenderGraphResource CreateResource(const std::string& name,
                                       const RenderGraphTextureDesc& desc,
     ...
     */
    void RecordAlias(RenderGraphResource from, RenderGraphResource to)
    {
        m_aliasPairs.push_back({from, to});
    }
...
private:
    ...
    void ComputeResourceLifetimes()
    {
        ...
        for (const auto& pass : m_passes)
        {
            ...
            auto updateLifetime = [&](RenderGraphResource handle)
            {
                ...
                if (res.firstUsePass == RenderGraphResource::INVALID_INDEX
                    || pi < res.firstUsePass)
                {
                    res.firstUsePass = pi;
                }
    ...
    void CullUnreferencedPasses()
    {
        ...
        while (!workQueue.empty())
        {
            ...
            for (const auto& r : pass.GetReads())
            {
                ...
                if (producer != RenderGraphResource::INVALID_INDEX
                    && producer < m_passes.size()
    ...
    void AnalyzeResourceAliasing()
    {
        ...
        for (const auto& [from, to] : m_aliasPairs)
        {
            ...
            if (srcRes.lastUsePass < dstRes.firstUsePass ||
                dstRes.lastUsePass < srcRes.firstUsePass)
            {
    ...
    std::string ResourceToString(RenderGraphResource handle) const
    {
        ...
        return m_resources[handle.index].name + ":v"
...
inline RenderGraphResource RenderGraphBuilder::Create(
    const std::string& name,
    const RenderGraphTextureDesc& desc)
{
...
inline void RenderGraphBuilder::Alias(RenderGraphResource from,
