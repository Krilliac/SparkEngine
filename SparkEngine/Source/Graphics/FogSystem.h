...
namespace FogMath {
    ...
    inline float HeightDensity(float y, float baseHeight, float heightFalloff, float baseDensity) {
        ...
        if (heightAboveBase < 0.0f) return baseDensity;
        return baseDensity * std::exp(-heightFalloff * heightAboveBase);
    }

    /// Integrated height fog along a ray from camera to fragment
    inline float IntegratedHeightFog(
        float camY, float fragY, float distance,
        float baseHeight, float heightFalloff, float baseDensity)
    {
        if (heightFalloff < 0.0001f) return ExponentialFog(distance, baseDensity);
...
public:
    ...
    float ComputeFogFactor(float distance) const {
        if (!m_enabled || m_mode == FogMode::None) return 0.0f;
    ...
    float ComputeHeightFogFactor(float cameraY, float fragmentY, float distance) const {
        ...
        if (m_mode == FogMode::Height || m_mode == FogMode::ExponentialHeight) {
            return FogMath::IntegratedHeightFog(
                cameraY, fragmentY, distance,
                m_heightSettings.baseHeight, m_heightSettings.heightFalloff,
                m_heightSettings.baseDensity);
        }
    ...
    std::string Console_GetStatus() const {
        static const char* modeNames[] = {
            "None", "Linear", "Exponential", "ExpSquared",
        ...
        s += "  Color: (" + std::to_string(m_color.r) + ", " + std::to_string(m_color.g) + ", " + std::to_string(m_color.b) + ")\n";
        if (m_mode == FogMode::Linear) {
            s += "  Range: " + std::to_string(m_linearSettings.start) + " - " + std::to_string(m_linearSettings.end) + "\n";
        } else {
