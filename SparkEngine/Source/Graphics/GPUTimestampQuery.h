...
public:
     ...
     */
    const std::unordered_map<std::string, float>& GetAllPassTimes() const
    {
        return m_passTimes;
    }
...
private:
    ...
    void CollectResults(ID3D11DeviceContext* context, uint32_t frameSlot)
    {
        ...
        HRESULT hr = context->GetData(frameData.disjointQuery.Get(), &disjointData,
        ...
        for (uint32_t i = 0; i < frameData.activeCount; ++i)
        {
            ...
            hr = context->GetData(frameData.beginQueries[i].Get(), &beginTicks,
            ...
            hr = context->GetData(frameData.endQueries[i].Get(), &endTicks,
                                   sizeof(UINT64), D3D11_ASYNC_GETDATA_DONOTFLUSH);
            if (hr != S_OK)
            {
                continue;
...
public:
    ...
    ~ScopedTimestamp()
    {
        m_query.EndTimestamp(m_context, m_timerID);
    }
