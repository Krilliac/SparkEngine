...
namespace SparkEditor
{
    ...
    SparkEngineIntegration::SparkEngineIntegration()
        : m_connectionStatus(EngineConnectionStatus::Disconnected), m_shouldStop(false), m_selectedEntityId(0),
          m_device(nullptr), m_context(nullptr), m_isInitialized(false), m_updateTimer(0.0f)
    {
    }
    ...
    void SparkEngineIntegration::CommunicationThread()
    {
        while (!m_shouldStop.load())
        {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<int>(m_updateInterval * 1000.0f)));
        }
    }
