...
namespace SparkEditor
{
    ...
    bool AssetDatabase::ImportAsset(const std::string& filePath)
    {
        ...
        try
        {
            ...
            asset.lastModified = std::chrono::duration_cast<std::chrono::seconds>(ftime.time_since_epoch()).count();
        }
        catch (...)
        {
            asset.fileSize = 0;
    ...
    bool AssetDatabase::LoadAssetMetadata(const std::string& assetPath)
    {
        std::string metadataPath =
            m_metadataDirectory + "/" + std::filesystem::path(assetPath).filename().string() + ".meta";
        ...
        if (!compressionFmt.empty())
            settings.compressionFormat = compressionFmt;
        if (!audioFmt.empty())
            settings.audioFormat = audioFmt;
        if (!scaleStr.empty())
        {
            try
            {
                settings.scaleFactor = std::stof(scaleStr);
            }
            catch (...)
            {
            }
    ...
    void AssetDatabase::FileSystemMonitoringThread()
    {
        ...
        HANDLE hDirectory = CreateFileA(m_assetDirectory.c_str(), FILE_LIST_DIRECTORY,
        ...
        while (m_isMonitoring)
        {
            ...
            BOOL result = ReadDirectoryChangesW(hDirectory, buffer.data(), bufferSize,
    ...
    void AssetDatabase::UpdateAssetModificationTime(const std::string& assetPath)
    {
        try
        {
            ...
            std::time_t modTime = std::chrono::duration_cast<std::chrono::seconds>(ftime.time_since_epoch()).count();
