...
namespace SparkEditor
{
    ...
    bool GameViewPanel::RenderWorldToTexture(int width, int height)
    {
        ...
        // Pose comes from this panel's fly camera (seeded from the scene camera in
        // SetWorld); lens settings come from the scene camera component.
        const XMMATRIX rotation = XMMatrixRotationRollPitchYaw(m_cameraPitch, m_cameraYaw, 0.0f);
        const XMFLOAT3 eyePosition{m_cameraPosX, m_cameraPosY, m_cameraPosZ};
    ...
    void GameViewPanel::HandleInput(float deltaTime)
    {
        ...
        // The fly camera stays panel-local. Writing it into the document camera's
        // Transform would let a play session silently move the authored camera,
        // and "Keep Changes" would then persist a move the user never made.
    ...
    void GameViewPanel::RenderFPSHUD()
    {
        ...
        // === Provenance banner ===
        // Every readout below is generated by this panel, not by a running game
        // module. Say so on screen so the HUD is never read as live game state.
        {
            ImDrawList* dl = ImGui::GetWindowDrawList();
            const char* banner = "SIMULATED HUD - not game data";
            const ImVec2 bannerSize = ImGui::CalcTextSize(banner);
            const ImVec2 bannerPos(contentCX - bannerSize.x * 0.5f, contentTop + 2.0f);
            dl->AddRectFilled(ImVec2(bannerPos.x - 6.0f, bannerPos.y - 2.0f),
                              ImVec2(bannerPos.x + bannerSize.x + 6.0f, bannerPos.y + bannerSize.y + 2.0f),
                              IM_COL32(60, 30, 0, 190), 3.0f);
            dl->AddText(bannerPos, IM_COL32(255, 200, 120, 235), banner);
        }
