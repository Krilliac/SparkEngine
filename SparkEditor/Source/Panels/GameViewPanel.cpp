...
bool GameViewPanel::Initialize() {
    ...
    m_scoreboardEntries = {
        {"Player1", 15, 8, 2200, 32, true},
        {"FragMaster", 18, 5, 2650, 45, false},
        {"SniperElite", 12, 9, 1800, 28, false},
        {"RocketJock", 10, 12, 1500, 55, false},
        {"NovaCombat", 8, 7, 1200, 40, false},
        {"GhostRecon", 14, 6, 2100, 38, false}
    };
...
void GameViewPanel::Update(float deltaTime) {
    ...
    if (m_killFeedSpawnTimer > 6.0f) {
        ...
        static const char* names[] = {"Player1", "FragMaster", "SniperElite", "RocketJock", "NovaCombat", "GhostRecon"};
        ...
        if (ki != vi) {
            ...
            while (m_killFeed.size() > 5) m_killFeed.pop_back();
        }
    }
...
void GameViewPanel::RenderToolbar() {
    ...
    if (ImGui::IsItemHovered()) ImGui::SetTooltip("Simulate taking damage");
    ImGui::SameLine();
...
void GameViewPanel::RenderGameContent() {
    ...
    for (int i = -15; i <= 15; i++) {
        ...
        drawList->AddLine(ImVec2(topX, groundY),
                         ImVec2(bottomX, pos.y + viewportSize.y),
    ...
    for (int i = 0; i < 5; i++) {
        ...
        drawList->AddRectFilled(
            ImVec2(sx, structY - sh), ImVec2(sx + sw, structY),
            IM_COL32(40, 45, 50, 150));
    }
...
void GameViewPanel::RenderFPSHUD() {
    ...
    if (m_showStats) {
        ...
        char stats[128];
        snprintf(stats, sizeof(stats), "HP: %.0f/%.0f  ARM: %.0f  STM: %.0f\nAmmo: %d/%d (+%d)\nPos: 128.5, 1.0, 64.2",
                 m_health, m_maxHealth, m_armor, m_stamina, m_ammo, m_maxAmmo, m_reserveAmmo);
        ImVec2 statPos(winPos.x + pad, contentTop + pad);
        dl->AddRectFilled(statPos, ImVec2(statPos.x + 220, statPos.y + 54),
...
void GameViewPanel::RenderHealthArmorBars(float baseX, float baseY, float width) {
    ...
    if (m_armor > 0) {
        // Background
        dl->AddRectFilled(ImVec2(baseX, armorY), ImVec2(baseX + width, armorY + barH),
                         IM_COL32(0, 0, 0, 160), 3.0f);
        // Border
        dl->AddRect(ImVec2(baseX, armorY), ImVec2(baseX + width, armorY + barH),
                   IM_COL32(60, 70, 90, 200), 3.0f);
        // Fill
    ...
    // Background
    dl->AddRectFilled(ImVec2(baseX, healthY), ImVec2(baseX + width, healthY + barH),
                     IM_COL32(0, 0, 0, 160), 3.0f);
    // Border
    dl->AddRect(ImVec2(baseX, healthY), ImVec2(baseX + width, healthY + barH),
               IM_COL32(60, 70, 60, 200), 3.0f);
    // Fill with gradient effect
    dl->AddRectFilled(ImVec2(baseX + 2, healthY + 2),
                     ImVec2(baseX + 2 + (width - 4) * healthPct, healthY + barH - 2),
                     healthCol, 2.0f);
    // Highlight stripe
    dl->AddRectFilled(ImVec2(baseX + 2, healthY + 2),
                     ImVec2(baseX + 2 + (width - 4) * healthPct, healthY + 5),
    ...
    float stamH = 4.0f;
    dl->AddRectFilled(ImVec2(baseX, stamY), ImVec2(baseX + width, stamY + stamH),
                     IM_COL32(0, 0, 0, 100), 2.0f);
    dl->AddRectFilled(ImVec2(baseX + 1, stamY + 1),
                     ImVec2(baseX + 1 + (width - 2) * stamPct, stamY + stamH - 1),
...
void GameViewPanel::RenderAmmoDisplay(float baseX, float baseY) {
    ...
    if (m_isReloading) {
        ...
        dl->AddRectFilled(ImVec2(px + 4, reloadY), ImVec2(px + panelW - 4, reloadY + 4),
...
void GameViewPanel::RenderMinimap(float x, float y, float size) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (dist < 1.0f) {
            ...
            if (i == 3) {
                ...
                dl->AddCircle(ImVec2(bx, by), 5.0f + pulse * 3.0f,
                             IM_COL32(245, 200, 35, (int)(100 * pulse)), 12, 1.0f);
            }
        }
    }
    ...
    snprintf(bearing, sizeof(bearing), "%d" "\xc2\xb0", degrees);
...
void GameViewPanel::RenderKillFeed(float x, float y) {
    ...
    for (const auto& entry : m_killFeed) {
        ...
        if (entry.timer < 1.0f) alpha = entry.timer;

        char feedText[128];
        snprintf(feedText, sizeof(feedText), "%s %s%s %s",
                 entry.killer.c_str(),
                 entry.headshot ? ICON_FA_BULLSEYE " " : "",
                 entry.weapon.c_str(),
                 entry.victim.c_str());
...
void GameViewPanel::RenderHitMarker(float cx, float cy) {
    ...
    ImU32 col = m_hitMarkerHeadshot
        ? IM_COL32(229, 57, 53, (int)(255 * alpha))
    ...
    if (m_hitMarkerHeadshot && alpha > 0.5f) {
        ...
        dl->AddText(ImVec2(cx - hsSize.x * 0.5f, cy - size - 18),
                   IM_COL32(229, 57, 53, (int)(255 * alpha)), hsText);
    }
}
...
void GameViewPanel::RenderWeaponInfo(float x, float y) {
    ...
    dl->AddRectFilled(ImVec2(px, y), ImVec2(px + panelW, y + panelH),
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        ImU32 dotCol = (i == m_currentWeaponSlot)
            ? IM_COL32(45, 140, 240, 220)
...
void GameViewPanel::RenderObjectiveBar(float x, float y, float width) {
    ...
    dl->AddRectFilled(ImVec2(x, y), ImVec2(x + width, y + barH),
                     IM_COL32(0, 0, 0, 140), 4.0f);
    dl->AddRect(ImVec2(x, y), ImVec2(x + width, y + barH),
               IM_COL32(60, 70, 80, 150), 4.0f);

    float fillW = (width - 4) * Clamp01(m_objectiveProgress);
    dl->AddRectFilled(ImVec2(x + 2, y + 2), ImVec2(x + 2 + fillW, y + barH - 2),
                     IM_COL32(45, 140, 240, 160), 2.0f);

    char objText[64];
    snprintf(objText, sizeof(objText), ICON_FA_BULLSEYE " %s — %.0f%%",
             m_objectiveText.c_str(), m_objectiveProgress * 100.0f);
    ImVec2 textSize = ImGui::CalcTextSize(objText);
    dl->AddText(ImVec2(x + (width - textSize.x) * 0.5f, y + 3),
               IM_COL32(255, 255, 255, 220), objText);
}
...
void GameViewPanel::RenderScoreboard(float cx, float cy, float width, float height) {
    ...
    dl->AddRect(ImVec2(x, y), ImVec2(x + width, y + height),
               IM_COL32(45, 140, 240, 150), 6.0f, 0, 2.0f);
    ...
    dl->AddLine(ImVec2(x + 8, headerY + 16), ImVec2(x + width - 8, headerY + 16),
               IM_COL32(60, 70, 80, 150));
    ...
    for (size_t i = 0; i < m_scoreboardEntries.size() && rowY + rowH < y + height - 4; i++) {
        ...
        if (e.isLocal) {
            dl->AddRectFilled(ImVec2(x + 4, rowY - 2),
                             ImVec2(x + width - 4, rowY + rowH - 2),
                             IM_COL32(45, 140, 240, 40), 2.0f);
        }

        ImU32 textCol = e.isLocal ? IM_COL32(45, 180, 240, 255)
...
void GameViewPanel::RenderLowHealthVignette(float x, float y, float w, float h) {
    ...
    for (int i = 0; i < (int)edgeW; i++) {
        ...
        dl->AddLine(ImVec2(x + i, y), ImVec2(x + i, y + h),
                   IM_COL32(229, 57, 53, (int)(alpha * t * t)));
    }
    // Right edge
    for (int i = 0; i < (int)edgeW; i++) {
        float t = 1.0f - (float)i / edgeW;
        dl->AddLine(ImVec2(x + w - i, y), ImVec2(x + w - i, y + h),
                   IM_COL32(229, 57, 53, (int)(alpha * t * t)));
    }
    // Top edge
    float edgeH = h * 0.1f;
    for (int i = 0; i < (int)edgeH; i++) {
        ...
        dl->AddLine(ImVec2(x, y + i), ImVec2(x + w, y + i),
                   IM_COL32(229, 57, 53, (int)(alpha * t * t)));
    }
    // Bottom edge
    for (int i = 0; i < (int)edgeH; i++) {
        float t = 1.0f - (float)i / edgeH;
        dl->AddLine(ImVec2(x, y + h - i), ImVec2(x + w, y + h - i),
                   IM_COL32(229, 57, 53, (int)(alpha * t * t)));
    }
}
...
void GameViewPanel::RenderInteractionPrompt(float cx, float cy) {
    ...
    dl->AddRectFilled(ImVec2(bgX, bgY), ImVec2(bgX + bgW, bgY + bgH),
                     IM_COL32(0, 0, 0, (int)(140 * pulse)), 4.0f);
    dl->AddRect(ImVec2(bgX, bgY), ImVec2(bgX + bgW, bgY + bgH),
               IM_COL32(200, 200, 200, (int)(100 * pulse)), 4.0f);

    dl->AddText(ImVec2(cx - textSize.x * 0.5f, bgY + 6),
               IM_COL32(255, 255, 255, alpha), promptText);
}
