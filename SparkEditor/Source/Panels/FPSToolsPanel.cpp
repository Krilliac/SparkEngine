...
FPSToolsPanel::FPSToolsPanel()
    : EditorPanel("FPS Tools", "fps_tools_panel") {
}
...
void FPSToolsPanel::RenderSpawnPointEditor() {
    ...
    for (int i = 0; i < (int)m_spawnPoints.size(); i++) {
        ...
        const char* teamIcon = sp.teamId == 1 ? ICON_FA_SHIELD " " :
                               sp.teamId == 2 ? ICON_FA_FLAG " " : "";
        ImVec4 teamColor = sp.teamId == 1 ? ImVec4(0.2f, 0.5f, 0.9f, 1.0f) :
                           sp.teamId == 2 ? ImVec4(0.9f, 0.3f, 0.2f, 1.0f) :
                           ImVec4(0.8f, 0.8f, 0.8f, 1.0f);

        if (!sp.active) teamColor.w = 0.4f;
    ...
    if (m_selectedSpawn >= 0 && m_selectedSpawn < (int)m_spawnPoints.size()) {
        ...
        ImGui::DragFloat("Rotation Y", &sp.rotY, 1.0f, -180.0f, 180.0f, "%.1f" "\xc2\xb0");
        ...
        dl->AddRectFilled(mapPos, ImVec2(mapPos.x + mapSize.x, mapPos.y + mapSize.y),
                         IM_COL32(20, 25, 30, 255), 4.0f);
        dl->AddRect(mapPos, ImVec2(mapPos.x + mapSize.x, mapPos.y + mapSize.y),
        ...
        for (int i = 0; i < (int)m_spawnPoints.size(); i++) {
            ...
            if (sx < mapPos.x || sx > mapPos.x + mapSize.x ||
                sy < mapPos.y || sy > mapPos.y + mapSize.y) continue;
            ...
            dl->AddLine(ImVec2(sx, sy),
                       ImVec2(sx + sinf(rad) * arrowLen, sy - cosf(rad) * arrowLen),
                       col, 2.0f);
        }
...
void FPSToolsPanel::RenderGameModeConfig() {
    ...
    const char* gameModes[] = {
        "Free Play", "Deathmatch", "Team Deathmatch", "Capture the Flag",
        "Domination", "Elimination", "Gun Game", "Search and Destroy",
    ...
    ImGui::Text("Score Limit"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderInt("##ScoreLimit", &m_scoreLimit, 0, 500); ImGui::NextColumn();

    ImGui::Text("Time Limit (sec)"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##TimeLimit", &m_timeLimit, 0, 3600, "%.0f"); ImGui::NextColumn();

    ImGui::Text("Respawn Delay"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##RespawnDelay", &m_respawnDelay, 0, 30, "%.1f s"); ImGui::NextColumn();

    ImGui::Text("Teams Enabled"); ImGui::NextColumn();
    ImGui::Checkbox("##Teams", &m_teamsEnabled); ImGui::NextColumn();

    ImGui::Text("Friendly Fire"); ImGui::NextColumn();
    ImGui::Checkbox("##FriendlyFire", &m_friendlyFire); ImGui::NextColumn();
    ...
    ImGui::Text("Damage Multiplier"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##DamageMult", &m_damageMultiplier, 0.1f, 5.0f, "%.1fx"); ImGui::NextColumn();

    ImGui::Text("Health Multiplier"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##HealthMult", &m_healthMultiplier, 0.1f, 5.0f, "%.1fx"); ImGui::NextColumn();

    ImGui::Text("Speed Multiplier"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##SpeedMult", &m_speedMultiplier, 0.1f, 5.0f, "%.1fx"); ImGui::NextColumn();

    ImGui::Text("Headshots"); ImGui::NextColumn();
    ImGui::Checkbox("##Headshots", &m_headshots); ImGui::NextColumn();

    if (m_headshots) {
        ImGui::Text("Headshot Multiplier"); ImGui::NextColumn();
        ImGui::SetNextItemWidth(-1);
        ImGui::SliderFloat("##HSMult", &m_headshotMultiplier, 1.0f, 10.0f, "%.1fx"); ImGui::NextColumn();
    }
...
void FPSToolsPanel::RenderPlayerConfig() {
    ...
    ImGui::Text("Max Health"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##MaxHealth", &m_playerHealth, 1.0f, 1000.0f, "%.0f"); ImGui::NextColumn();

    ImGui::Text("Max Armor"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##MaxArmor", &m_playerArmor, 0.0f, 500.0f, "%.0f"); ImGui::NextColumn();
    ...
    ImGui::Text("Move Speed"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##MoveSpeed", &m_playerSpeed, 1.0f, 30.0f, "%.1f"); ImGui::NextColumn();

    ImGui::Text("Jump Height"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##JumpHeight", &m_playerJumpHeight, 0.5f, 20.0f, "%.1f"); ImGui::NextColumn();

    ImGui::Text("Gravity"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##Gravity", &m_playerGravity, 1.0f, 50.0f, "%.1f"); ImGui::NextColumn();

    ImGui::Text("Friction"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##Friction", &m_playerFriction, 0.0f, 1.0f, "%.2f"); ImGui::NextColumn();
    ...
    ImGui::Text("Mouse Sensitivity"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##Sensitivity", &m_mouseSensitivity, 0.1f, 10.0f, "%.1f"); ImGui::NextColumn();

    ImGui::Text("Field of View"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##FOV", &m_fov, 60.0f, 120.0f, "%.0f" "\xc2\xb0"); ImGui::NextColumn();
...
void FPSToolsPanel::RenderCombatTesting() {
    ...
    ImGui::Text("Incoming Damage"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##TestDmg", &m_testDamage, 1.0f, 500.0f, "%.1f"); ImGui::NextColumn();

    ImGui::Text("Target Health"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##TestHP", &m_testHealth, 1.0f, 1000.0f, "%.0f"); ImGui::NextColumn();

    ImGui::Text("Target Armor"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##TestArm", &m_testArmor, 0.0f, 500.0f, "%.0f"); ImGui::NextColumn();

    ImGui::Text("Distance (units)"); ImGui::NextColumn();
    ImGui::SetNextItemWidth(-1);
    ImGui::SliderFloat("##TestDist", &m_testDistance, 1.0f, 200.0f, "%.1f"); ImGui::NextColumn();
    ...
    ImGui::TextColored(remainHP > 0 ? ImVec4(0.3f, 0.8f, 0.3f, 1) : ImVec4(0.9f, 0.2f, 0.2f, 1),
                      "%.1f", remainHP);
    ImGui::NextColumn();
    ...
    if (totalBar > 0) {
        ...
        dl->AddRectFilled(barPos, ImVec2(barPos.x + barW, barPos.y + barH),
                         IM_COL32(30, 35, 40, 255), 3.0f);

        // Armor portion
        float armorW = (m_testArmor / totalBar) * barW;
        dl->AddRectFilled(ImVec2(barPos.x, barPos.y),
                         ImVec2(barPos.x + armorW, barPos.y + barH),
                         IM_COL32(45, 140, 240, 180), 3.0f);

        // Health portion
        dl->AddRectFilled(ImVec2(barPos.x + armorW, barPos.y),
        ...
        dl->AddLine(ImVec2(dmgX, barPos.y), ImVec2(dmgX, barPos.y + barH),
                   IM_COL32(229, 57, 53, 255), 2.0f);

        // Labels
        dl->AddText(ImVec2(barPos.x + 4, barPos.y + 4),
                   IM_COL32(255, 255, 255, 220), ICON_FA_SHIELD " Armor");
        dl->AddText(ImVec2(barPos.x + armorW + 4, barPos.y + 4),
                   IM_COL32(255, 255, 255, 220), ICON_FA_HEART " Health");
