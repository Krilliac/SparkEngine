...
namespace SparkEditor
{
    ...
    SimpleHierarchyPanel::SimpleHierarchyPanel() : EditorPanel("Hierarchy", "simple_hierarchy_panel") {}
    ...
    void SimpleHierarchyPanel::Render()
    {
        ...
        if (BeginPanel())
        {
            ...
            ImGui::InputTextWithHint("##HierarchySearch", ICON_FA_SEARCH " Search objects...", m_searchFilter,
            ...
            for (size_t i = 0; i < m_sceneObjects.size(); ++i)
            {
                ...
                if (object.find("Camera") != std::string::npos)
                    icon = ICON_FA_CAMERA;
                else if (object.find("Light") != std::string::npos)
                    icon = ICON_FA_LIGHTBULB;
                else if (object.find("Player") != std::string::npos)
                    icon = ICON_FA_CROSSHAIRS;
                else if (object.find("Plane") != std::string::npos || object.find("Ground") != std::string::npos)
                    icon = ICON_FA_VECTOR_SQUARE;
                else if (object.find("Sphere") != std::string::npos)
                    icon = ICON_FA_CIRCLE;
            ...
            if (ImGui::BeginPopup("CreateObject"))
            {
                ...
                if (ImGui::BeginMenu(ICON_FA_SHAPES " 3D Object"))
                {
                    ...
                    if (ImGui::MenuItem(ICON_FA_CIRCLE " Sphere"))
                        CreateObject("Sphere");
                    if (ImGui::MenuItem(ICON_FA_VECTOR_SQUARE " Plane"))
                        CreateObject("Plane");
                    ImGui::EndMenu();
                }
                ...
                {
                    if (ImGui::MenuItem(ICON_FA_CROSSHAIRS " Player Spawn"))
                        CreateObject("Player Spawn");
                    if (ImGui::MenuItem(ICON_FA_FLAG " Capture Point"))
                        CreateObject("Capture Point");
                    if (ImGui::MenuItem(ICON_FA_SHIELD " Cover Point"))
