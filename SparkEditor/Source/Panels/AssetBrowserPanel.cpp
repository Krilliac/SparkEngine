...
static const char* GetFileTypeIcon(const std::string& ext) {
    if (ext == ".fbx" || ext == ".obj" || ext == ".gltf" || ext == ".glb") return ICON_FA_CUBE;
    if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".tga" || ext == ".bmp" || ext == ".dds") return ICON_FA_IMAGE;
    if (ext == ".wav" || ext == ".mp3" || ext == ".ogg") return ICON_FA_VOLUME_UP;
    if (ext == ".cpp" || ext == ".h" || ext == ".hpp" || ext == ".lua") return ICON_FA_CODE;
    if (ext == ".hlsl" || ext == ".glsl" || ext == ".shader") return ICON_FA_PAINT_BRUSH;
    if (ext == ".mat" || ext == ".material") return ICON_FA_CIRCLE;
    if (ext == ".scene" || ext == ".map") return ICON_FA_MAP;
    if (ext == ".prefab") return ICON_FA_SHAPES;
    if (ext == ".ttf" || ext == ".otf") return ICON_FA_FONT;
    if (ext == ".json" || ext == ".xml" || ext == ".ini" || ext == ".cfg") return ICON_FA_COG;
    return ICON_FA_FILE;
}

static ImU32 GetFileTypeColor(const std::string& ext) {
    if (ext == ".fbx" || ext == ".obj" || ext == ".gltf" || ext == ".glb") return IM_COL32(100, 200, 255, 255);
    if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".tga") return IM_COL32(200, 150, 255, 255);
    if (ext == ".wav" || ext == ".mp3" || ext == ".ogg") return IM_COL32(255, 200, 100, 255);
    if (ext == ".cpp" || ext == ".h" || ext == ".hpp" || ext == ".lua") return IM_COL32(100, 255, 150, 255);
    if (ext == ".hlsl" || ext == ".glsl" || ext == ".shader") return IM_COL32(255, 150, 150, 255);
            ...
            if (!m_projectPath.empty() && std::filesystem::exists(m_projectPath)) {
                for (const auto& entry : std::filesystem::directory_iterator(m_projectPath)) {
                    if (entry.is_directory()) {
                        ...
                        std::string nodeLabel = std::string(isCurrentFolder ? ICON_FA_FOLDER_OPEN : ICON_FA_FOLDER) + "  " + folderName;

                        ImGuiTreeNodeFlags nodeFlags = ImGuiTreeNodeFlags_OpenOnArrow;
                        if (isCurrentFolder) nodeFlags |= ImGuiTreeNodeFlags_Selected;
    ...
    if (ImGui::BeginTable("AssetGridTable", columns)) {
        ...
        for (const auto& asset : m_assets) {
            ...
            ImVec2 iconRenderPos(
                pos.x + (size.x - iconTextSize.x) * 0.5f,
            ...
            if (ImGui::IsItemHovered()) {
                ...
                ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "Type: %s", ext.empty() ? "Unknown" : ext.c_str());
                try {
