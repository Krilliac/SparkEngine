...
bool VersionControlSystem::Initialize() {
    ...
    m_lfsPatterns = { "*.png", "*.jpg", "*.jpeg", "*.tga", "*.bmp", "*.psd",
                      "*.fbx", "*.obj", "*.blend", "*.dae",
                      "*.wav", "*.mp3", "*.ogg",
                      "*.mp4", "*.avi", "*.mov",
                      "*.sparkscene", "*.sparkmat", "*.sparkshader" };
...
VCSOperationResult VersionControlSystem::StageFiles(const std::vector<std::string>& filePaths) {
    if (!m_repositoryInfo) return { false, "No repository open", "", -1, 0.0f, {} };
...
std::vector<CommitInfo> VersionControlSystem::GetCommitHistory(int maxCommits, const std::string& branchName) {
    ...
    std::string cmd = "git log --format='COMMIT_SEP%nHash:%H%nShortHash:%h%nAuthor:%an%nEmail:%ae%nDate:%aI%nMessage:%s'";
...
bool VersionControlSystem::ResolveMergeConflict(MergeConflict& conflict, const std::string& resolution) {
    if (!m_repositoryInfo) return false;
...
bool VersionControlSystem::AddIgnorePattern(const std::string& pattern) {
    ...
    while (std::getline(lineStream, line)) {
        if (line == pattern) return true; // Already present
    }
...
bool VersionControlSystem::RemoveIgnorePattern(const std::string& pattern) {
    ...
    std::filesystem::path gitignorePath = std::filesystem::path(m_repositoryInfo->path) / ".gitignore";
    if (!std::filesystem::exists(gitignorePath)) return false;
...
std::vector<std::string> VersionControlSystem::GetIgnorePatterns() const {
    ...
    if (!m_repositoryInfo) return patterns;

    std::filesystem::path gitignorePath = std::filesystem::path(m_repositoryInfo->path) / ".gitignore";
    if (!std::filesystem::exists(gitignorePath)) return patterns;
...
void VersionControlSystem::RenderChangesPanel() {
    ...
    for (auto& change : m_repositoryInfo->changedFiles) {
        ...
        switch (change.status) {
            case FileStatus::MODIFIED:   statusLabel = "[M]"; statusColor = ImVec4(1.0f, 0.8f, 0.0f, 1.0f); break;
            case FileStatus::ADDED:      statusLabel = "[A]"; statusColor = ImVec4(0.0f, 1.0f, 0.0f, 1.0f); break;
            case FileStatus::DELETED:    statusLabel = "[D]"; statusColor = ImVec4(1.0f, 0.0f, 0.0f, 1.0f); break;
            case FileStatus::RENAMED:    statusLabel = "[R]"; statusColor = ImVec4(0.5f, 0.5f, 1.0f, 1.0f); break;
            case FileStatus::UNTRACKED:  statusLabel = "[?]"; statusColor = ImVec4(0.5f, 0.5f, 0.5f, 1.0f); break;
            case FileStatus::CONFLICTED: statusLabel = "[C]"; statusColor = ImVec4(1.0f, 0.0f, 0.0f, 1.0f); break;
    ...
    if (ImGui::Button("Stage Selected")) {
        ...
        for (auto& [path, sel] : selectedFiles) {
            if (sel) toStage.push_back(path);
        }
        if (!toStage.empty()) {
            StageFiles(toStage);
            for (auto& [path, sel] : selectedFiles) sel = false;
        }
    }
    ImGui::SameLine();
    if (ImGui::Button("Unstage Selected")) {
        ...
        for (auto& [path, sel] : selectedFiles) {
            if (sel) toUnstage.push_back(path);
        }
        if (!toUnstage.empty()) {
...
void VersionControlSystem::RenderConflictsPanel() {
    ...
    ImGui::TextColored(ImVec4(1.0f, 0.3f, 0.3f, 1.0f), "%zu conflict(s) detected:",
                       m_repositoryInfo->conflicts.size());
    ImGui::Separator();
...
std::vector<FileChange> VersionControlSystem::ParseGitStatus(const std::string& output) {
    ...
    while (std::getline(stream, line)) {
        if (line.size() < 4) continue; // Minimum: "XY filename"
...
void VersionControlSystem::AutoSync() {
    if (!m_repositoryInfo) return;
    if (m_collaborationSettings.autoSyncInterval <= 0.0f) return;
