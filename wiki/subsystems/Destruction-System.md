...
| `meshName` | `std::string` | `""` | Asset name for the debris mesh (loaded via [Asset Pipeline](../gameplay-tools/Asset-Pipeline.md)) |
...
| `SetDestructionSound(name)` | Set the [audio](Audio.md) asset played on destruction |
...
The `DestructionSystem` is **not thread-safe**. All methods must be called from the main game thread. The system participates in the [ECS execution order](Entity-Component-System.md): Physics -> Animation -> AI -> Audio -> Lifecycle -> Render. Destruction processing happens during the Lifecycle phase.
...
| Debris floats in air | Physics not applied to spawned debris | Ensure [Physics](Physics.md) system processes debris rigid bodies |
| Too many debris, FPS drops | `m_maxDebris` too high for hardware | Reduce via `SetMaxDebris()` or increase lifetime multiplier |
| Destruction sound not playing | Sound asset not loaded | Check [Audio](Audio.md) system; verify sound asset name |
...
- [Physics](Physics.md) -- Debris uses rigid body simulation
- [Entity Component System](Entity-Component-System.md) -- DestructibleComponent
- [Gameplay Systems](../gameplay-tools/Gameplay-Systems.md) -- Weapons dealing damage to destructibles
- [Audio](Audio.md) -- Destruction sound effects
- [Rendering and Graphics](Rendering-and-Graphics.md) -- Damage stage visual switching
- [Event System](Event-System.md) -- Publishing destruction events
- [Asset Pipeline](../gameplay-tools/Asset-Pipeline.md) -- Loading debris mesh assets
