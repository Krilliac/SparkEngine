...
This page documents the internal data formats used by SparkEngine's [asset pipeline](../gameplay-tools/Asset-Pipeline.md) for meshes, textures, materials, audio, and animations.
...
SparkPak is an MPQ-inspired binary archive format for bundling assets into single files. Archives are optional — the engine loads loose files by default and mounts .spk archives on top via the [VirtualFileSystem](../subsystems/Mod-System.md).
...
See [Asset Pipeline](../gameplay-tools/Asset-Pipeline.md) for the full pipeline architecture.
