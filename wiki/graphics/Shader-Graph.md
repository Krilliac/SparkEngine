...
The shader graph system bridges the gap between visual material editing and GPU shader code. The editor (see [SparkEditor](../gameplay-tools/SparkEditor.md)) provides a node canvas where users connect input, math, and utility nodes to output nodes. The `ShaderGraphCompiler` traverses this graph and emits valid HLSL.
...
- **SparkEditor**: The material editor panel creates `ShaderGraphInput` from the visual canvas and calls `Compile()`. See [SparkEditor](../gameplay-tools/SparkEditor.md)
- **SparkShaderCompiler**: The standalone tool can also compile shader graphs from serialized JSON. See [Shader Pipeline](../gameplay-tools/Shader-Pipeline.md)
- **Material System**: Generated HLSL is fed to the shader compilation pipeline and cached. See [Rendering and Graphics](../subsystems/Rendering-and-Graphics.md)
...
- [Visual Scripting](../subsystems/Visual-Scripting.md) — General visual scripting system (references shader graph as prior art)
- [Shader Pipeline](../gameplay-tools/Shader-Pipeline.md) — HLSL compilation, cross-compilation, and hot-reload
- [Rendering and Graphics](../subsystems/Rendering-and-Graphics.md) — Material system and PBR pipeline
- [Asset Pipeline](../gameplay-tools/Asset-Pipeline.md) — Texture and material asset management
