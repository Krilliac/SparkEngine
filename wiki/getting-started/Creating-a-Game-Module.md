...
| `tests` | Every file exists under `Tests/`, is registered in `Tests/CMakeLists.txt`, and defines at least one `TEST(` matching a declared prefix; every prefix matches at least one `TEST(` in the listed files; each `count` is a positive integer equal to the number of `TEST(` definitions, across every source registered in `Tests/CMakeLists.txt`, whose name starts with the prefix (the anchored rule `SPARK_TEST_NAME_PREFIX` applies, so `RPG_` does not count `ARPG_*` tests). When a family has `TEST(` definitions inside an `#ifdef _WIN32` / `SPARK_PLATFORM_WINDOWS` branch (or its `#ifndef`/`#else` complement), `count` is `{"windows": N, "other": M}` with different values; other preprocessor conditions are assumed true |
