...
Five split waves reduced the over-threshold file count from **104 to 19** (85 files split into coherent sibling TUs plus small internal headers, all moves byte-identical and build+test verified). The remaining 19: 18 are Windows-only (`*Windows.cpp`, `ProcessWin32.cpp`) that cannot be compile-verified in a Linux container — split them only from a session with a working MSVC/MinGW build — and `GameModules/SparkGameMMOFPS/Source/Core/TFTypes.h` (301 lines, a FROZEN CONTRACT header deliberately left alone). Gotcha discovered: `Tests/CMakeLists.txt` links some module `.cpp` files by explicit path (e.g. `TFOutfitStore.cpp`), so splitting one of those requires adding the new sibling TU there too — the module's own recursive glob is not enough.
