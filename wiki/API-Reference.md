...
| Page | Contents |
|------|----------|
| [Symbol Index](Symbol-Index) | Every class, struct, enum, function, method, macro, and type alias — alphabetical, with file-and-line source links. |
| [Function Index](Function-Index) | Every free function and out-of-line method definition (headers + `.cpp`). |
| [Class & Struct Index](Class-Index) | Every declared class and struct in the codebase. |
| [Enum Index](Enum-Index) | Every enum with its source location. |
| [Macro & Alias Index](Macro-Index) | Every `#define` (excluding header guards) plus `typedef`/`using` aliases. |
| [File Tree](File-Tree) | Hierarchical listing of every source file with LOC and `@brief`. Includes a Mermaid module-dependency graph. |
| [Class Hierarchy](Class-Hierarchy) | Mermaid `classDiagram` per top-level module showing `public` inheritance. |

## Quick-reference indexes (scoped)

| Page | Contents |
|------|----------|
| [Entity Component System](Entity-Component-System) | ECS components and systems overview. |
| [Architecture Overview](Architecture-Overview) | High-level engine architecture. |
| [Codebase Statistics](Codebase-Statistics) | LOC, file counts, subsystem metrics. |
| [Engine Architecture Flowchart](Engine-Architecture-Flowchart) | ASCII/Mermaid flowcharts of subsystems. |
