...
Transport controls for the editor's play-state preview. This is **not** an
in-editor game session: `PlayModeManager::Update` advances local time and
statistics counters, but the editor does not tick the game's ECS, physics,
AI, or scripting systems through this control. The subsystem toggles affect
those counters, not live gameplay. The authored scene remains a preview.
...
- Preview, Pause, Stop, and Step-Frame buttons
...
- F5 starts only the editor state/HUD preview. Use Play Control > Launch Game for a real game process.
