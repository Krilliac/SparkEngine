...
namespace Spark::Gateway
{
    ...
    bool GatewayApplication::Start()
    {
        ...
        if (!m_ingress->Start())
        {
            ...
            // The ingress holds a GatewayCoordinator reference, so it must be
            // destroyed before the coordinator it borrows — otherwise the
            // failed-start object outlives its referent until ~GatewayApplication.
            m_ingress->Stop();
            m_ingress.reset();
            m_coordinator.reset();
            m_worldServer->Stop();
