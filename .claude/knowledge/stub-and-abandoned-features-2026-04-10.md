...
| `AIIntegratedSystem` + `ParallelPerceptionSystem` | `SparkEngine/Source/Engine/AI/AIIntegration.h` (227 lines, header-only) plus `Engine/AI/ParallelPerception.h/.cpp` (~250 lines). Documented in `wiki/AI-and-Navigation.md` as "the top-level AI ECS system" combining the heavy `AISystem` with `ParallelPerceptionSystem`, `AIBudgetLimiter`, and `NavMeshObstacleManager`. | None for `AIIntegratedSystem`; none for `ParallelPerceptionSystem`. (`AIBudgetLimiter` has `Tests/TestAIBudgetLimiter.cpp`; `NavMeshObstacleManager` is intentionally a passive registry per the existing lifecycle comment.) | **NONE — discovered 2026-04-14 audit.** `AIIntegratedSystem` is never instantiated anywhere; only references are inside its own header and the wiki. The currently-running AI path is the lighter `Spark::ECS::AIUpdateSystem` (registered in `EngineSetup.h:194` and ticking behavior trees per agent in `ECSystems.cpp:340`). The "heavy" `Spark::AI::AISystem` (parallel perception + behavior + movement pipeline in `AISystem.cpp`) is also never called, only reachable via `AIIntegratedSystem`. **Decision pending:** wiring it would need to *replace* `AIUpdateSystem` per the comment in `AIIntegration.h:84-85` ("Can be used as an ECS ISystem, replacing AIUpdateSystem in the PhaseSystemManager") to avoid double-ticking behavior trees, which is a behaviour-affecting refactor — needs explicit owner sign-off before action. Until then this is the largest known unwired island in the engine. |
