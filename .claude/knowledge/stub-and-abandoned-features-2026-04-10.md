...
| `AIIntegratedSystem` + `ParallelPerceptionSystem` | `SparkEngine/Source/Engine/AI/AIIntegration.h` (~270 lines, header-only) plus `Engine/AI/ParallelPerception.h/.cpp` (~250 lines). Documented in `wiki/AI-and-Navigation.md` as "the top-level AI ECS system" combining the heavy `AISystem` with `ParallelPerceptionSystem`, `AIBudgetLimiter`, and `NavMeshObstacleManager`. | `Tests/TestAIIntegratedSystem.cpp` (new, 11 real-class tests). `AIBudgetLimiter` already had `Tests/TestAIBudgetLimiter.cpp`. | **RESOLVED 2026-04-14 (later session).** Resolved the double-tick concern by adding an `AIIntegrationConfig::runCoreAISystem` flag (default **false**) that gates the inner `Spark::AI::AISystem::Update()` call inside `AIIntegratedSystem::Update()`. With the flag off, `Update()` only runs the additive subsystems (parallel perception spatial index rebuild + agent update, NavMesh obstacle ticking) and does **not** tick behavior trees, so it can coexist with the existing `Spark::ECS::AIUpdateSystem` registered in `EngineSetup.h`. Game modules that want the full heavy pipeline call `SetRunCoreAISystem(true)` after disabling `AIUpdateSystem`. Wire-in: added `GetInstance()` singleton accessor, then in `GameplayLifecycleShared.cpp` (a) include `Engine/AI/AIIntegration.h`, (b) `Initialize()` alongside the other AI singletons in `InitAIAndWorldSystems`, (c) `Update(*world, dt)` inside a `SPARK_GUARDED_UPDATE("AIIntegrated", ...)` block in `UpdateECSDependentSystems`, (d) `Shutdown()` in `ShutdownAIAndWorldSystems`. New `Tests/TestAIIntegratedSystem.cpp` covers singleton liveness, init/shutdown lifecycle, default-config no-core-tick guarantee, `SetRunCoreAISystem` toggle, subsystem accessors with default + opt-out config, `Update` no-op before init, `Update` on empty world (default config), `Console_GetStatus` before/after init, and re-init after shutdown — 11 tests against the real header class. `@warning` headers on both `AIIntegration.h` and `ParallelPerception.h` were rewritten as `@note WIRED` blocks. |
