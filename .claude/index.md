...
- **Tests**: 361 test files, 4684 tests on SparkTests (Phases U–X add 73 new tests: 9 ShaderHotReload wire-up + 16 ShaderDiskCache wire-up + 19 ShaderCrossCompiler wire-up + 15 HandlePool real-class + 14 TransientBufferAllocator real-class); all pass on native Linux; total 119041+ assertions. Theme 3A (shader hot-reload surface) **complete**; Theme 3B (RHI backend parity) Phase X landed real-class test coverage for the two header-only utility orphans flagged `@note intentionally demand-driven`.
