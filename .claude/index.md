...
- **Tests**: 364 test files, 4704 tests on SparkTests on Linux (ImGui-gated Phase AA tests inactive without libimgui; +22 tests will activate when a Linux-ImGui CI job is added). Phase U–AA cumulative: 117 new tests (9 ShaderHotReload + 16 ShaderDiskCache + 19 ShaderCrossCompiler + 15 HandlePool real-class + 14 TransientBufferAllocator real-class + 20 NullRHIDevice Phase Y + 2 updated headless + 11 LevelStreamingSystem + 11 VersionControlSystem). Theme 3A **complete**; Theme 3B **complete for compilable-on-Linux scope**; Theme 3C **2 of 4 orphans wired** (LevelStreamingSystem + VersionControlSystem; AnimationTimeline / AdvancedAssetPipeline / MaterialEditor deferred for a future session with a Linux-ImGui CI).
