...
- **Tests**: 362 test files, 4704 tests on SparkTests (Phases U–Z add 95 new tests: 9 ShaderHotReload + 16 ShaderDiskCache + 19 ShaderCrossCompiler + 15 HandlePool real-class + 14 TransientBufferAllocator real-class + 20 NullRHIDevice Phase Y wire-up + 2 updated headless resource tests); all pass on native Linux; total 119094+ assertions. Theme 3A (shader hot-reload surface) **complete**; Theme 3B (RHI backend parity) **complete for the compilable-on-Linux scope**: NullRHIDevice fully wired + real backends (D3D11/D3D12/Vulkan/OpenGL) have TransientBufferAllocator integrated behind their platform guards (Windows CI / gated-backend verification).
