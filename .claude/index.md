...
- **Tests**: 370 test files, 4770 tests on SparkTests on Linux (Phase U–DD add 183 cumulative new tests; ImGui-gated Phase AA tests inactive without libimgui). Theme 3A **complete**; Theme 3B **complete for compilable-on-Linux scope**; Theme 3C **2 of 4 outside-Panels/ orphans wired**; Theme 3D ongoing — Phase BB (ScriptHookManager + DynamicQualityScaler, 25 tests), Phase CC (GPUStallProfiler + AsyncComputeScheduler, 22 tests), Phase DD (AIDebugRenderer singleton wired + DirtyRegionGrid real-class tests, 19 tests) — 8 orphans wired into `GameplayLifecycleShared.cpp` with touch-based lifecycles.
