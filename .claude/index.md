...
- **Tests**: 381 test files, 4867 tests on SparkTests on Linux (Phase U–II add 262 cumulative new tests; ImGui-gated Phase AA tests inactive without libimgui). Theme 3A **complete**; Theme 3B **complete for compilable-on-Linux scope**; Theme 3C **2 of 4 outside-Panels/ orphans wired**; Theme 3D **28 orphans wired across Phases BB/CC/DD/EE/FF/GG/HH/II** — deep-wire session used parallel sweep agents + multiple heuristics to find ScriptHookManager, DynamicQualityScaler, GPUStallProfiler, AsyncComputeScheduler, AIDebugRenderer, DirtyRegionGrid, EventResponseSystem, EntityPresetManager, AssetMigrationRegistry, TelemetrySystem, CacheDebugger, MathUtilsExtended, CpuDebugger, LODGenerator, TextureCompressor, DatablockRegistry, NetworkSecurity, AIDirector, HLODBuilder.
