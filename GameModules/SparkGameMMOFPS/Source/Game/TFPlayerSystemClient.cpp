 ...
 *        in TFPlayerSystem.cpp and TFPlayerSystemSpawn.cpp (same class, split
 *        per repo file-size rules).
 */
...
namespace Terrafront
{
    ...
    void TFPlayerSystem::RenderDebugUI()
    {
        ...
        if (!ImGui::CollapsingHeader("TF Players"))
            return;
        ImGui::Text("records: %zu  (pawn map %zu, pending local %u)", m_players.size(), m_pawnOwner.size(),
                    m_pendingLocalPawn);
        for (const auto& [player, rec] : m_players)
        {
            PawnInfo p{};
            ...
            {
                ImGui::Text("p%u %s %s pawn=%u pos=(%.1f %.1f %.1f) hp=%.0f+%.0f", player, FactionTag(p.faction),
                            p.alive ? "alive" : "dead", p.entity, p.pos[0], p.pos[1], p.pos[2], p.health, p.shield);
            }
            else
            {
