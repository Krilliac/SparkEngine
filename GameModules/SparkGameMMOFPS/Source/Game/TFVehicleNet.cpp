 ...
 *        visuals. Server game logic lives in TFVehicleSystem.cpp, client UX
...
void TFVehicleSystem::NetFixedUpdate(float fdt)
{
    ...
    else
    {
        ...
        if (!m_knownClients.empty() || !m_lastSent.empty())
        {
...
void TFVehicleSystem::ServerEnsureNetHandlers()
{
    ...
    nm.RegisterHandler(static_cast<MessageType>(kTFVehMsg_Purchase),
...
void TFVehicleSystem::ServerBroadcastUpdates()
{
    ...
    std::memcpy(payload.data() + sizeof(hdr), records.data(),
                records.size() * sizeof(TF_RepVehicleUpdate));
...
void TFVehicleSystem::ServerSendCreate(PlayerId target, const VehicleRec& v)
{
    ...
    c.posX = v.pos[0]; c.posY = v.pos[1]; c.posZ = v.pos[2];
    c.yaw = v.yaw;
    c.health = v.hp;
    c.maxHealth = v.maxHp;
...
void TFVehicleSystem::SendNet(PlayerId target, uint16_t msgId, const void* payload,
...
{
    auto& nm = Spark::Net::NetworkManager::GetInstance();
    for (uint16_t id : {kTFRepMsg_VehCreate, kTFRepMsg_VehUpdate, kTFRepMsg_VehDestroy,
                        kTFRepMsg_VehSeats})
    {
        nm.RegisterHandler(static_cast<Spark::Net::MessageType>(id),
                           [](const Spark::Net::NetworkMessage&) {});
    }
...
void TFVehicleSystem::OnNetVehCreate(const void* data, size_t size)
{
    ...
    m.info.pos[0] = c.posX; m.info.pos[1] = c.posY; m.info.pos[2] = c.posZ;
    m.info.yaw = c.yaw;
    m.info.hp = c.health;
    ...
    m.targetPos[0] = c.posX; m.targetPos[1] = c.posY; m.targetPos[2] = c.posZ;
    m.targetYaw = c.yaw;
    m.hasTarget = true;
    ...
    for (auto& [entity, m] : m_mirror)
    {
        ...
        if (world && m.local != 0)
        {
            ...
            if (world->GetRegistry().valid(e))
            {
                ...
                {
                    t->position = {m.info.pos[0], m.info.pos[1], m.info.pos[2]};
                    t->rotation = {m.targetPitch * kRadToDeg, m.info.yaw * kRadToDeg,
                                   m.targetRoll * kRadToDeg};
                }
            }
        }
    }
