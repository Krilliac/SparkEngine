 ...
 * @brief W3 vehicles — server replication half of TFVehicleSystem on the
 *        0x54F8+ block (TFRepProtocol.h): the fixed-tick net driver for both
 *        roles, server create/update/destroy/seats broadcasting with dirty
 *        checks + late-join bursts, the C->S purchase handler, and the W8
 *        turret-aim channel (0x5448, TFVehicleSystem.h) 10 Hz-on-change /
 *        1 Hz-keepalive S->C send. The pure-client mirror store with
 *        interpolated OBJ visuals lives in TFVehicleNetMirror.cpp, server
 *        game logic in TFVehicleSystem.cpp, client UX in TFVehicleClient.cpp
 *        (same class, split per repo file-size rules); shared internals live
 *        in TFVehicleSystemInternal.h.
 */
