...
bool TFRegionSystem::LoadPersisted()
{
    ...
    if (!f.is_open())
    {
        SPARK_LOG_INFO(Spark::LogCategory::Game,
                       "[TF] no territory save at %s — seeding from regions.json",
                       path.c_str());
        return false;
    }
    ...
    if (!root.IsObject())
    {
        SPARK_LOG_WARN(Spark::LogCategory::Game,
                       "[TF] territory save %s is malformed — ignored", path.c_str());
        return false;
    }

    const size_t count = m_state.size();
    const Value& owners = root["owners"];
    if (static_cast<size_t>(root["regionCount"].AsInt(-1)) != count ||
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        FactionId owner = (raw >= 0 && raw < static_cast<int>(FactionId::COUNT))
                              ? static_cast<FactionId>(raw) : FactionId::None;
    ...
    m_dirty = false;
    SPARK_LOG_INFO(Spark::LogCategory::Game,
                   "[TF] territory restored from %s (hash 0x%08X%s)",
                   path.c_str(), TerritoryHash(), m_domActive ? ", dominion hold" : "");
    return true;
}
...
bool TFRegionSystem::PersistNow()
{
    ...
    root["continent"] = Value(m_ctx->data && m_ctx->data->IsLoaded()
                                  ? m_ctx->data->GetContinent().name : std::string());
...

    {
        ...
        {
            SPARK_LOG_WARN(Spark::LogCategory::Game,
                           "[TF] territory save failed: cannot write %s", tmp.c_str());
            return false;
        }
        ...
        {
            SPARK_LOG_WARN(Spark::LogCategory::Game,
                           "[TF] territory save failed: short write to %s", tmp.c_str());
            return false;
        }
    }
    ...
    {
        SPARK_LOG_WARN(Spark::LogCategory::Game,
                       "[TF] territory save failed: rename to %s (%s)",
...
void TFRegionSystem::ServerPollNewClients()
{
    ...
    for (const auto& [id, info] : clients)
    {
        ...
        SendFullStateTo(id);
        SPARK_LOG_INFO(Spark::LogCategory::Game,
                       "[TF] regions: client %u joined — %zu region states sent",
                       id, m_state.size());
    }
...
void TFRegionSystem::SendCaptureTick(size_t idx)
{
    ...
    SendNet(kInvalidPlayer, static_cast<uint16_t>(TFMsg::CaptureTick),
            &msg, sizeof(msg), /*reliable*/ false);
    ++m_ticksSent;
}

void TFRegionSystem::SendNet(PlayerId target, uint16_t msgId, const void* payload,
...
void TFRegionSystem::EnsureClientHandlers()
{
    ...
    nm.RegisterHandler(
        static_cast<MessageType>(static_cast<uint16_t>(TFMsg::RegionState)),
        [this](const NetworkMessage& m) { OnNetRegionState(m.payload.data(), m.payload.size()); });
    nm.RegisterHandler(
        static_cast<MessageType>(static_cast<uint16_t>(TFMsg::CaptureTick)),
