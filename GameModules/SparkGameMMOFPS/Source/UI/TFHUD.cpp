...
void TFHUD::RenderUI()
{
    ...
    if (!m_chatOpen && m_ctx->InWorld() && !ImGui::GetIO().WantTextInput &&
...
void TFHUD::DrawChat()
{
    ...
    const float controlsH = m_chatOpen
        ? ImGui::GetFrameHeight() * (compact ? 2.0f : 1.0f) +
    ...
    const float desiredHeight = lineH * static_cast<float>(std::max<size_t>(visible.size(), 1)) +
                                controlsH + ImGui::GetStyle().WindowPadding.y * 2.0f;
    const float height = std::min(maxHeight, std::max(52.0f, desiredHeight));
    ImGui::SetNextWindowPos(ImVec2(vp->Pos.x + margin,
                                   vp->Pos.y + vp->Size.y - height - margin));
    ...
    ImGuiWindowFlags chatFlags =
        ...
        ImGuiWindowFlags_NoBringToFrontOnFocus;
    if (!m_chatOpen)
        chatFlags |= ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoNav |
    ...
    for (auto it = visible.rbegin(); it != visible.rend(); ++it)
    {
        ...
        char prefix[48]{};
        std::snprintf(prefix, sizeof(prefix), "[%s] p%u: ",
                      ChatChannelName(line.channel), line.from);
        const ImU32 channelColor = line.channel == ChatChannel::Squad
            ? IM_COL32(110, 225, 150, 255)
            : line.channel == ChatChannel::Faction
                ? FactionCol(m_ctx->localFaction)
                : line.channel == ChatChannel::Yell
    ...
    const bool submitted = ImGui::InputText("##TFChatInput", m_chatInput,
        sizeof(m_chatInput), ImGuiInputTextFlags_EnterReturnsTrue);
