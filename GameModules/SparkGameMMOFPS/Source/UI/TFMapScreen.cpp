...
void TFMapScreen::SendRegionSpawnRequest(RegionId region)
{
    ...
    SPARK_LOG_INFO(Spark::LogCategory::Game, "[TF] map deploy request -> region %u",
...
void TFMapScreen::DrawMapContents()
{
    ...
    if (regions.empty()) {
        AddTextCentered(dl, 24.0f, ImVec2(vp->Pos.x + vp->Size.x * 0.5f,
    ...
    m_hexSize = std::clamp(std::min(availW / (maxX - minX + 2.6f),
                                    availH / (maxY - minY + 2.8f)),
                           kHexSizeMin, kHexSizeMax);
    const ImVec2 mapCenter(vp->Pos.x + vp->Size.x * 0.5f,
                           vp->Pos.y + vp->Size.y * 0.52f);
    ...
    for (const RegionDef& rd : regions) {
        ...
        float thick = 1.5f;
        if (rd.id == m_deployHint) { outline = IM_COL32(255, 200, 60, 220); thick = 3.0f; }
        if (rd.id == m_hovered)    { outline = IM_COL32(255, 255, 255, 230); thick = 3.0f; }
        ...
        if (progress > 0.001f && capturing != FactionId::None) {
            dl->PathArcTo(c, m_hexSize * 0.62f, -kPi * 0.5f,
                          -kPi * 0.5f + progress * 2.0f * kPi, 24);
        ...
        if (rd.tier == "skyanchor") {
            ...
            dl->AddTriangleFilled(ImVec2(c.x, c.y - s * 1.5f),
                                  ImVec2(c.x - s, c.y),
                                  ImVec2(c.x + s, c.y),
        ...
        AddTextCentered(dl, 12.0f, ImVec2(c.x, c.y + m_hexSize * 0.38f),
                        IM_COL32(235, 235, 235, 190), TierTag(rd.tier));
        char name[20];
        std::snprintf(name, sizeof(name), "%.16s%s", rd.name.c_str(),
                      rd.name.size() > 16 ? ".." : "");
        AddTextCentered(dl, 12.0f, ImVec2(c.x, c.y + m_hexSize * 1.06f),
                        IM_COL32(210, 210, 214, 200), name);
        ...
        if (dead && rs && myFaction != FactionId::None && rs->CanSpawnAt(rd.id, myFaction)) {
            ...
            dl->AddPolyline(big, 6, ImGui::ColorConvertFloat4ToU32(
                                        ImVec4(0.55f, 0.95f, 0.55f, a)),
...

    {
        ...
        dl->AddRectFilled(ImVec2(base.x - 10.0f, base.y - 10.0f),
                          ImVec2(base.x + 250.0f, base.y + 96.0f),
                          IM_COL32(14, 16, 20, 200), 4.0f);
        float y = base.y;
        for (FactionId f : {FactionId::MRA, FactionId::AUC, FactionId::HLX}) {
            dl->AddRectFilled(ImVec2(base.x, y + 2.0f), ImVec2(base.x + 14.0f, y + 14.0f),
                              FactionCol(f, 0.95f), 2.0f);
            char line[80];
            std::snprintf(line, sizeof(line), "%s  %s  -  %u regions", FactionTag(f),
                          FactionName(f), rs ? rs->RegionsHeld(f) : 0u);
        ...
        if (m_deployHint != kInvalidRegion) {
            ...
            char line[80];
            std::snprintf(line, sizeof(line), "Rally hint: %s",
                          hd ? hd->name.c_str() : "?");
    ...
    if (hoveredDef) {
        ...
        if (progress > 0.001f && capturing != FactionId::None)
            ImGui::Text("Capturing: %s %d%%%s", FactionTag(capturing),
    ...
    // ---- click ------------------------------------------------------------------
    if (hoveredDef && ImGui::IsMouseClicked(ImGuiMouseButton_Left) &&
        ImGui::IsWindowHovered())
    {
        if (dead) {
            if (rs && myFaction != FactionId::None &&
                rs->CanSpawnAt(hoveredDef->id, myFaction))
            {
