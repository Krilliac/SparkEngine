...
namespace Spark
{
    ...
    void HUDSystem::AddDamageFromWorldPos(const DirectX::XMFLOAT3& damageSource, const DirectX::XMFLOAT3& playerPos,
    ...
    void HUDSystem::AddKillFeedEntry(const std::string& killer, const std::string& victim, const std::string& weapon,
                                     bool headshot)
    {
    ...
    {
        m_objectives.erase(std::remove_if(m_objectives.begin(), m_objectives.end(),
    ...
    void HUDSystem::UpdateLowHealthEffects(float dt)
    {
        ...
        if (healthPct < 0.3f)
        {
            ...
            if (m_lowHealthPulseTimer > 6.2832f)
            { // 2*PI
