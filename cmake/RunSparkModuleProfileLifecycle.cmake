cmake_minimum_required(VERSION 3.25)

function(_spark_validate_lifecycle_result child_result child_stdout child_stderr out_ok out_reason)
    set(_ok TRUE)
    set(_reason "")

    if(NOT "${child_result}" STREQUAL "0")
        set(_ok FALSE)
        set(_reason "child exit status was ${child_result}, expected 0")
    else()
        set(_stdout "${child_stdout}")
        set(_stderr "${child_stderr}")
        string(REPLACE "\r\n" "\n" _stdout "${_stdout}")
        string(REPLACE "\r" "\n" _stdout "${_stdout}")
        string(REPLACE "\r\n" "\n" _stderr "${_stderr}")
        string(REPLACE "\r" "\n" _stderr "${_stderr}")

        # Require exactly one standalone record. Logger-prefixed copies are
        # deliberately excluded: only the post-teardown wWinMain write is
        # admissible evidence.
        set(_combined "${_stdout}\n${_stderr}")
        string(REGEX MATCHALL
            "SPARK_D3D11_DEVICE driver=warp certification=software-only"
            _device_records "${_combined}")
        list(LENGTH _device_records _device_record_count)
        if(NOT _device_record_count EQUAL 1)
            set(_ok FALSE)
            set(_reason
                "found ${_device_record_count} explicit WARP D3D11 device records, expected exactly 1")
        endif()
        string(REPLACE ";" "\\;" _combined "${_combined}")
        string(REPLACE "\n" ";" _lines "${_combined}")
        # Count every line carrying the lifecycle token before validating its
        # shape.  Filtering to already-valid records would let a real host
        # record coexist with a malformed or logger-prefixed copy and still
        # produce a false-green lifecycle result.
        set(_lifecycle_candidates)
        foreach(_line IN LISTS _lines)
            if(_line MATCHES "SPARK_MODULE_LIFECYCLE")
                list(APPEND _lifecycle_candidates "${_line}")
            endif()
        endforeach()
        list(LENGTH _lifecycle_candidates _candidate_count)
        if(_ok AND NOT _candidate_count EQUAL 1)
            set(_ok FALSE)
            set(_reason "found ${_candidate_count} lifecycle marker lines, expected exactly 1")
        elseif(_ok)
            list(GET _lifecycle_candidates 0 _marker)
            if(NOT _marker MATCHES
               "^SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=[0-9]+ load=[0-9]+ update=[0-9]+ fixed=[0-9]+ render=[0-9]+ unload=[0-9]+ destroy=[0-9]+ faults=[0-9]+$")
                set(_ok FALSE)
                set(_reason "the single lifecycle marker is not an exact standalone SparkGameFPS record")
            endif()

            if(_ok)
                string(REGEX MATCH "create=([0-9]+)" _unused "${_marker}")
                set(_create "${CMAKE_MATCH_1}")
                string(REGEX MATCH "load=([0-9]+)" _unused "${_marker}")
                set(_load "${CMAKE_MATCH_1}")
                string(REGEX MATCH "update=([0-9]+)" _unused "${_marker}")
                set(_update "${CMAKE_MATCH_1}")
                string(REGEX MATCH "fixed=([0-9]+)" _unused "${_marker}")
                set(_fixed "${CMAKE_MATCH_1}")
                string(REGEX MATCH "render=([0-9]+)" _unused "${_marker}")
                set(_render "${CMAKE_MATCH_1}")
                string(REGEX MATCH "unload=([0-9]+)" _unused "${_marker}")
                set(_unload "${CMAKE_MATCH_1}")
                string(REGEX MATCH "destroy=([0-9]+)" _unused "${_marker}")
                set(_destroy "${CMAKE_MATCH_1}")
                string(REGEX MATCH "faults=([0-9]+)" _unused "${_marker}")
                set(_faults "${CMAKE_MATCH_1}")
            endif()

            if(_ok AND _create LESS 1)
                set(_ok FALSE)
                set(_reason "CreateModule has no successful callback evidence")
            elseif(_ok AND _load LESS 1)
                set(_ok FALSE)
                set(_reason "OnLoad has no successful callback evidence")
            elseif(_ok AND _update LESS 1)
                set(_ok FALSE)
                set(_reason "OnUpdate has no successful callback evidence")
            elseif(_ok AND _fixed LESS 1)
                set(_ok FALSE)
                set(_reason "OnFixedUpdate has no successful callback evidence")
            elseif(_ok AND _render LESS 1)
                set(_ok FALSE)
                set(_reason "OnRender has no successful callback evidence")
            elseif(_ok AND _unload LESS 1)
                set(_ok FALSE)
                set(_reason "OnUnload has no successful callback evidence")
            elseif(_ok AND _destroy LESS 1)
                set(_ok FALSE)
                set(_reason "DestroyModule has no successful callback evidence")
            elseif(_ok AND NOT "${_faults}" STREQUAL "0")
                set(_ok FALSE)
                set(_reason "guarded module callback fault/disabled dispatch count was ${_faults}, expected 0")
            endif()
        endif()
    endif()

    set(${out_ok} "${_ok}" PARENT_SCOPE)
    set(${out_reason} "${_reason}" PARENT_SCOPE)
endfunction()

if(SPARK_LIFECYCLE_PARSER_INCLUDE_ONLY)
    return()
endif()

if(SPARK_LIFECYCLE_PARSER_SELF_TEST)
    function(_spark_expect_lifecycle_case name result stdout stderr expected_ok)
        _spark_validate_lifecycle_result("${result}" "${stdout}" "${stderr}" _actual_ok _reason)
        if(expected_ok AND NOT _actual_ok)
            message(FATAL_ERROR "Lifecycle parser case '${name}' unexpectedly failed: ${_reason}")
        elseif(NOT expected_ok AND _actual_ok)
            message(FATAL_ERROR "Lifecycle parser case '${name}' unexpectedly passed")
        endif()
    endfunction()

    set(_device "[info] SPARK_D3D11_DEVICE driver=warp certification=software-only\n")
    set(_marker
        "SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=1 load=1 update=4 fixed=2 render=4 unload=1 destroy=1 faults=0\n")
    _spark_expect_lifecycle_case(valid 0 "${_device}${_marker}" "" TRUE)
    _spark_expect_lifecycle_case(valid-plus-malformed-token 0
        "${_device}${_marker}SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=1 load=1 update=4 fixed=2 render=4 unload=1 destroy=1 faults=0 extra=1\n"
        "" FALSE)
    _spark_expect_lifecycle_case(nonzero-exit 2 "${_device}${_marker}" "" FALSE)
    _spark_expect_lifecycle_case(missing-warp-record 0 "${_marker}" "" FALSE)
    _spark_expect_lifecycle_case(logger-copy 0 "${_device}[info] ${_marker}" "" FALSE)
    _spark_expect_lifecycle_case(duplicate 0 "${_device}${_marker}${_marker}" "" FALSE)
    _spark_expect_lifecycle_case(unknown-module 0
        "${_device}SPARK_MODULE_LIFECYCLE module=SparkUnknown create=1 load=1 update=4 fixed=2 render=4 unload=1 destroy=1 faults=0\n" "" FALSE)
    _spark_expect_lifecycle_case(malformed-fields 0
        "${_device}SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=1 load=1 update=4 fixed=2 render=4 unload=1 destroy=1 faults=0 extra=1\n" "" FALSE)
    _spark_expect_lifecycle_case(missing-destroy 0
        "${_device}SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=1 load=1 update=4 fixed=2 render=4 unload=1 faults=0\n" "" FALSE)
    _spark_expect_lifecycle_case(zero-create 0
        "${_device}SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=0 load=1 update=4 fixed=2 render=4 unload=1 destroy=1 faults=0\n" "" FALSE)
    _spark_expect_lifecycle_case(zero-load 0
        "${_device}SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=1 load=0 update=4 fixed=2 render=4 unload=1 destroy=1 faults=0\n" "" FALSE)
    _spark_expect_lifecycle_case(zero-update 0
        "${_device}SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=1 load=1 update=0 fixed=2 render=4 unload=1 destroy=1 faults=0\n" "" FALSE)
    _spark_expect_lifecycle_case(zero-fixed 0
        "${_device}SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=1 load=1 update=4 fixed=0 render=4 unload=1 destroy=1 faults=0\n" "" FALSE)
    _spark_expect_lifecycle_case(zero-render 0
        "${_device}SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=1 load=1 update=4 fixed=2 render=0 unload=1 destroy=1 faults=0\n" "" FALSE)
    _spark_expect_lifecycle_case(zero-unload 0
        "${_device}SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=1 load=1 update=4 fixed=2 render=4 unload=0 destroy=1 faults=0\n" "" FALSE)
    _spark_expect_lifecycle_case(zero-destroy 0
        "${_device}SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=1 load=1 update=4 fixed=2 render=4 unload=1 destroy=0 faults=0\n" "" FALSE)
    _spark_expect_lifecycle_case(nonzero-faults 0
        "${_device}SPARK_MODULE_LIFECYCLE module=SparkGameFPS create=1 load=1 update=4 fixed=2 render=4 unload=1 destroy=1 faults=1\n" "" FALSE)
    message(STATUS "SparkGameFPS D3D11 lifecycle parser contract passed")
    return()
endif()

foreach(_required
        SPARK_ENGINE_EXECUTABLE
        SPARK_GAME_MODULE
        SPARK_WORKING_DIRECTORY
        SPARK_RHI_BACKEND)
    if(NOT DEFINED ${_required} OR "${${_required}}" STREQUAL "")
        message(FATAL_ERROR "RunSparkModuleProfileLifecycle.cmake requires -D${_required}=<value>")
    endif()
endforeach()

if(NOT SPARK_RHI_BACKEND STREQUAL "d3d11")
    message(FATAL_ERROR
        "The stable-v1 source lifecycle smoke requires SPARK_RHI_BACKEND=d3d11")
endif()

if(NOT EXISTS "${SPARK_ENGINE_EXECUTABLE}")
    message(FATAL_ERROR "SparkEngine executable is missing: ${SPARK_ENGINE_EXECUTABLE}")
endif()
if(NOT EXISTS "${SPARK_GAME_MODULE}")
    message(FATAL_ERROR "Game module is missing: ${SPARK_GAME_MODULE}")
endif()
if(NOT IS_DIRECTORY "${SPARK_WORKING_DIRECTORY}")
    message(FATAL_ERROR "Lifecycle working directory is missing: ${SPARK_WORKING_DIRECTORY}")
endif()

execute_process(
    COMMAND "${CMAKE_COMMAND}" -E env
        "SPARK_RHI_BACKEND=${SPARK_RHI_BACKEND}"
        "SPARK_D3D11_DRIVER=warp"
        "${SPARK_ENGINE_EXECUTABLE}"
        -game "${SPARK_GAME_MODULE}"
        -require-game
        -test-seconds 1.0
        -threads 2
        -window-size 640x360
        -no-subprocess
    WORKING_DIRECTORY "${SPARK_WORKING_DIRECTORY}"
    RESULT_VARIABLE _result
    OUTPUT_VARIABLE _stdout
    ERROR_VARIABLE _stderr
    TIMEOUT 90
    ENCODING UTF-8
)

_spark_validate_lifecycle_result("${_result}" "${_stdout}" "${_stderr}" _lifecycle_ok _lifecycle_reason)
if(NOT _lifecycle_ok)
    message(FATAL_ERROR
        "SparkGameFPS D3D11 production lifecycle evidence failed: ${_lifecycle_reason}.\n"
        "stdout:\n${_stdout}\n"
        "stderr:\n${_stderr}")
endif()

message(STATUS "SparkGameFPS D3D11 production lifecycle completed with all module phases evidenced")
