...
public:
    ...
    bool HasQuest(uint32_t id) const {
        return m_quests.count(id) > 0;
    }

    const std::unordered_map<uint32_t, QuestDef>& GetAllQuests() const {
        return m_quests;
    }
...
namespace QuestOps {
     ...
     */
    inline bool StartQuest(QuestJournalComponent& journal, const QuestRegistry& registry,
                           uint32_t questId) {
        ...
        for (const auto& aq : journal.activeQuests) {
            if (aq.questId == questId) return false; // already active
        }
    ...
    inline void CleanupFinished(QuestJournalComponent& journal) {
        journal.activeQuests.erase(
            std::remove_if(journal.activeQuests.begin(), journal.activeQuests.end(),
                [](const ActiveQuest& aq) {
