...
namespace Spark
{
    ...
    Vehicle::Vehicle(const VehicleDefinition& def)
        : m_definition(def), m_health(def.maxHealth), m_armor(def.maxArmor), m_seats(def.seats)
    {
        SetName(def.name);
    }
    ...
    void Vehicle::Update(float dt)
    {
        ...
        for (size_t i = 0; i < m_seats.size(); ++i)
        {
            ...
            {
                XMFLOAT3 seatWorldPos = {pos.x + m_seats[i].localOffset.x, pos.y + m_seats[i].localOffset.y,
    ...
    void Vehicle::FireWeapon(int seatIndex)
    {
        if (seatIndex < 0 || seatIndex >= (int)m_seats.size())
            return;
        if (m_seats[seatIndex].role != VehicleSeatRole::GUNNER && m_seats[seatIndex].role != VehicleSeatRole::DRIVER)
            return;
    ...
    bool Vehicle::EnterSeat(Player* player, int seatIndex)
    {
        ...
        if (seatIndex < 0 || seatIndex >= (int)m_seats.size())
            return false;
        if (m_seats[seatIndex].isOccupied)
            return false;
    ...
    int Vehicle::GetDriverSeatIndex() const
    {
        for (int i = 0; i < (int)m_seats.size(); ++i)
        {
            if (m_seats[i].role == VehicleSeatRole::DRIVER || m_seats[i].role == VehicleSeatRole::PILOT)
            {
                return i;
            }
        }
    ...
    void Vehicle::UpdateAerialVehicle(float dt)
    {
        ...
        if (m_throttle > 0)
        {
            m_currentSpeed = std::min(handling.maxSpeed, m_currentSpeed + handling.acceleration * m_throttle * dt);
        }
        else if (m_throttle < 0)
        {
            m_currentSpeed = std::max(-handling.reverseSpeed, m_currentSpeed + handling.acceleration * m_throttle * dt);
        }
        else
        {
    ...
    void VehicleSystem::Update(float deltaTime)
    {
        ...
        m_vehicles.erase(std::remove_if(m_vehicles.begin(), m_vehicles.end(), [](const std::unique_ptr<Vehicle>& v)
    ...
    void VehicleSystem::RemoveVehicle(Vehicle* vehicle)
    {
        m_vehicles.erase(std::remove_if(m_vehicles.begin(), m_vehicles.end(),
                                        [vehicle](const std::unique_ptr<Vehicle>& v) { return v.get() == vehicle; }),
                         m_vehicles.end());
    }
    ...
    std::string VehicleSystem::Console_ListVehicles() const
    {
        ...
        for (size_t i = 0; i < m_vehicles.size(); ++i)
        {
            ...
            ss << "  [" << i << "] " << v->GetVehicleName() << " HP:" << v->GetHealth() << "/" << v->GetMaxHealth()
               << " Pos:(" << v->GetPosition().x << "," << v->GetPosition().y << "," << v->GetPosition().z << ")"
               << (v->IsOccupied() ? " [OCCUPIED]" : " [EMPTY]") << "\n";
        }
        return ss.str();
    }

    bool VehicleSystem::Console_SpawnVehicle(const std::string& type, float x, float y, float z, ID3D11Device* device,
                                             ID3D11DeviceContext* context)
    {
        VehicleType vType = VehicleType::BUGGY;
        if (type == "buggy")
            vType = VehicleType::BUGGY;
        else if (type == "tank")
            vType = VehicleType::TANK;
        else if (type == "apc")
            vType = VehicleType::APC;
        else if (type == "motorcycle")
            vType = VehicleType::MOTORCYCLE;
        else if (type == "truck")
            vType = VehicleType::TRUCK;
        else if (type == "helicopter")
            vType = VehicleType::HELICOPTER;
        else if (type == "jet")
            vType = VehicleType::JET;
        else if (type == "dropship")
            vType = VehicleType::DROPSHIP;
        else if (type == "drone")
            vType = VehicleType::DRONE;
        else
            return false;
    ...
    void VehicleSystem::InitAPC()
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            VehicleSeat passenger(VehicleSeatRole::PASSENGER, {(i % 2 == 0 ? -0.8f : 0.8f), 0.8f, -1.0f - i * 0.8f});
            def.seats.push_back(passenger);
        }
    ...
    void VehicleSystem::InitHelicopter()
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            VehicleSeat pass(VehicleSeatRole::PASSENGER, {(i % 2 == 0 ? -1.2f : 1.2f), 0.2f, -0.5f - (i / 2) * 0.8f});
    ...
    void VehicleSystem::InitDropship()
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
            VehicleSeat pass(VehicleSeatRole::PASSENGER, {(i % 2 == 0 ? -2.0f : 2.0f), 0.5f, 2.0f - i * 0.8f});
            def.seats.push_back(pass);
        }
