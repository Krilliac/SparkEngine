...
HRESULT Terrain::Initialize(ID3D11Device* /*device*/, ID3D11DeviceContext* /*ctx*/,
    const wchar_t* file, const TerrainDesc& desc)
{
    m_desc = desc;
    ...
    if (file) {
        ...
        if (in) {
            ...
            in.read(reinterpret_cast<char*>(data.data()),
                    static_cast<std::streamsize>(desc.width) * desc.height);
        }
    }
...
HRESULT Terrain::Initialize(ID3D11Device* device, ID3D11DeviceContext* ctx,
    const wchar_t* file, UINT w, UINT h, float s)
{
    TerrainDesc desc;
    desc.width = w;
    desc.height = h;
...
void Terrain::BuildMesh(const std::vector<uint8_t>& h, UINT w, UINT hgt, float s)
{
    ...
    for (UINT z = 0; z < hgt; ++z) {
        for (UINT x = 0; x < w; ++x) {
            ...
            m_vertices[idx] = {
                { static_cast<float>(x) - w / 2.0f, height, static_cast<float>(z) - hgt / 2.0f },
                { 0, 1, 0 },
...
{
    if (m_vertices.empty() || m_heightData.empty()) return;
...
void Terrain::ModifyHeight(const XMFLOAT3& worldPos, float radius, float strength)
{
    ...
    int cx, cz;
    if (!WorldToHeightmap(worldPos.x, worldPos.z, cx, cz)) return;

    int r = static_cast<int>(radius / m_desc.cellSpacing) + 1;
    for (int z = cz - r; z <= cz + r; ++z) {
        for (int x = cx - r; x <= cx + r; ++x) {
            if (x < 0 || x >= (int)m_desc.width || z < 0 || z >= (int)m_desc.height) continue;
...
{
    if (m_vertices.empty() || layerIndex < 0 || layerIndex > 3) return;
    int cx, cz;
    if (!WorldToHeightmap(worldPos.x, worldPos.z, cx, cz)) return;
    ...
    for (int z = cz - r; z <= cz + r; ++z) {
        for (int x = cx - r; x <= cx + r; ++x) {
            ...
            if (total > 0.0f) { w[0] /= total; w[1] /= total; w[2] /= total; w[3] /= total; }
        }
    }
    m_meshDirty = true;
}
...
{
    int hx, hz;
    if (!WorldToHeightmap(worldX, worldZ, hx, hz)) return 0.0f;
...
{
    int hx, hz;
    if (!WorldToHeightmap(worldX, worldZ, hx, hz)) return { 0, 1, 0 };
...
{
    if (x < 0 || x >= (int)m_desc.width || z < 0 || z >= (int)m_desc.height) return 0.0f;
