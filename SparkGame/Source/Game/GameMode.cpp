...
int GameMode::GetTeamScore(Team team) const {
    if (team == Team::Alpha) return m_alphaScore;
    if (team == Team::Bravo) return m_bravoScore;
    return 0;
}
...
void GameMode::CheckWinCondition() {
    if (m_rules.type == GameModeType::FreePlay) return;
...
Team GameMode::GetLeadingTeam() const {
    if (!m_rules.teamsEnabled) return Team::None;
    if (m_alphaScore > m_bravoScore) return Team::Alpha;
    if (m_bravoScore > m_alphaScore) return Team::Bravo;
...
const char* GameMode::GameModeTypeToString(GameModeType type) {
    switch (type) {
    case GameModeType::FreePlay:          return "Free Play";
    case GameModeType::Deathmatch:        return "Deathmatch";
    case GameModeType::TeamDeathmatch:    return "Team Deathmatch";
    case GameModeType::CaptureTheFlag:    return "Capture the Flag";
    case GameModeType::Domination:        return "Domination";
    case GameModeType::Elimination:       return "Elimination";
    case GameModeType::GunGame:           return "Gun Game";
    case GameModeType::SearchAndDestroy:  return "Search and Destroy";
    case GameModeType::KingOfTheHill:     return "King of the Hill";
