...
`fps_starter_atlas.png` is repository-original generated artwork directed specifically for this template. It contains a tactical training rifle, target drone, HUD glyphs, material swatches, decals, and an arena emblem on transparency. The reflected scene pairs the built-in ground primitive with the repository-original `training_target`, `cover_barrier`, `arena_wall`, and `training_rifle` OBJ/MTL sets. Host rendering/UI adapters can slice the atlas using their normal asset pipeline.
